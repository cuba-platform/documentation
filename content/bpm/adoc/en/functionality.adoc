...
BPM subsystem allows creating custom elements for process model designer. Basically, a custom element is `ServiceTask` that saves the developer from the necessity of typing long expressions for method invocation, like this `${app_MyBean.someMethod(argument1, 'argument2')}`.

[[custom_stencils_example]]
==== Example of Element Creation

Suppose, there is a middleware bean with a name `app_DiscountManager`. There is a `makeDiscount(int discountPercent, UUID contractId)` method in the bean. The method updates the contract amount by subtracting the discount.

In this example we will create a custom model element, that will invoke the method. The discount percent will be defined as a parameter of a model element.
...
* Bean name: *app_DiscountManager*

* Method name: *makeDiscount*

Add method arguments according to the screenshot. Arguments must be added in the proper order. The *Caption* column is a caption in element properties panel.
...
TIP: `entityId` is a name of the process variable. This process variable is added automatically to each process that is linked to the entity. The variable stores an entity identifier, you can use it in any method calls.

During the process deployment a custom element will be transformed to the serviceTask:
...
==== Elements Import and Export

The elements set can be exported to the ZIP archive and then restored from the archive. It may be useful when new elements are created on a developer's machine and then imported to the production server. Import and export are performed with the corresponding buttons on the elements editor screen.

[[custom_stencils_reset]]
==== Reset elements set

*Reset* button removes all custom groups and elements and reverts the elements set to its initial state.
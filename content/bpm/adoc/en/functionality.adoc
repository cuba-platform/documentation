...
Suppose, there is a middleware bean with the `app_DiscountManager` name. There is a `makeDiscount(int discountPercent, UUID contractId)` method in the bean. The method updates the contract amount by subtracting the discount.
...
[WARNING]
====
The *Bean name* lookup contains only beans that implements an interface. The *Method name* lookup contains methods of implemented interfaces.
====

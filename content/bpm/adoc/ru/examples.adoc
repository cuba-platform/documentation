...
Ниже приведён пример программного запуска отчёта с заданными ролями из контроллера экрана редактирования сущности с помощью сервиса `ProcessRuntimeService`. Полный код данного тестового приложения можно посмотреть  на https://github.com/Haulmont/cuba-bpm[GitHub].

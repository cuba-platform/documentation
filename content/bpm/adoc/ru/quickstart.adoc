...
В секции *Data model* панели навигатора выделите сущность Contract и нажмите кнопку *New > Generic UI screen*. В списке шаблонов выделите *Entity browser* и нажмите *Create*, затем выделите *Entity editor* и снова нажмите *Create*. Закройте список шаблонов.
...
В студии в секции *Data model* навигатора нажмите на *Generate DB scripts*. В открывшемся окне *Database scripts* нажмите кнопку *Create database*.

image::StudioCreateDatabase.png[align="center"]

Запустите сервер, выполнив команду *Run -> Start application server*.

Откройте приложение в браузере по адресу http://localhost:8080/app или кликните на ссылку в панели статуса студии.

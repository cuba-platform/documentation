...
* `chart:balloon` - sets the configurations of balloons (tooltips) of the chart that follow the mouse cursor when you roll-over the data items. The balloon text is defined by the `balloonText` attribute of each chart graph. For example:
+
--
...
* `withHeader` - add header row with column names. Work for CSV and XLSX format. Default value is `true`.
--
...
+
--
* `arrows` - includes the nested `arrow` elements for the chart axes.
+
[source, xml]
----
<chart:arrows>
...
+
The `item` elements enable you to set fixed values for the chart.
+
[source, xml]
----
<chart:funnelChart id="ratingChart"
...
. Using the `chart:data` element.
+
The `item` elements enable you to set key-value pairs of data for the chart.
+
[source, xml]
----
<chart:pieChart id="pieChart"

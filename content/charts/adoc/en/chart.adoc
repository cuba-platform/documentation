...
[[chart_accessibleDescription]]
The `accessibleDescription` attribute enables setting the description text for the chart SVG element.

[[chart_renderingDelay]]
By default, all charts are already drawn when the screen is opened. You can use the `renderingDelay` attribute if you want the user to see the chart drawing animation. The delay time is set in milliseconds and can be applied to all chart types except <<stock_chart,StockChart>>.
...
The `bezierX` and `bezierY` attributes of `chart:serialChart` enable to set horizontal and vertical tension of smoothed chart line.

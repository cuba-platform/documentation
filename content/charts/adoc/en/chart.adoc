...
At the moment of writing, the chart display subsystem supports only the *Web Client* for *Vaadin7*.

AmCharts library which is the basis of the chart display subsystem is distributed under a license, which allows its free
use if you keep the link to the library website. Alternatively, you can link:$$http://www.amcharts.com/online-store$$[purchase^]
an AmCharts license for your project and remove the link.

[[chart_dependency]]
=== Add Charts Dependency
To use charts in project, you need to activate the *Charts* subsystem in the *Base projects* list in the *Project properties* page ( *Project properties* -> *Edit*).
...
link:$$http://docs.amcharts.com/3/javascriptcharts$$[docs.amcharts.com/3/javascriptcharts^].
...
providing chart data is the most universal, but it requires you to create instances of `DataProvider` and `DataItem`
...
For the first example we will create the chart similar to link:$$https://www.amcharts.com/demos/3d-stacked-column-chart/$$[3D Stacked Column Chart^]
...
image::chart/stock-chart-with-datasets.svg[align="center"]
...
image::chart/stock-chart-with-stockevents.svg[align="center"]
...
image::chart/stock-chart-with-panels.svg[align="center"]

[[chart_replacement]]
=== AmCharts Replacement with Another Version

An instance of AmCharts library included into CUBA platform could be replaced with another. To do this:

. Download charts and stock charts from link:$$https://www.amcharts.com/download/$$[AmCharts site^].
. Merge `amcharts` folder from both archives in one.
. Copy `amcharts` folder to `{project.rootDir}/modules/web/web/VAADIN/resources`
. Redeploy the application.

To use new attributes added in new version you need to set custom `json` in screen controller as shown below.

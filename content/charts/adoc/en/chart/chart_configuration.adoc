...
Charts can be described both in a {main_man_url}/screen_descriptors.html[screen XML-descriptor] or in a {main_man_url}/screen_controllers.html[screen controller]. To do this, you should connect the corresponding `namespace` to the descriptor:
...
Each chart type has its own set of attributes and methods, which replicate the functionality of the corresponding charts from AmCharts library. Documentation on the properties and methods of the charts is available at http://docs.amcharts.com/3/javascriptcharts.
...
Messages used for displaying charts can be overridden or localized in the main message pack of the web module. Some localizations are available on CUBA https://github.com/cuba-platform/translations[GitHub].
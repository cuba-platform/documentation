...
Let us consider the use of events. We will add handling of a graph item click to the screen created in <<cdb_screen_controller>>. Open the screen controller and inject the chart. In order to show a notification, inject the `Notifications` bean into the screen controller.

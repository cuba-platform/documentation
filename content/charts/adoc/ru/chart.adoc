...
Атрибут `accessibleDescription` позволяет задать текстовое описание SVG-элемента диаграммы.
...
Атрибуты `bezierX` и `bezierY` компонента `chart:serialChart` позволяют задать натяжение сплайнов кривых линии диаграммы по горизонтали и вертикали.

...
* `chart:balloon` - задаёт настройки всплывающих пузырьковых подсказок, появляющихся при наведении курсора мыши на элемент данных диаграммы. Текст подсказки указывается в атрибуте `balloonText` для каждого графа. Например:
+
--

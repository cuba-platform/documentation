...
* `properties` – коллекция ключ-значение атрибутов `pivotTable`, где ключ – имя атрибута в {main_man_url}/gui_data_containers.html[контейнере данных], а значение – его локализованное значение, используемое при отображении.
...
* `rowOrder` – порядок отображения строк <<chart_PivotTable_renderers,рендерером>> таблицы.
...
* `dataContainer` – ссылка на {main_man_url}/gui_data_containers.html[контейнер данных], указанный в секции `data` XML-дескриптора экрана. Контейнер должен иметь тип `CollectionContainer`.

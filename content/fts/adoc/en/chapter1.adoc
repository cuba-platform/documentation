...
Search results are filtered according to the limitations applied by the platform&rsquo;s security subsystem. I.e. if the current user&rsquo;s access group does not allow access to certain entity instances, such instances will not appear in search results.

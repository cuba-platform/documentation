...
Once the steps above are completed, full text search functionality will be added to the application and ready to work. If you log out of the system and then log in again, a search field will appear in the top right panel of the main application window. *Full text search* can be also used together with link:{main_man_url}/gui_Filter.html[generic Filters]: you can add one FTS condition to a filter by clicking *Add search condition* link and selecting *FTS condition*.

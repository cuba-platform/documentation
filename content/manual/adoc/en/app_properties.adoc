...
+
--
Defines a <<rest_api_v2,>> token expiration timeout for the default client in seconds.
...
[[cuba.web.standardAuthenticationUsersWhiteList]]
cuba.web.standardAuthenticationUsersWhiteList::
+
--
A comma-separated list of user logins that are allowed to log in to the system using standard authentication if <<cuba.web.externalAuthentication,external authentication>> was unsuccessful. So if the password (hash actually) of a user from this list is not empty in the main database, and they entered the valid password, they still can log in even if the external authentication failed.

An empty list means nobody can log in bypassing the external authentication.

...
+
--
Enables invoking setters for unloaded attributes of <<entity_states,Detached>> entities. For more information, see <<views,>>.
...
+
--
Prohibits automatic generation of the `orm.xml` file for <<extension,extended>> entities. Enables creating and registering this file manually in the `openjpa.MetaDataFactory` parameter of the <<persistence.xml,persistence.xml>> file.

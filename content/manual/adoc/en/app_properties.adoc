...
+
--
Defines the <<rest_api,REST API v1>> version. If the value is set to `1`, the REST API of the platform versions before 5.4 is used. Setting the value to `2` enables the new REST API version with support for service calls. This property is *not applicable* to <<rest_api_v2,REST API v2>>.

...
+
--
A comma-separated list of user logins that should log in to the system using standard authentication instead of external authentication such as <<ldap,LDAP>> or <<sso,IDP SSO>>.

An empty list means everybody can log in using the external authentication if it is on.

...
+
--
For a <<sso,SSO>> Identity Provider, sets a list of service provider URLs. The values must be separated with commas. Trailing '/' in URLs is required.
...
+
--
For a <<sso,SSO>> Identity Provider, sets a comma-separated list of Java Regex masks for permitted service URLs. This property enables to handle external links correctly if user is not signed in.

Choose the masks carefully to avoid redirecting to untrusted URLs, so trailing '/' in URLs is required.
...
+
--
A comma-separated list of users that are not allowed to use <<cuba.web.externalAuthentication,external authentication>> (using <<sso,SSO>> Identity Provider) and should log in to the system using standard authentication only, see also <<cuba.web.standardAuthenticationUsers,cuba.web.standardAuthenticationUsers>>.
...
Possible values are the elements of `com.haulmont.cuba.web.gui.UrlHandlingMode` enumeration:

* NONE URL – changes are not handled at all;

* BACK_ONLY – `CubaHistoryControl` is used to handle changes. This value replaces the old property <<cuba.web.allowHandleBrowserHistoryBack,cuba.web.allowHandleBrowserHistoryBack>>;

* URL_ROUTES – changes are handled by <<url_manual_ui_state_reflection,UrlRouting>>, <<url_history,History>> and <<url_changes_handling_and_ui_state_reflection,UrlChangeHandler>>.

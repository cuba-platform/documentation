...
+
--
Allows the <<gui_Filter,generic filter>> to use the immediate mode for filtering. When set to `true`, the filter works in the immediate mode when every change of filter parameters automatically reloads data. When set to `false`, the filter works in the explicit mode. In this case, the filter will be applied only after the *Search* button is clicked. See also <<gui_filter_immediately,applyImmediately>> filter attribute.

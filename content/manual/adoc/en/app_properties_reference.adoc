...
+
--
Determines whether the server should run the <<db_update_server,database update scripts>> at the application start.

Since CUBA 7.2, you can control the execution of database update scripts separately for the main and additional data stores: for the main data store use the `cuba.automaticDatabaseUpdate_MAIN` property, for additional data stores use properties in the form of `cuba.automaticDatabaseUpdate_<store_name>`. The concrete properties have priority over the common one.
...
Default value: `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/conf`, which points to a subdirectory of the <<app_home,application home>>.
...
+
--
Indicates that <<dataManager,DataManager>> should check entity operation <<permissions,permissions>> and in-memory <<constraints,constraints>> on the middle tier.
...
Default value: `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/work`, which points to a subdirectory of the <<app_home,application home>>.
...
+
--
When using <<legacy_roles>>, defines the set of files with the user's default permissions. Default permission values are used when no <<roles,role>> defines an explicit value for permission target. Used mostly for denying roles, see more in the <<default-permission-values.xml,default-permission-values.xml>> section.
...
+
--
If set to `true`, <<dataManager,DataManager>> checks entity attribute <<permissions,permissions>>. When `false`, attribute permissions are checked only in <<gui_framework,Generic UI>> data-aware components and {rest_man_url}[REST API] endpoints.
...
When set to `true`, the <<gui_Filter,generic filter>> works in the immediate mode when every change of filter parameters automatically reloads data. When set to `false`, the filter will be applied only after the *Search* button is clicked. See also <<gui_filter_immediately,applyImmediately>> filter attribute.
...
+
--
Defines the properties hierarchy depth in the "Add Condition" dialog window. For example, if the depth value is 2, then you can select an entity attribute `contractor.city.country`, if the value is 3, then `contractor.city.country.name`, etc.
...
+
--
Defines the layout of the dialog displayed when a user attempts to close a <<gui_screens,screen>> with unsaved changes in <<gui_DataContext,DataContext>>.
...
* Row-level <<constraints,security constraints>> are filtered out some elements of a collection attribute. In this case, the security token is also included to JSON returned by REST API, see {rest_man_url}#rest_api_v2_security_constraints[REST API documentation].
...
+
--
Specifies the location of the <<log_dir,log folder>> for an <<app_tiers,application block>>.

Default value: `${app.home}/logs`, which points to the `logs` subdirectory of the <<app_home,application home>>.
...
+
--
Defines expiration timeout for "remember me" cookies and `RememberMeToken` entity instances.

Default value: `30 * 24 * 60 * 60` (30 days)
...
+
--
For backward compatibility, enables using <<default-permission-values.xml>> configuration file. See <<legacy_roles>> for details.
...
+
--
For backward compatibility, determines the security roles behavior. See <<legacy_roles>> for details.
...
+
--
Enables synchronous sending of new user sessions created on login to the cluster. See <<cluster_mw_server_sync_session_repl,Synchronous replication of user sessions>> for details.
...
Default value: `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/temp`, which points to a subdirectory of the <<app_home,application home>>.
...
+
--
Defines password used to create `TrustedClientCredentials`. The Middleware layer can authenticate users who connect via the trusted client <<app_tiers,block>> without checking the user password.
...
+
--
A regular expression which is used by <<unique_constraint_violation_exceptions, UniqueConstraintViolationHandler>> to find out that the exception is caused by a database unique constraint violation. The constraint name will be obtained from the first non-empty group of the expression. For example:
...
+
--
When `true`, the Web Client and Web Portal <<app_tiers,blocks>> invoke the Middleware services locally bypassing the network stack, which has a positive impact on system performance. It is possible in the case of <<fast_deployment,fast deployment>>, single <<build.gradle_buildWar,WAR>> and single <<build.gradle_buildUberJar,Uber JAR>>. This property should be set to false for all other deployment options.
...
+
--
Enables the <<folders_pane,folders panel>> functionality and using <<shortcuts,keyboard shortcuts>> in the folders.
...
+
--
Determines whether the `HtmlSanitizer` bean should be used by the UI components implementing the `HasHtmlSanitizer` interface to to prevent cross-site scripting (XSS) in HTML content. The sanitization can also be enabled or disabled by individual components using <<gui_attr_htmlSanitizerEnabled,htmlSanitizerEnabled>> attribute.
...
+
--
Defines what screen will be open for non-authenticated users when they first open the application URL. Requires the <<cuba.web.allowAnonymousAccess,cuba.web.allowAnonymousAccess>> to be set to `true`.
...
+
--
Identifier of a screen to be used as <<gui_LoginScreen,login screen>> of the application.
...
+
--
Identifier of a screen to be used as <<gui_MainScreen,main screen>> of the application.

...
+
--
Defines the set of files with the user's default permissions. Default permission values are used when no <<roles,role>> defines an explicit value for permission target. It is necessary mostly for denying roles: without this file the user with a denying role by default doesn't have access to <<main_window_layout,mainWindow>> screen and to <<gui_Filter,filter>> screens.

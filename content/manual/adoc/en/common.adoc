...
The platform has a mechanism for the protection against password brute force cracking. The protection is enabled by the `cuba.bruteForceProtection.enabled=true` application property in the Middleware tier. If the protection is enabled then the combination of user login and IP address is blocked for a time interval in case of multiple unsuccessful login attempts.

A maximum number of login attempts for the combination of user login and IP address is defined by the `cuba.bruteForceProtection.maxLoginAttemptsNumber` application property (default value is 5).

Blocking interval in seconds is defined by the `cuba.bruteForceProtection.blockIntervalSec` application property (default value is 60).

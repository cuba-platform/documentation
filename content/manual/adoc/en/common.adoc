...
* The class and its fields (or access methods if the attribute has no corresponding field) must be <<entity_annotations,annotated>> in a definite way for correct operation of <<jpa, JPA>> (in case of a persistent entity) and the <<metadata_framework,metadata framework>>.
...
* `BaseGenericIdEntity` – implements `BaseEntity` with added <<jpa, JPA>> annotations without specifying the type of the identifier (i.e. the primary key) of the entity.
...
Annotations of the `javax.persistence` package are needed for <<jpa, JPA>>, annotations of `com.haulmont.*` packages are designed for <<metadata_framework,metadata>> management and control of other mechanisms in the platform.
...
+
--
Indicates that the annotated field stores version for <<optimistic_locking, optimistic locking>> support.
...
In soft deletion mode, the platform automatically filters out the deleted instances when loading by `id` and when using <<jpql, JPQL queries>>, as well as the deleted elements of the related entities in collection attributes. However, related entities in single-value attributes are loaded, regardless of whether the related instance was deleted or not.
...
<<lazy_loading,Lazy loading>> can not help in most cases because data processing is usually performed not in the transaction where the entities were loaded but, for example, on client <<app_tiers,tier>> in UI. At the same time it is unacceptable to apply <<eager_fetching, eager fetching>> by <<entity_annotations,entity annotations>> as it leads to permanent retrieval of the entire graph of related entities which can be very large.
...
* In the process of data loading using <<dataManager,DataManager>> the client code provides required view together with <<jpql, JPQL>> query.
...
Also, like any other beans, infrastructure interfaces can be obtained using static methods of `AppBeans` class, and can be used in non-managed components (<<pojo, POJO>>, helper classes etc.).

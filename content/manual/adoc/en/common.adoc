...
The platform has a mechanism for the protection against password brute force cracking. The protection is enabled by the `cuba.bruteForceProtection.enabled=true` application property on Middleware. If the protection is enabled then the combination of user login and IP address is blocked for a time interval in case of multiple unsuccessful login attempts.

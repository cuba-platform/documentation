...
See also <<login-events,Login Events>>.
--
...
----
+
Alternatively, the <<events, application events>> mechanism can be used for the same purpose, for example:
+
[source, java]
----
include::{sourcesdir}/common/appcontext_2.java[]
----
+
--
...
You can also place the message in a <<message_packs,localized messages pack>> and use the following format to specify the message in an annotation: `{msg://message_pack/message_key}` or simply `{msg://message_key}`. For example:

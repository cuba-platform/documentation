...
`Events` bean encapsulates application-scope event publication functionality. Application events can be used to exchange information between loosely coupled components. `Events` bean is a simple facade for ApplicationEventPublisher of the Spring Framework.
...
It has only one method `publish()` that receive an event object. `Events.publish()` notifies all matching listeners registered with this application of an application event. You can use `PayloadApplicationEvent` to publish any object as an event.
...
First of all, we have to create a new event class. It should extend ApplicationEvent class. An event class can contain any additional data. For instance:
[source, java]
----
...
Beans can publish an event using Events bean:
[source, java]
----
@Component
...
In the first case, we have to create a bean that implements `ApplicationListener` with type of an event:
[source, java]
----
@Component
...
+
--
Usually, `Events` delegates event publishing to the ApplicationContext. On web tier you can use a special interface for event classes - `UiEvent`. It is a marker interface for events that are sent to UIs screens in the current UI instance (the current web browser tab). Please note that UiEvent instances are not sent to Spring Framework beans.
...
If an event is `UiEvent` and fired using Events bean from UI thread then opened windows / frames with such methods will receive the event. Event handling is synchronous. Only opened UI screens of current web browser tab of a User receive the event.
--

...
+
--
The localize messages in the exception window, you should override the localization keys for the `DeletePolicyHandler` in the main message pack:
...
The `showExceptionDialog()` method is intended to display an exception dialog with the stacktrace of given throwable. The method is applied to `WindowManager` and has the following parameters:
...
[source, java]
----
windowManager.showExceptionDialog(new Throwable(), "Exception is thrown", "Something is wrong");
----

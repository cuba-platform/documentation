...
. The `UNLINK` policy for one-to-many and many-to-many collection attributes is not supported: `UnsupportedOperationException` will be thrown if you try to delete the `Owner` entity instance (on the owner side of the association).
+
`Owner.java`
+
[source, java]

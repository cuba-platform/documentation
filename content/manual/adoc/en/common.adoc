...
+
Time-consuming operations can be annotated as `@ManagedOperation` with special `description` parameters:
+
--
* `@JmxLongOperation` - if timeout can not or should not be set,

* `@JmxLongOperation(timeout)` - to set the timeout in milliseconds.

[source, java]
----
@ManagedOperation(description = "Recalculates an order amount @JmxLongOperation(3000)")
----
--
+

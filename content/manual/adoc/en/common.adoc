...
Properties that determine configuration and deployment parameters are specified in special property files named according to the `*app.properties` pattern. Each application <<app_tiers,block>> contains a set of such files, including files from the platform <<base_projects,base projects>>. The set of property files is defined as follows:

...
The platform offers a mechanism for managing related entities when deleting, which is largely similar to *ON DELETE* rules for database foreign keys. This mechanism works on the middle <<app_tiers,tier>> and uses <<onDelete_annotation,@OnDelete>>, <<onDeleteInverse_annotation,@OnDeleteInverse>> annotations on entity attributes. The entity should support <<soft_deletion,soft deletion>>.

...
Enumerations can be created in Studio on the `Enumerations` tab. To be used as an entity attribute, choose `ENUM` in the `Attribute type` field of the attribute editor and select the Enumeration class in the `Type` field. Enumeration values can be associated with <<enum_localization,localized names>> that will be displayed in the user interface of the application.

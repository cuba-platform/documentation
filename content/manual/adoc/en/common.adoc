...
* `deletePolicy.caption.Customer` - notification caption for concrete entity.

* `deletePolicy.references.message.Customer` - notification message for concrete entity.
--
...
* <<metadata.xml,metadata.xml>> files from <<app_components,application components>> and from the project itself are scanned for the `datatypes` element and if it is found, custom `Datatype` implementations are loaded. See an example of a custom datatype <<datatype_custom_example,below>>.
...
`Datatype` determines two sets of methods for formatting and parsing: considering and not considering locale. Conversion considering locale is applied everywhere in user interface, ignoring locale – in system mechanisms, for example, serialization in <<rest_api_v2,REST API>>.
...
The parsing formats considering locale are provided in the <<main_message_pack,main messages pack>>, in the strings with the following keys:
...
All the listed formats are specified in the main message pack of CUBA <<app_components,application component>> by default, and can be overridden in the similar files of the application project.
...
For locale-dependent conversion of some widely used data types to and from strings you can use `DatatypeFormatter`. It provides easy means of formatting based on the current locale. For example:
...
`EntityImportExportService` interface provides methods for exporting a collection of entities and importing them.

Interface methods:

* `exportEntitiesToJSON()` - serializes a collection of entities to JSON using `EntitySerializationAPI`.
+
[source, java]
----
...
* `exportEntitiesToZIP()` - serializes a collection of entities to JSON and packs the JSON file into ZIP archive. In the following example the ZIP archive is saved to the storage using the <<fileLoader,FileLoader>> interface:
+
[source, java]
----
...
* `importEntities()` - persists entities according to the rules, described by the `entityImportView` parameter. If the entity is not present in the database, it will be saved. Otherwise the fields of the existing entity that are in the `entityImportView` will be updated.
+
[source, java]
----
include::{sourcesdir}/common/entityImportExportService_4.java[]
----

* `importEntitiesFromJSON()` - deserializes the JSON and persists deserialized entities according to the rules, described by the `entityImportView` parameter. If the entity is not present in the database, it will be saved. Otherwise the fields of the existing entity that are in the `entityImportView` will be updated.

* `importEntitiesFromZIP()` - reads a ZIP archive that contains a JSON file, deserializes the JSON and persists deserialized entities according to the rules, described by the `entityImportView` parameter. If the entity is not present in the database, it will be saved. Otherwise the fields of the existing entity that are in the `entityImportView` will be updated.
+
[source, java]
----
...
`DataManager` always applies <<security_subsystem,security>> restrictions on entity operations and attributes when invoked from a client, and ignores them by default when invoked from middleware code. If you want to check these restrictions when using `DataManager` in your middleware code, obtain a wrapper via `DataManager.secure()` method and call its methods. Alternatively, you can set the <<cuba.dataManagerChecksSecurityOnMiddleware,cuba.dataManagerChecksSecurityOnMiddleware>> application property to turn on security check for the whole application. Note that access group <<constraints,constraints>> (row-level security) are always applied regardless of whether `DataManager` is called from the client or middle tier.
...
You can also place the message in a <<message_packs,localized messages pack>> and use the following format to specify the message in an annotation: `{msg://message_pack/message_key}`. For example:

...
The default implementation of `AuthenticationManager` is `AuthenticationManagerBean`, which delegates to a chain of `AuthenticationProvider` instances. An `AuthenticationProvider` is an authentication module that can process a specific `Credentials` implementation, also it has a special method to allow the caller to query if it supports a given `Credentials` type.

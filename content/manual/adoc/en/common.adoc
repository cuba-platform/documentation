...
+
`load()` and `loadList()` methods check user <<permissions,permission>> `EntityOp.READ` for the entity being loaded. Additionally, loading entities from DB is subject for access group <<constraints,constraints>>.
+

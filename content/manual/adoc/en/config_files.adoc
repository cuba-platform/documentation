...
** `id` – unique identifier of an element, used for caption localization (see <<menu.xml_localized,below>>). Can be used as screen `id` for linking to one of the UI screen elements registered in <<screens.xml,screens.xml>> file, if no actions declared. When user clicks on a menu item, the corresponding screen will be opened in the main application window.

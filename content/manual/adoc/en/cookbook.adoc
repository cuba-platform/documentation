...
Parameters can be passed from `<<abstractFrame_openWindow,openWindow()>>`, `openLookup()` or `openEditor()` method of one screen to the `<<abstractFrame_init,init()>>` method of another screen. Suppose we want to filter the products in the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/Product.java[Product] browser passing some parameters from the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/Order.java[Order] editor.

* https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/order/OrderEdit.java[OrderEdit] screen contains the `addOrderLine()` method which is invoked by `addOrderLine` action. The method <<abstractFrame_openWindow,opens>> a products lookup screen passing two parameters to it:
+
** the currently selected customer,
+
** the list of already added products.
+
After a user selects a product, the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/order/QuantityDialog.java[QuantityDialog] screen is opened for entering product quantity. When the user closes it, a new instance of https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/OrderLine.java[OrderLine] entity is created and added to the table.
+
[source, java]
----
...
Suppose we want to customize the caption of the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/Customer.java[Customer] browser if it is opened from a `PickerField` in the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/Product.java[Product] editor.

 * In the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/product/ProductEdit.java[ProductEdit] screen we set the parameters for the `PickerField` <<lookupAction,LookupAction>> using its `setLookupScreenParams()` method:
+
[source, java]
----
...
* Then, we inject the passed parameter in the <<abstractFrame_init,init()>> method of https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/customer/CustomerBrowse.java[CustomerBrowse] screen:
+
[source, java]
----
...
https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/order/OrderEdit.java[OrderEdit] screen controller demonstrates two ways of looking up a https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/global/src/sample/entity/Customer.java[Customer]: from a lookup screen and from an arbitrary screen, both returning a `Customer` instance.
...
It is often convenient to edit the <<composition_oto_recipe,One-to-One composition>> in one single editor. Let's see how it can be implemented taking the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/customer/Customer.java[Customer] and the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/customer/CustomerDetails.java[CustomerDetails] relationship as an example.

* https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/customer/customer-edit.xml[customer-edit.xml] descriptor contains the main `customerDs` and the nested `detailsDs` datasources:
+
--
[source, xml]
----
...
Fields for editing both entities are grouped into two fieldGroups, each bound to a separate datasource:
...
* In the https://github.com/oshiryaeva/screen-manipulation/blob/master/modules/web/src/sample/web/customer/CustomerEdit.java[CustomerEdit] controller we override the `initNewItem()` method. It will create a `CustomerDetails` instance and link it to the new `Customer` instance when the latter is just created:
+
--
[source, java]
----
...
Then, to make the nested `detailsDs` datasource track and save changes in the `CustomerDetails` instance alongside with the edited `Customer` instance, we override the `postInit()` method:
...
[source, java]
----
include::{sourcesdir}/cookbook/oto_single_editor_5.java[]
----
--

...
. Namespace is used as a prefix for DB tables, so for a public component it should be composite, like `jptams`, not just `search`. It will minimize the risk of name collisions in the target application. You cannot use underscores and dashes in namespace, only letters and digits.
...
[[app_components_deploy]]
==== Installing App Components

[[app_components_install_local]]
Deployment into local Maven repository::
+
--
...
[[app_components_install_remote]]
Deployment into remote Maven repository::
+
--
. Set up a repository as explained in <<private_repo>>.

. Open `build.gradle` of the component project in a text editor. Replace the repository and its credentials in `buildscript/repositories` section and add the `uploadRepository` to the `cuba` section:
+
[source,groovy]
----
...
. Run the  `gradle assemble` task in command line of the application component project root directory. This ensures your new repository caches CUBA artifacts required for working in Studio.
...
. Open the application component project in Studio.

. In the Studio *Search* dialog (Alt-/), find the `uploadArchives` Gradle task and run it. You can also run this task from the command line. The component's artifacts will be uploaded to your repository.

. Remove the component artifacts from your local Maven repository to ensure that they will be downloaded from the remote repository during the next assembling: just delete the `.m2/repository/com/company` folder located in your user home directory.

. Open the application component in Studio. The repository URL in its `build.gradle` will be automatically changed to the one specified in the Studio server window.

. Now, when you assemble and run the application that uses this component, it will be downloaded from the remote repository.
--

[[app_components_install_local]]
Deployment into Bintray repository::
+
--
. Register at https://bintray.com/signup/oss2
...
[[app_components_add]]
==== Adding App Components to Your Application

Below ar the instructions on how to add an application component to your project from different sources.

From Studio with binaries::
+
--
. On the *App components* panel of *Project Properties* click the plus button next to *Custom components*.

. In the *Custom application component* dialog, select the component in the *Registered project* drop-down list. The list contains all project registered in Studio that have an `app-component.xml` descriptor.
+
If the component was not registered in Studio, you can enter its coordinates directly in the *Coordinates field*, for example:
+
[source, plain]
----
...
. Click *OK* in the dialog. The Maven coordinates of the component will appear in the list of custom components. Save the project properties by clicking *OK*.

[TIP]
====
If the application component is installed into a <<app_components_install_local,local>> Maven repository, don't forget to check the *Use local Maven repository* checkbox on the *Advanced* tab of *Project Properties*.
====
--

From build.gradle with binaries::
+
--
Add the component as a dependency in your project's `build.gradle`:

[source, groovy]
----
dependencies {
    appComponent("com.haulmont.cuba:cuba-global:$cubaVersion")
    appComponent("com.company.customers:cust-global:2.0.+")
}
----

[TIP]
====
If the component is installed in Bintray, set Bintray as the main repository for your project. It can be done in Studio's *Project properties* or directly in `build.gradle`:
...
+
--
. Import the GitHub project in Studio by clicking *Import* > *Git*.

. Generate the app component descriptor.

. Invoke *Run > Install app component* command to install the component into the local Maven repository.

. Open your project in Studio and check the *Use local Maven repository* checkbox on the *Advanced* tab of *Project Properties*.

. Add the component's dependency to the project using *App components > Custom components (+)*, as described above.
--

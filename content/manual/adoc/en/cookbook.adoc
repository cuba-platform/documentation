...
----
+
`Airline` will be also marked by default as the owning side of the relationship, as the "master-detail" relation is not mandatory for <<manyToMany_annotation,@ManyToMany>> annotation.
...
===== Many-to-Many Association with Additional Entity

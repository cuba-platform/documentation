...
We will use jQuery for the upload and get a JSON with http://files.cuba-platform.com/swagger/6.7/#!/Files/post_files[data] which is the newly created `FileDescriptor` instance. We can access the uploaded file by its `FileDescriptor` id with the access token name:

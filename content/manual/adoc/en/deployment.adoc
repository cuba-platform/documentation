...
* lib – directory of the libraries that are loaded by the server&rsquo;s _common classloader_. These libraries are available for both the server and all web applications deployed in it. In particular, this directory should have JDBC drivers of the utilized databases (hsqldb-XYZ.jar, postgresql-XYZ.jar, etc.)

* `logs` – application and server <<logging,logs>> directory. The main log file of the application is app.log.

* shared/lib – directory of libraries that are available to all deployed applications. These libraries classes are loaded by the server&rsquo;s special _shared classloader_. Its usage is configured in the conf/catalina.properties file as described above.
+
...
In this case and when more complex deployment options are used, the Web Client&rsquo;s <<cuba.useLocalServiceInvocation,cuba.useLocalServiceInvocation>> application property should be set to `false`, and <<cuba.connectionUrlList,cuba.connectionUrlList>> property should contain the URL of the Middleware block.
...
+
where `++domain_name\user_name++` is the user&rsquo;s domain and name
...
+
Check the app.log file located in <<log_dir,the server&rsquo;s log folder>> for a message about SQL execution from the `com.haulmont.cuba.core.sys.DbUpdaterEngine` logger and, possibly, further error messages to identify the error reasons.
...
[WARNING]
====
When updating the database from the command line, the existing Groovy scripts are started, but only their main part gets executed. Due to the lack of the server context, the script&rsquo;s `PostUpdate` part is ignored with the corresponding message written to the console.
====

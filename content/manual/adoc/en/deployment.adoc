...
A Docker image with the CUBA app should be based on OpenJDK. We recommend to use a `Dockerfile` to specify the information that Docker needs to know to run the app — a base Docker image to run from, the location of your project code, any dependencies it has, and what commands to run at start-up.
...
To have more information about Dockerfile instructions see the https://docs.docker.com/engine/reference/builder/[Dockerfile reference].
...
[TIP]
====
If you have more than one Middleware server you have to list all of them in the <<cuba.connectionUrlList,cuba.connectionUrlList>> property and configure a cluster of Web Client servers, as it described in the <<scaling>> section.
====
...
Create two sub folders in the `docker-image` folder for the web and core jars. You should create a separate containers for each jar, so you need to configure two `Dockerfiles`.
...
Image pull or push operations against the public Docker Hub registry or a private registry may require authentication. You can provide your credentials in the registryCredentials closure. Set your credentials in the https://docs.gradle.org/current/userguide/build_environment.html[gradle.properties] file:
[source, groovy]
----
...
Run the `pushImage` task from the terminal or from the *Search* field in Studio.

----
gradle pushImage
----

Task successively builds Uber JAR, generates the `Dockerfile`, builds the image and pushes this image to your Docker Hub registry.

===== Container deployment to Heroku

Set up the single Uber JAR as it desctibes in the <<single_jar_deployment>> section.

Create a Heroku account and install Heroku CLI as it described in the <<heroku_war_deployment>> section.
...
Go to the Idea and open the `jetty-env.xml` file. You have to change the url (host and database name), username and password.
...
Then log in to the container registry. It's Heroku location for storing images.

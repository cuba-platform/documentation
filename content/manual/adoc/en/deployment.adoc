...
Now you can find the new database on the *Resources* tab. The database is connected to the Heroku application. Click on Postgres database link:
...
Git::
--
To initiate Heroku application deployment you need the ability to perform push to your remote git repository. For that reason a git utility installed on your computer is mandatory. As an option, you can install built-in git during Heroku CLI installation. Also you need to
--
Heroku account::
+
--
...
Now you can find a new database on the *Resources* tab. The database is connected to the Heroku application. Click on Postgres database link:
...
. Create a file `system.properties` with following content (for local JDK 1.8.0_121):
+
[source]
----
...
----
+
. Check that files `system.properties, gradlew, gradlew.bat and gradle` are not in `.gitignore`
...
This action adds `gradlew and gradlew.bat` file into the project. Because Heroku uses this wrapper, you must add `gradlew, gradlew.bat gradle/wrapper` to GitHub. Ensure that these items are not in `.gitignore`.
--

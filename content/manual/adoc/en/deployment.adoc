...
. Create a <<logging, Logback>> configuration file. You can take the `logback.xml` file from the `build\tomcat\conf` project directory after the standard <<fast_deployment,fast deployment>> to Tomcat. Replace the `${catalina.home}` variable to `${app.home}` in the file content and place the file into the application home directory.
...
. Log files are created in the `c:\work\app_home\logs` directory.

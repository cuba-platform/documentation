...
TIP: If Tomcat was previously installed, and some of the variables sensitive to Tomcat server startup were included into the environment variables, then it may cause problems starting the server without any reflection in logs. In this case, remove all `CATALINA_` and `CLASSPATH` variables and reboot your PC.
...
+
WARNING: Please note that current implementation of Polymer UI requires manual setting of `<base>` tag to the actual web context available at run time. So changing the context by using `-frontContextName` command line argument can make the Polymer UI inaccessible.
...
TIP: In the examples below, we provide paths to configuration files as if <<fast_deployment>> is used.

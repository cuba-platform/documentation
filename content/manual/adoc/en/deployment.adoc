...
. Open command prompt at any folder of your computer and type:
+
[source]
----
heroku login
...
+
image::heroku_data.png[align="center"]
+
...
+
image::heroku_data_add_database.png[align="center"]
+
. On next screen click *Install Heroku Postgr...*
+
image::heroku_data_install_postgres_database.png[align="center"]
+
. Next connect the database to Heroku application selected from a dropdown list
+
image::heroku_data_connect_database_to_app.png[align="center"]
+
. Next select your Plan (for ex. `hobby-dev`)
+
image::heroku_data_select_plan.png[align="center"]
+
...
Now you can find the new database on the *Resources* tab. The database is connected to the Heroku application. Click the Postgres database link:

image::heroku_resources_database.png[align="center"]

To obtain database credentials go to the *Datasource* page of your Heroku account and scroll down to *Administration* section:

image::heroku_db_view_options_1.png[align="center"]
image::heroku_db_view_options_2.png[align="center"]

Click *View credentials* button:

image::heroku_db_view_options_3.png[align="center"]
...
Go to *Project Properties* section and set *Database type* to PostgreSQL then save options.

image::heroku_project_database_setup.png[align="center"]

Go to *Data Model* section, click *Create DB scripts* then click OK.

image::heroku_db_scripts.png[align="center"]
...
+
image::heroku_war_deployment_settings_1.png[align="center"]
+
* Click *Generate* button next to the *Custom context.xml path* field. Fill your database connection details in modal window. Then open the generated `modules/core/web/META-INF/war-context.xml` file and check connection params and credentials
+
image::heroku_fill_database_creds.png[align="center"]
+
[source, xml]
----
<Context>
...
image::heroku_war_deployment_settings_2.png[align="center"]
...
The section describes how to deploy single bundle CUBA with Tomcat on https://www.heroku.com/[Heroku®] cloud platform. This guide is intended for users who have a CUBA project located on GitHub.
...
Git::
--
To initiate Heroku application deployment you need the ability to perform push to your remote git repository. For that reason a git utility installed on your computer is mandatory. As an option, you can install built-in git during Heroku CLI installation. Also you need to have GitHub repository containing your CUBA project.
--
Heroku account::
+
--
Second create an account on Heroku using the web browser, free account `hobby-dev` is enough. Then login to the account and create new application using *New* button at the top of the page.
...
Now you can find a new database on the *Resources* tab. The database is connected to the Heroku application. Click the Postgres database link:

image::heroku_view_postgres_creds_3.png[align="center"]
--

Database administration::

--
To obtain database credentials go to the *Datasource* page of your Heroku account and scroll down to *Administration* section, then click *View credentials* button:

image::heroku_view_postgres_creds_4.png[align="center"]
...
. Create a file `system.properties` in `$PROJECT_FOLDER` with following content (example corresponds to local JDK 1.8.0_121 installed):
+
[source]
----
...
Building process is available in the dashboard on the *Activity* tab:

...
This directory can be used by the application for creating arbitrary temporary files at run time. The path to the temporary directory is determined by the <<cuba.tempDir,cuba.tempDir>> application property. In case of <<fast_deployment,fast deployment>> in Tomcat, it is a subdirectory with the name of the web application in the `tomcat/temp` directory. For other deployment scenarios, the temporary directory is located inside the <<app_home,application home>>.
...
----
The JDBC driver will be available in the list of detected drivers if you have copied `postgresql-x.y.z.jar` as described above.
...
The section describes how to deploy CUBA applications to the https://www.docker.com/[Docker®] platform.
...
The simplest way to run your CUBA application in a production environment is to use the Uber JAR approach. You can build an all-in-one JAR file using the
Gradle task and then you will be able to run the application from the command line using the java executable, there is
no need to use special web servers or servlet containers. Simply start your application as an executable. All parameters
of the application are defined at the build time but can be overridden when running.


We will show you how to set up monolithic and distributed application configurations.


[[single_jar_deployment]]
===== Single Uber JAR deployment

Open your CUBA project in Studio, navigate to *Deployment settings*, go to *Uber JAR* tab and then configure options as described below.
...
A Docker image with the CUBA app should be based on OpenJDK. To specify the information that Docker needs to know to run the app — a base Docker image to run from,
the location of your project code, any dependencies it has, and what commands to run at start-up we recommend to use a `Dockerfile`.
...
To define and run multi-container docker applicftion use the Docker Compose tool. With Compose, you use a YAML file to configure your application’s services, so they can be run together in an isolated environment.
...
After the task is complete you should be able to open the application at http://localhost:8080/app
...
* <<cuba.useLocalServiceInvocation,cuba.useLocalServiceInvocation>> property  should be set to `false` in our case, because we deploy core and web servers in the different
containers.
* <<cuba.trustedClientPermittedIpList,cuba.trustedClientPermittedIpList>> property defines the list of IP addresses, from which the login to the application
is allowed.

[TIP]
====
If you have more than one Middleware server you have to list all of them in the <<cuba.connectionUrlList,cuba.connectionUrlList>> property and configure a cluster of Web Client servers, as it describes in the <<scaling>> section.
====

Run the <<build.gradle_buildUberJar>> task to regenerate the jars.

Create two sub folders in the `docker-image` folder for the web and core jars. You should create a separate
containers for each jar, so you need to configure two `Dockerfiles`.
...
In this section we cover the building and pushing the Docker images with the single Uber JAR using Gradle.
Set up the single Uber JAR as it desctibes in the <<single_jar_deployment>> section.

There are many Gradle plugins for Docker.
We are going to use the https://github.com/bmuschko/gradle-docker-plugin[bmuschko/gradle-docker-plugin].
...
You can provide those credentials in the registryCredentials closure. Set your credentials in the gradle.properties file. You can access a project property in your build script simply by using its name as you would use a variable:
[source, groovy]
----
...
Run the `pushImage` task from the terminal or from the *Search* field in Studio. Task
successively built Uber JAR, generates the `Dockerfile`, built the image and push this image to your Docker Hub registry.

===== Container deployment to Heroku

Set up the single Uber JAR as it desctibes in the <<single_jar_deployment>> section.

Create a Heroku account and install Heroku CLI as it describes in the <<heroku_war_deployment>> section.
...
After the task is complete you have to configure the database credentials in the `jetty-env.xlm` file for the connection
to the database created by Heroku.

. Go to the https://dashboard.heroku.com.
. Select you project, open the *Resources* tab and select the database.
. In the newly opened window open the *Settings* tab and click the *View Credentials* button.

image::heroku-db.png[Db,1200,1000]

Go to the Idea and open the `jetty-env.xlm` file. You have to change the url (host and database name), username and password.
Copy credentials from the site and past in the file.
[source, html]
----
<?xml version="1.0"?>
...
Also, you have to add some changes to the `Dockerfile`. First of all, if you use the free account, you have to restrict the amount of memory consumed by
the application. Then you need to obtain the port to the app from the Heroku and add it to the image.

The `Dockerfile` should looks like the following:
----
### Dockerfile
...
Then log in to Container Registry. It's Heroku location for storing images.

----
heroku container:login
----

Then build the image and push it to Container Registry

----
heroku container:push web
----

Here `web` is the process type of the application. When you run this command, by default Heroku is going to build the
image using the `Dockerfile` in the current directory, and then push it to Heroku.
...
The third way to open the application is running the following command, but you have to add the `app` context to the link:
...
Every application block deployed as a web application provides a health check URL. An HTTP GET request of this URL returns `ok` if the block is ready to work.

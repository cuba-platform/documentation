...
. Create a <<logging, Logback>> configuration file. You can take the `logback.xml` file from the `build\tomcat\lib` project directory after the standard <<fast_deployment,fast deployment>> to Tomcat. Replace the `${catalina.home}` variable to `${app.home}` in the file content and place the file into the application home directory.

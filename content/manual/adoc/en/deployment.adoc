...
[[uberjar_https]]
===== HTTPS Configuration for UberJAR

If you want to deploy HTTPS for your application, follow the steps below:
...
. In the project root folder create the `jetty.xml` file with SSL configuration:
+
[source, xml]
----
...
. Add `jetty.xml` to Uber JAR task configuration:
+
[source, groovy]
----
...
+
The application will become available by https://localhost:8443/app/.

...
The document describes how to build WAR to run CUBA on https://www.heroku.com/[Heroku®].
...
. *Heroku account*
. *CUBA Studio* installed on your computer
. *Heroku CLI* https://devcenter.heroku.com/articles/heroku-command-line
. *Heroku CLI deploy* plugin
. Install *Git* on your computer or use built-in in Heroku CLI
. Tomcat *Webapp Runner* +
  https://mvnrepository.com/artifact/com.github.jsimone/webapp-runner +
  The version of Webapp Runner is pinned to the version of the underlying Tomcat server. +
  Thus, version 8.5.11.3 of Webapp Runner uses version 8.5.11 of Tomcat. +
  Put it into root of CUBA project *./webapp-runner.jar*
. *Tomcat DBCP* +
  https://mvnrepository.com/artifact/org.apache.tomcat/tomcat-dbcp +
  Version 8.5.11 tested, use the same as Webapp Runner +
  Put it into *libs/tomcat-dbcp.jar*
. *PostgreSQL driver* actual for your database +
  https://mvnrepository.com/artifact/org.postgresql/postgresql/9.4.1212 +
  Version 9.4.1212 tested with Heroku PostgreSQL 9.6.1 +
  Put it into *libs/postgresql.jar*

*Heroku account*

First create an account on https://www.heroku.com/[Heroku®] using the web browser, free account `hobby-dev` is enough. Then login to Heroku account and create new application using 'New' button at the top of the page.

image::heroku_new_app.png[align="center"]

Select unique name (or left the field blank to assign automatically) and choose server location. Now you have an application, for example `morning-beach-4895`, this is a Heroku application name.

First time you will be redirected to `Deploy` tab. Use Heroku Git deployment method. This tutorial does not cover Github integration.
...
. Create an empty folder for CUBA project
. Navigate to this folder `$PROJECT_FOLDER` and type command `heroku login`
...
Return to web browser with Heroku https://dashboard.heroku.com[dashboard], go to Resources tab and click `Find more add-ons` button to find the database add-on.
...
----
where `morning-beach-4895` is your Heroku application name.
...
. Create new `CUBA project` located in `$PROJECT_FOLDER` in CUBA Studio.
. Open deployment settings and configure options as described below.

image::heroku_war_settings.png[align="center"]

* Tick `Build WAR` +
* Point application home directory as '.' (dot) +
* Tick `Include Tomcat's context.xml` +
* Point `Custom context.xml path` as `modules/core/web/META-INF/war-context.xml`. Click `Generate`
* Open local file `modules/core/web/META-INF/war-context.xml` and fill connection details using your database connection params and credentials like shown below:
...
* Tick `Single WAR for Middleware and Web Client`
* Fulfil `Custom context.xml path` as `modules/web/web/WEB-INF/single-war-web.xml`. Click `Generate` to create a file.

*App properties*

Copy text from example below and paste it to `App properties` field:
...
Build WAR one of two ways.

* Run command from Studio. Click `Search command` button and type `bu`, then choose `buildWar - gradle task` from dropdown list

image::heroku_war_build.png[align="center"]

OR

* run shell command from $PROJECT_FOLDER
...
Open local command line utility. Navigate to `$PROJECT_FOLDER`. Run commands listed below:
...
Open local command line utility. Navigate to your CUBA project folder and run deploy command:
...
Open Heroku dashboard, Resources tab. A new Dyno should appear with a command from Procfile:

image::heroku_dyno.png[align="center"]

The application is deploying now. Monitor logs to track the process.

*Check logs*

Wait for a message `https://morning-beach-4895.herokuapp.com/  deployed to Heroku` +
Now you can open the application in a web browser.

To track heroku application logs type a shell command:
...
After the deployment process is completed, your application will become accessible in browser using the https://morning-beach-4895.herokuapp.com/.

You can open the application from Heroku dashboard using 'Open app' button.

image::heroku_open_app_full.png[align="center"]
...
Append following gradle task to your `$PROJECT_FOLDER/build.gradle`
...
cuba.automaticDatabaseUpdate = true
        ...
        webAppDir.renameTo( new File('deploy/tomcat/webapps/ROOT') )
...
Tomcat logs are available in web application:

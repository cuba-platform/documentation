...
The section describes how to deploy CUBA on https://www.heroku.com/[Heroku®] cloud platform.
...
. Go to *Data Model* section, click *Create DB scripts* then click *Save and close*.
...
* Fill your database connection details in modal window.
...
+
--
This guide is intended for users who have a CUBA project located on GitHub.
To initiate Heroku application deployment you need the ability to perform push to your remote git repository. For that reason you need Git installed on your computer. As an option, you can install built-in git during Heroku CLI installation. Also you need to have GitHub repository containing your CUBA project.
--
...
Building process is available in the dashboard on the *Activity* tab. Click *View build log* link to track build log.

After building process is completed, your application will become accessible in browser using the `++https://space-sheep-02453.herokuapp.com/++`. You can open the application from Heroku dashboard using the button *Open app*.
--

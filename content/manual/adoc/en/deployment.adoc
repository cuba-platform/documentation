...
CUBA Studio provides WAR deployment to Heroku® cloud.

[TIP]
====
Heroku cloud deployment is applicable for projects using PostgreSQL database.
====


===== Download prerequisities

. Heroku CLI https://devcenter.heroku.com/articles/heroku-cli
. Tomcat DBCP from your local CUBA project folder +
  %USER_PROFILE%\studio-projects\%PROJECT_FOLDER%\deploy\tomcat\lib\tomcat-dbcp.jar +
Put it into *libs/tomcat-dbcp.jar*
. Tomcat Webapp Runner, must be the same version as tomcat-dbcp.jar +
  https://mvnrepository.com/artifact/com.github.jsimone/webapp-runner +
  The version of Webapp Runner is pinned to the version of the underlying Tomcat server. Thus, version 8.5.11.3 of Webapp Runner uses version 8.5.11 of Tomcat +
  Put it into root of CUBA project *./webapp-runner.jar*
. PostgreSQL driver +
  https://mvnrepository.com/artifact/org.postgresql/postgresql/9.4.1212 +
  Put it into *libs/postgresql.jar*


===== Create Heroku account

Login to your Heroku account and create new application using '`New`' button
...
Now you have an application, for example *peaceful-castle-71397*, this is a Heroku application name. +
At first you will be redirected to 'Deploy' tab. Please consider not to choose Github deployment now to avoid Cuba application crash during commits.


===== Heroku CLI

You may have to https://devcenter.heroku.com/articles/heroku-command-line[download] and install Heroku CLI +
Go to your cuba project folder ($PROJECT_FOLDER) and type 'heroku login'. Enter your credentials when prompted.
...
Go to Resources tab and click 'Find more add-ons' button to create database.
Find 'Heroku Postgres' block and click it.
Follow instruction on the screen, click 'Login to install' then +
'Install Heroku Postgres' OR type command in Heroku CLI:

[source, bash]
----
heroku addons:create heroku-postgresql:hobby-dev --app peaceful-castle-71397
----

Find new database mention appears on Resources tab.
...
Database d2tk6hacoaqans
User nmmdenmqbcpozo
Port 5432
Password 9c05b77840c0609ff910f454314c69bb560ec5b8b0f4f8ce35d57749af35fd6e
URI postgres://nmmdenmqbcpozo:9c05b77840c0609ff910f454314c69bb560ec5b8b0f4f8ce35d57749af35fd6e@ec2-176-34-111-152.eu-west-1.compute.amazonaws.com:5432/d2tk6hacoaqans
----
...
[source, ruby]
----
heroku pg:psql postgresql-asymmetrical-95175 --app peaceful-castle-71397
----

Next time run heroku commands from current project folder without password prompt.
...
Create and configure CUBA project using Studio
Change default database to PostgreSQL. Leave database URL and credentials as is.

image::heroku_war_params.png[align="center"]


*Configure params on WAR tab*

* Tick 'Build WAR' and point application home directory as '.' (dot)
* Enable 'Include JDBC driver'
* Tick 'Include Tomcat's context.xml'. Fulfil 'Custom context.xml path' as modules/core/web/META-INF/war-context.xml . Click 'Generate'
...
<Context>
    ...
    <Resource
      ...
      url="jdbc:postgresql://ec2-176-34-111-152.eu-west-1.compute.amazonaws.com/d2tk6hacoaqans"
      username="nmmdenmqbcpozo"
      password="9c05b77840c0609ff910f454314c69bb560ec5b8b0f4f8ce35d57749af35fd6e"/>
...
* Tick 'Single WAR for Middleware and Web Client'. Fulfil 'Custom context.xml path' as modules/web/web/WEB-INF/single-war-web.xml . Click 'Generate' to create a file.


*App properties*

* Fulfil application properties as an example
...
Deploy WAR one of two ways.

Run command from Studio. Click 'Search command' button and type buildw, then choose 'buildWar - gradle task' from dropdown list

image::heroku_war_build.png[align="center"]

OR +
run gradle command from project directory
...
Dyno / process +
Add a ./Procfile to your CUBA application folder in order to define its process types:
  Procfile +
[source, groovy]
----
...
After upload/run project Procfile will be read by Heroku and new option at Resources tab appears.
...
----
git init
heroku git:remote -a peaceful-castle-71397
git add .
...
[WARNING]
====
Skip PUSH command because currently it fails!
====


===== Deploy application

Instead of *push* run deploy command from your CUBA project folder:
...
In dashboard on Resources tab a Dyno should appear:
...
First, track command line window, then heroku logs
[source, bash]
----
heroku logs --tail
----
Wait a message *'https://peaceful-castle-71397.herokuapp.com/  deployed to Heroku'*


===== Access to web application

Go to https://peaceful-castle-71397.herokuapp.com/ , login and navigate to menu.

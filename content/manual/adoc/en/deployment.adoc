...
. Install *Git* to your computer or use built-in in Heroku CLI
...
First create an account on https://www.heroku.com/[Heroku®] using the web browser, free account 'hobby-dev' is enough. +
Then login to Heroku account and create new application using button 'New' at the top of the page.
...
Create an empty folder for CUBA project. Navigate to this folder ($PROJECT_FOLDER) and type command 'heroku login'. +
Enter your credentials when prompted. From this moment you won't needed to enter credentials for this project anymore. +
Then install Heroku CLI deployment plugin
...
----
where peaceful-castle-71397 is your Heroku application name.
...
image::heroku-war-settings.png[align="center"]
...
Wait for a message *'https://peaceful-castle-71397.herokuapp.com/  deployed to Heroku'*
Now you can open the application in a web browser.

To track heroku logs type a shell command:
...
After the deployment process is completed, your application will become accessible in browser using the https://peaceful-castle-71397.herokuapp.com/.

You can open the application from Heroku dashboard using the button 'Open app'

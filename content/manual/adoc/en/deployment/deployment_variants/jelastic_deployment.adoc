...
Below is an example of building and deployment application to the https://jelastic.com/[Jelastic] cloud.
...
* If the created environment uses PostgreSQL, you will receive an email with the database connection details. Go to the database administration web interface by the link in the email received after the creation of the environment and create an empty database. The database name should be specified later in the custom `context.xml`.
...
* If your project uses PostgreSQL, press *Generate* button next to the *Custom context.xml path* field. Specify the database user, password, host, and name that you specified earlier when you created database by the web interface from the email.
+
...
* Add `${cuba.logDir}` value in the *App properties* field:
+
[source, properties]
----

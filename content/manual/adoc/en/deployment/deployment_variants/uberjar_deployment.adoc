...
----
+
In order to connect to the web client at `++http://localhost:8080++`, run the command:
+
[source,plain]
----
...
[WARNING]
====
UberJAR doesn't work correctly with the `frontContextName`, `contextName` arguments for React frontend clients.
====
...
You can do it using `local.app.properties` file, OS environment variables or Java system properties provided in the command line.

The `local.app.properties` file must be located at application home. It is the current working directory if you haven't specified the `-Dapp.home=some_path` command line argument. For example:
...
Java system properties should be specified in the command line (keep in mind that they will be shown by task managers and `ps` command):
...
OS environment variables override values from `app.properties` files, Java system variables override both OS environment variables and property files.
--

...
* Compare the Hibernate Validator versions in WildFly and CUBA application. If the platform uses a newer version, replace the `C:\wildfly\modules\system\layers\base\org\hibernate\validator\main\hibernate-validator-x.y.z-sometext.jar` with the newer file from `tomcat\shared\lib`, for example, `hibernate-validator-6.1.1.Final.jar`.

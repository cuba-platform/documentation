...
CUBA Platform is not restricted by the current set of visual components. If any component is absent in the platform distribution then a developer has 3 ways:

. *Integrate a Vaadin add-on*. Many third-party Vaadin components are distributed as add-ons. Add-ons library is located by address https://vaadin.com/directory. A developer can integrate any of them to the project.

. *Integrate a JavaScript component*. Vaadin allows to create server components integrated with a JavaScript library.

. *Create an own Vaadin component* with a client part written on GWT.

Each of the methods listed above gives a Vaadin component as an output. A developer then can initialize it and add to the screen. This option is appropriate if the component is used in one or two screens, but if it is used many times and we want to be able to declare the component in screen XML-descriptor, then we have to integrate the Vaadin component to the platform Generic UI.

This section gives you examples of new visual components creation with all the methods described above. Integration to the Generic UI is the same for all methods, so it is described only for example of <<vaadin_addon_sample_gui,using a Vaadin add-on>>.
...
Below there is an example of using the *Stepper* component, available at http://vaadin.com/addon/stepper. This component enables changing text field value in steps using the keyboard, mouse scroll or built-in up/down buttons.

To demonstrate an integration of new component let's create a new project in CUBA Studio. Name it `addon-demo`.

Vaadin add-on may be integrated if the project has a *web-toolkit* module. Create the module by clicking  the *Create web-toolkit module* button of the *Project properties* section of Studio navigation panel.

Then click the *Create new UI component* button - a window for UI component creation window will be opened. Select the *Vaadin add-on* value in the *Component type* section.

image::ui_component/studio_vaadin_addon_wizard_no_gui.png[align="center"]

Let's look at fields that should be filled.

* *Add-on maven dependency* - this field contains maven coordinates of the Vaadin add-on to be included it as a dependency to the current project. You can define coordinates in two ways:

. XML copied from the add-on web site (http://vaadin.com/addon/stepper):
+
[source, xml]
----
<dependency>
...
. In one line in the way you add dependencies in <<development.adoc#build.gradle,build.gradle>>:
+
[source, plain]
----
...
* *Inherited widgetset* - this fields contains a wedgetset name of the add-on. In out case it is `org.vaadin.risto.stepper.widgetset.StepperWidgetset`.

* *Integrate into generic UI* - in this example this checkbox must be switched off because we don't integrate the component into the Generic UI.

Press the *OK* button and the add-on will be attached to the project. Let's see what Studio has done for us. Open the project in the IDE. You can do this by clicking the *IDE* button in *Project properties* section of Studio navigator.

Studio has changed 2 files:

. <<development.adoc#build.gradle, build.gradle>>. Module *web* now contains a dependency to the add-on that contains the component.
+
[source, java]
----
configure(webModule) {
...
. `AppWidgetSet.gwt.xml` file of *web-toolkit* module now contains an inheritance of add-on widgetset:
+
[source, xml]
----
<module>
...
----
+
TIP: You can increase the widgetset compilation speed by defining a `user.agent` property. In this example widgetset will be compiled only for browsers based on WebKit: Chrome, Safari, etc.

Now the component from the Vaadin add-on is attached. Let's see how to use it in project screens.

* Create a new entity `Customer` with two fields:

** `name` of String type
** `score` of Integer type

* Generate standard screens for the new entity. Ensure that the *In module* field value is `Web Module`. Screens that use Vaadin components directly must be placed in the *web* module.
+
TIP: Actually, the screen can be placed in the *gui* module as well, but then a code that uses a Vaadin component should be carried out to the separate <<gui.adoc#companions,companion>>.

* Next, we will add the `stepper` component to the screen. You can place it both inside the `FieldGroup` and outside it. Let's examine both methods.
+
--
. Make the `score` field of the <<gui.adoc#gui_FieldGroup, fieldGroup>> component at the `customer-edit.xml` screen custom. Add an attribute `custom = "true"`.
+
[source, xml]
----
...
+
Component data binding is implemented by setting the current value to the `stepper` component from the edited `Customer` in the `postInit()` method. Additionally, the corresponding entity attribute is updated through the implemented value change listener, when the user changes the value.
...
----
+
Inject the container to the screen controller, retrieve a link to the Vaadin container and add the component to it:
+
[source, java]
----
...
* To adapt the component style, create a theme extension in the project. Run the *Create theme extension* command in the *Project properties* section of the Studio navigation panel. Select the `halo` theme to be extended. After that, open the `themes/halo/halo-ext.scss` file in the web module and add the following code:
+
[source, css]
----
...
* Create the database, run the project and enjoy the result.
...
To demonstrate an integration of new component let's create a new project in CUBA Studio. Name it `addon-demo-gui`. Type `adg` in the *Project namespace* field.

Vaadin add-on may be integrated if the project has a *web-toolkit* module. Create the module by clicking  the *Create web-toolkit module* button of the *Project properties* Studio navigation panel.
...
* *Add-on maven dependency* - this field contains maven coordinates of the Vaadin add-on to be included as a dependency to the current project. You can define coordinates in two ways:
...
* *Inherited widgetset* - this fields contains a widgetset name of the add-on. In out case it is `org.vaadin.risto.stepper.widgetset.StepperWidgetset`.

* *Integrate into generic UI* - defines that a component should be integrated into the platform Generic UI.

* *Component name in screen XML* - component name in screen XML-descriptor. Enter the value `stepper`.

* *Component interface name* - an interface name for the platform Generic UI. Enter the value `Stepper`.
...
* Add Vaadin add-on as *web* module dependency in *build.gradle*

* Include add-on widgetset in `AppWidgetSet.gwt.xml` of *web-toolkit* module

* If the *Integrate into generic UI* checkbox is checked, Studio generates stubs for the following files:

** `Stepper` - an interface of the component in *gui* module

** `WebStepper` - a component implementation in *web* module

** `StepperLoader` - a component XML-loader in *gui* module

** `ui-component.xsd` - a new component XML schema description. If the file already exists when the component is generated, then the information about new component will be added to the existing file.

** `cuba-ui-component.xml` - the file that registers a new component loader in *web* module. If the file already exists when the component is generated, then the information about new component will be added to the existing file.

Open the project in IDE. You can do this by clicking the *IDE* button of *Project properties* section of Studio navigation panel.

Let's walk through generated files add make necessary changes.

* Open the `Stepper` interface of *gui* module. Replace its content with the following code:
+
[source, java]
----
...
----
+
The base interface for the component is `Field`, which enables data binding, i.e. the ability to view and edit an entity attribute.
...
----
+
The chosen base class is `WebAbstractField`. The base class implements the <<gui.adoc#datasources,datasource binding>> `Field` logic and other methods of the `Field` interface.

* `StepperLoader` in *gui* module - an implementation of XML loader in the *gui* module.
+
[source, java]
----
...
----
+
The `AbstractFieldLoader` class contains a code for `Field` component basic properties loading. It is sufficient to load the specific properties of the `IntStepper` component only.

* `cuba-ui-component.xml` file of *web* module. The new component and its loader are registered here. Remain the file unchanged.
+
[source, xml]
----
...
* `cuba-ui-component.xsd` file of *gui* module - an XML schema description of new visual components. Add `stepper` attributes definition.
+
[source, xml]
----
...
Let's see how to add the new component to the screen.
...
* Add the `stepper` component to the editor screen. You can place it both inside the `FieldGroup` and outside it. We'll examine both methods.
...
* Define a new namespace `xmlns:app="http://schemas.company.com/agd/0.1/ui-component.xsd"`

* Remove the `stepper` field from the `FieldGroup`
...
. Using the component inside a <<gui.adoc#gui_FieldGroup>>:
+
[source, xml]
----
...
In this example we will attach a https://jqueryui.com/slider/#range[slider] component of the *jquery-ui* library. The slider will have two drag handlers that define a values range.

To integrate the JavaScript component to the application we have to do the following:

. Create a server Vaadin component in *web* module. A server component defines an API for server code, different getters and setters, event listeners, etc. The server component must extend `AbstractJavaScriptComponent` class. Note that *web-toolkit* module and widgetset compilation is not required when integrating a JavaScript component.

. Create a JavaScript connector. A connector defines a function that initializes a JavaScript component and that is responsible for JavaScript code and server interaction.

. Create a state class. It defines what data are sent between the server and the client. Public fields of this class will be automatically serialized on the server side and deserialized on the client. The class must extend `JavaScriptComponentState`.

To demonstrate how to integrate a JavaScript component let's create a new project in CUBA Studio. The project name is `jscomponent`.

Click the *Create new UI component* button of the *Project properties* section in Studio navigation panel - a new UI component creation window will be opened.

Select the `JavaScript component` value in the `Component type` section.

image::ui_component/studio_js_component_wizard.png[align="center"]

Enter the value `SliderServerComponent` in the *Vaadin component class name* field.

Remove the *Integrate into Generic UI* flag. The process of integration the component into the Generic UI is similar to that is described at <<vaadin_addon_sample_gui>>, so we won't describe it here.

After clicking the *OK* button Studio will generate the files:

* `SliderServerComponent` - a Vaadin component integrated with JavaScript

* `SliderState` - a state class of Vaadin component

* `slider-connector.js` - a JavaScript connector for Vaadin component

Let's examine the generated files and make necessary source code modifications. Open the project in IDE.
...
* Vaadin server component `SliderServerComponent`.
+
[source, java]
----
...
----
+
The server component defines getters and setters to work with a slider state and an interface for values change listener. The class extends `AbstractJavaScriptComponent`.
+
`addFunction` method in the class constructor defines a handler for an RPC-call for the function `valueChanged` from the client.
+
@JavaScript and @StyleSheet annotations point to files that must be loaded at the web page. In our example these are JavaScript files of the `jquery-ui` library, connector and a stylesheet for `jquery-ui`. You need to put these files to the package with Vaadin server component.

Download an archive with `jquery-ui` from http://jqueryui.com/download and put files `jquery-ui.js` and `jquery-ui.css` from the archive to the package with `SliderServerComponent` class. At the jquery-ui download page you can select which components should be put into the archive. For this demo it is enough to select just a `Slider` item of `Widgets` group.
...
----
+
Connector is a function that initializes a JavaScript component on a web page loading. The function name must correspond to the server component class name where dots in package name are replaces with underscore characters.
+
...
+
--
* Initializes a `slider` component of the `jquery-ui` library. The `slide` function is invoked when a position of any drag handler changes. This function in its turn invokes a connector method `valueChanged`. `valuedChanged` is the method that we defined on the server side in the `SliderServerComponent` class.

* Defines an `onStateChange` function. It is called when the state object is changed on the server side.
--

To demonstrate how the component works let's create a `Product` entity with three attributes:

* `name` of String type
* `minDiscount` of type Double
* `maxDiscount` of type Double

Generate standard screens for the entity. Ensure that the *In module* field value is `Web Module`. Screens that use Vaadin components directly must be placed in the *web* module.

TIP: Actually, the screen can be placed in the *gui* module as well, but then a code that uses a Vaadin component should be carried out to the separate <<gui.adoc#companions,companion>>.

In the example we will set product minimal and maximum discount values with the `slider` component we just created.

Open the `product-edit.xml` file. Make `minDiscount` and `maxDiscount` fields not editable by adding the `editable="false"` attribute to the corresponding tags. Then add a new custom field `slider` to the `fieldGroup`.

As a result an XML descriptor of the editor screen should look like this:
...
Go to the `ProductEit.java` file. Replace its content with the following code:
...
In the `initNewItem()` method initial values for discounts of a new product are set.

Method `init()` initializes the `slider` custom field. It sets current, minimal and maximum values of the `slider` and defines a value change listener. When a drag handler moves, a new value will be set to the corresponding field of the editable entity.

Run the project and open the product edit screen. Changing the drop handler position must change the value of fieldGroup field.
...
In this section we will cover a creation of a simple GWT component (a rating field that looks like 5 stars) and a usage of this component inside application screens.

image::ui_component/rating_field_component.png[align="center"]

Create a new project in CUBA Studio. The project name is `ratingsample`.

To be able to create a GWT component a project must contain a *web-toolkit* module. Create it by clicking the *Create web-toolkit module* button in the *Project properties* section of Studio navigation panel.

Click the *Create new UI component* button, and a new UI component creation window will be opened. Select the *New GWT component* value in the *Component type* section.

image::ui_component/studio_gwt_component_wizard.png[align="center"]

Type `RatingFieldServerComponent` in the *Vaadin component class name* field.
...
Let's look at the generated files and make necessary changes in them. Open the project in IDE.

* GWT widget `RatingFieldWidget.java`. Replace its content with the following code:
+
[source, java]
----
...
----
+
A widget is an isolated class responsible for the component displaying and for the reaction on events. It doesn't need to know about the server side, it just defines interfaces for working with the server side. In our case these are the `setValue` method and the `StarClickListener` interface.

* Vaadin component class `RatingFieldServerComponent`. It defines an API for the server code, different getters and setters, event listeners and data sources connection. Developers use this class methods in their code.
+
[source, java]
----
...
* State class `RatingFieldState` defines what data are sent between the client and the server. It contains public fields that are automatically serialized on server side and deserialized on the client.
+
[source, java]
----
...
* Interface *RatingFieldServerRpc* defines a server API that is used by the client. Its methods may be invoked by the RPC mechanism build into Vaadin. We will implement this interface in the component.
+
[source, java]
----
...
* Connector `RatingFieldConnector` links client code with the server.
+
[source, java]
----
...
`RatingFieldWidget` source code doesn't contain any mention about the component appearance except assigning style names to key elements. To define an appearance of the component we'll create stylesheet files. Use the *Create theme extension* action of the *Project properties* section for this. In the dialog select the `halo` theme. Studio creates SCSS files for theme extension in the `themes` directory of the *web* module. Instead of icons `halo` theme uses FonAwesome font glyphs. We'll use this fact.

It is usual to put component styles into the separate file `componentname.scss` in the `components/componentname` directory in form of SCSS mixture. Let's create a directories structure `components/ratingfield` in the `themes/halo` directory of *web* module. Then inside the `ratingfield` directory create a `ratingfield.scss` file:
...
Include this file in the main theme file `halo-ext.scss`:
...
Add the screen to the application menu. Go to the *Main menu* section of Studio navigation panel and click the *Edit* button. A menu editor will be opened. Add the screen created earlier to the `application` menu.
...
Open the screen controller class `RatingScreen.java` and add the code that puts the component to the screen.
...
===== Custom Visual Components Support in CUBA Studio

This section describes how to add а new custom visual components support to the screen layout designer of CUBA Studio. This support allows custom component to be added to the components palette. A developer then will be able to drag and drop the component to the layout and edit its properties in the component properties panel.

Let's walk through the process of extending the Studio by the support of the `stepper` visual component. This component creation was described in <<vaadin_addon_sample_gui>>.

Open the project containing the `stepper` component.

TIP: There is no harm, if you didn't create this project. You can still reproduce the steps listed below in the new project. In this case you will see how Studio supports the component, but you won't be able to run the application.

Click the *Extend Studio* button of the *Project properties* section.

image::ui_component/ui_component_extension_window.png[align="center"]

In the *Extend Studio* screen select the *UI Component configuration* configuration type.

We have to fill the following fields:

* *Configuration name* - a configuration identifier. Enter the `stepper` value.

* *Component name in screen XML* - a component name as it will be used in a screen XML descriptor. It is `stepper` in our case.
+
*Component class name* and *Component model class name* fields are filled automatically based on the value of the *Component name in screen XML*. Leave their values without changes.

* *Component namespace URI* - a namespace from the XSD that describes the component. If you've generated the new component with the CUBA Studio, then you can take the value of this field from from the file `ui-component.xsd`.

* *Component namespace prefix* - a prefix of component XML tag.

* *Standard properties* - standard properties that should be available for editing in the component properties panel of Studio screen designer.
+
Select `caption`, `datasource` and `property` checkboxes.
+
TIP: `id`, `align`, `height`, `width`, `enable`, `stylename` and `visible` properties are attached to any component by default.

* *Custom properties* - the table is used for declaring component specific properties that should be edited in the component properties panel of Studio screen designer.
+
--
Add the following properties:

* *manualInput*, `Boolean` type, the default value is `true`

* *mouseWheel*, `Boolean` type, the default value is `true`

* *stepAmount*, `Integer` type, the default value is `0`

* *maxValue*, `Integer` type, the default value is `0`

* *minValue*, `Integer` type, the default value is `0`
--

Press the *OK* button.

New visual components are initialized on Studio server start. Go to the Studio server window, stop the server and start it again.

Re-create standard screens for the `Customer` entity to erase the results of our previous experiments.

Then go to the *Screens* section of Studio navigation panel and open the `customer-edit` screen.

Remove the `stepper` field from the `fieldGroup` because we will use a separate component for editing the score. Go to the *XML* tab and remove the line `<field id="score">`.

Go back to the *Layout* tab of the screen designer.

Find the new `Stepper` component on the components palette, then drag it to the screen below the `fieldGroup`.
...
There is a new namespace with an `app` prefix in the screen XML, a `stepper` component is added to the screen, and its properties have the values we set.

...
* `appName`  - name of the web application which the WAR file will be searched for. By default it corresponds the *web* module name without `-web`, e.g., `app`.
...
* `environment`  - the environment in which the application WAR will be deployed. You can set either the environment name or its ID.

* `hostUrl`  - URL of the API host. Typically it is `app.jelastic.<host name>`.

* `srcDir`  - the directory in which the WAR is located. By default it is `"${project.buildDir}/distributions/war"`.

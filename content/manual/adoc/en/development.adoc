...
.  The <<views,view>> of the airport editing screen should contain the `terminals` collection attribute:
+
[source, xml]
----
...
* Then we get the `FileDescriptor` instance with the picture stored in the <<file_storage,File Storage>>. The link to this picture is stored in the `imageFile` attribute of the `Employee` entity. The `FileDescriptorResource` resource type is used to set the source for the `Image` component.

...
* `singleJar` - parameter that defines if a single Uber JAR should be created from all modules (core, web, portal, polymer). By default: `false`.

* `coreJettyConfPath` - the relative path to a file to be used for Jetty Server configuration for the Core Uber JAR (if `singleJar` = `false`). See https://www.eclipse.org/jetty/documentation/9.4.x/jetty-xml-config.html.

* `webJettyConfPath` - the relative path to a file to be used for Jetty Server configuration for the Web Uber JAR (if `singleJar` = `false`). If `singleJar` = `true`, `webJettyConfPath` is used to configure the Jetty Server for a single Uber JAR.

* `portalJettyConfPath` - the relative path to a file to be used for Jetty Server configuration for the Portal Uber JAR (if `singleJar` = `false`).
...
* `logbackConfigurationFile` - parameter that defines a relative path to a file to be used for logging configuration.

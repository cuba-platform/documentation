...
* A set of <<artifact_repository, repositories>> for loading project dependencies. When you create a new project in CUBA Studio, it contains the link to the repository which is selected in the Studio server window. By default it is the public repository:
+
[source]
----
...
- link:http://www.sonatype.org/downloads/nexus-latest-bundle.zip[Download] Sonatype Nexus OSS version 2.x (2.14.3 tested)
- Unpack zip file to the directory `c:\nexus-2.14.3-02`
- Navigate to the folder `c:\nexus-2.14.3-02\bin`
- Modify settings are located in file `c:\nexus-2.14.3-02\conf\nexus.properties`
...
On the opened *Repositories* page click the *Add* button, then choose *Proxy Repository*. A new repository will be added. Fill required fields at *Configuration* tab:
...
- Enable Authentication, set Username: *cuba*
- Set Password: *cuba123*
- Click *Save*

Create a Repository Group, in Nexus click *Add*, then choose *Repository Group* and do the following on *Configuration* tab:

- Fill the Group ID: *cuba-group*
- Fill the Group Name: *cuba-group*
- Choose the Provider: Maven2
- Add the repository *cuba-work* from Available Repositories to Ordered Group Repositories
- Click *Save*

Now proxy repository is ready to use. Use the URL of this group `http://localhost:8081/nexus/content/groups/cuba-group/` with credentials *admin/admin123* as a repository link in Studio or your build file.

If you have a subscription to the *Premium Add-ons*, then create the same repository *cuba-premium* with the following settings:
...
- Auto Blocking Enabled: *false*
- Enable Authentication, set Username: *The first part of your license key*
- Set Password: *The second part of your license key*
- Click *Save*
- Click *Refresh*
- Click on the repository *cuba-premium*
- On *Configuration* tab add the repository *cuba-premium* to the group *cuba-group* below the repository *cuba-work*
- Click *Save*

Find the group URL at the top of the screen:
----
http://localhost:8081/nexus/content/groups/cuba-group
----
- Launch CUBA Studio Server.
- Copy and paste it to CUBA Studio launcher window.
- Click *Auth*
- Fill login and password fields; defaults are *admin/admin123*.
- Click *Start*.
- Open http://localhost:8111/studio/ in your browser.
- Open your project.
- Build the project.

During the first build your Nexus server downloads necessary artifacts and keeps it in the cache for the next usage. You may find them in `c:\nexus-2.14.3-02\sonatype-work` folder.

[WARNING]
====
For CUBA version 6.4 and below, before the first run of the Studio in the isolated environment, do the following steps:

- Set the group URL in your build.gradle
+
----
http://localhost:8081/nexus/content/groups/cuba-group
----
+
- Assemble the project manually
+
--
[source]
----
gradle assemble setupTomcat deploy createDb idea
----
--
====
...
If you need to develop on CUBA in an isolated network, do the following steps on the isolated machine:

- Install another copy of the repository manager in the target network and copy the contents of the downloaded cache into it (the path may differ according to your custom settings):
+
--
----
c:\nexus-2.14.3-02\sonatype-work
----
to the same place on the second server. Then restart the service *nexus*. The repository URL will be the same as on the first server.
--
+
- Download https://services.gradle.org/distributions/[Gradle] (version 3.1 is required) and copy the archive to the isolated machine.
- Install `gradle` program to the specific folder, for example, `c:\gradle-3.1`, the path will be called `$GRADLE_HOME`.

Now you may open CUBA Studio in the isolated network:

- Open CUBA Studio launcher window.
- Press *Advanced* button and set $GRADLE_HOME.
- Fill your actual Repository URL in the Studio launcher window, for example, `http://localhost:8081/nexus/content/groups/cuba-group`.
- Fill login and password fields with actual values; defaults are *admin/admin123*.
- Start Studio.
- Open your project.
- Build the project.

If you need to add artifacts of a new platform version to the isolated repository, do the following:

- Go to the environment connected to the internet
- Make a build through its repository
- Copy the contents to the isolated environment again.
...
TIP: If you didn't create this project, you can still reproduce the steps listed below in a new project. In this case, you will see how Studio supports the component, but you won't be able to run the application.
...
In the *Extend Studio* screen, select the *UI component configuration* option.
...
+
TIP: `id`, `align`, `height`, `width`, `enable`, `stylename` and `visible` properties are available to any component by default.
...
The custom visual components support is initialized when the Studio server start. Go to the Studio server window, stop the server and start it again.
...
[TIP]
====
Using the *Install app component* command is necessary on Windows to avoid file locking issues. On Linux or Mac OS, you can also use the `install` Gradle task directly.
====
...
. Set up a repository and make it a proxy for the public CUBA repository `++https://repo.cuba-platform.com/content/groups/work++`.

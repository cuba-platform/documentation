...
`deploy` – the task of the `CubaDeployment` which performs <<fast_deployment,fast deployment>> of a module to Tomcat. It is declared in the *core*, *web* and *portal* modules. Parameters:
...
`buildWar` – the task of the `CubaWarBuilding` type which builds a <<war_deployment, WAR file>> from the application code and its dependencies. It should be declared in the root of `build.gradle`. The resulting WAR file(s) are located in the `build/distributions` project subdirectory.

To create a single WAR file comprises both Middleware and Web Client blocks, use the following task configuration:
[source, groovy]
----
...
* `coreProject` - the Gradle project representing the *core* module (Middleware).

* `webProject` - the Gradle project representing the *web* module (Web Client).

* `appName` – the name of the resulting WAR file. In case of separate WAR-files, the `-core` suffix will be added for the Middleware WAR.
...
+
In the `appHome` parameter, you can specify an absolute or relative path to the home directory, or a system variable, which should be set at server start. For example: `++appHome = '/work/sales_home'++` or `++appHome = '${app.home}'++`.

* `webXml` - the path to the special `web.xml` file for the single WAR, because it is different from the ones created for <<fast_deployment, fast deployment>> by default.

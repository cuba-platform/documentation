...
Then you need to select the `hsqldb-x.x.x.jar`, driver, which you should download from link:http://hsqldb.org/[HSQLDB] website in advance. If you are using *HyperSQL* only for CUBA development, then you can add the driver supplied as part of CUBA Studio (it is located in the lib subfolder).
...
In the appeared window select the `hsqldb-x.x.x.jar` driver that you downloaded from link:http://hsqldb.org/[HSQLDB] website in advance. If you are using *Hyper SQL* only for CUBA development, you can just add the driver supplied as part of CUBA Studio (it is located in the lib subfolder).
...
* `*[application]*` – the name of the Tomcat-deployed web application whose code has logged the message. This information enables identifying messages from different application blocks (Middleware, Web Client), since they are written into the same file.

...
[WARNING]
====
If your project uses Premium Add-ons and you are starting build tasks outside Studio, you pass the Premium Add-ons repository credentials in the command line arguments with `-P` prefix, for example:
...
Or, you can simply create a `~/.gradle/gradle.properties` file with the following content in order to use gradle tasks from command line without passing any additional parameters:
...
, where `premiumRepoUser` is the first part of your license key before dash, and `premiumRepoPass` is the part after dash, see the <<access_to_repo,Access to CUBA Premium Add-ons>> section.

See <<build.gradle, above>> for the details.
====

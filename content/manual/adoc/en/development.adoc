...
----
+
Instead of passing the credentials each time you build the project from the command line, you can once set the environment variables:
+
[source, properties]
----
...
The <<web.xml,web.xml>> file defines two servlet context listeners that load the application blocks: `SingleAppCoreServletListener` and `SingleAppWebServletListener`. All initialization parameters are passed to them through context parameters.

In order to use custom `web.xml` files for separate WARs, put them in the same directory as the default `web.xml` and specify the following parameters:

* `coreWebXmlPath` - the relative path to a file to be used as a `web.xml` of the *core* module.

* `webWebXmlPath` - the relative path to a file to be used as a `web.xml` of the *web* module.

Example of using a custom `web.xml`:
...
* `webXmlPath` - the relative path to a file to be used as a `web.xml` of the single WAR.
+

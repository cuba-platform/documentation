...
Unique index may contain a column with the NULL value, e.g. TITLE and DELETE_TS where DELETE_TS is NULL by default. In this case MySQL will allow to store multiple records with the same TITLE value and NULLs in DELETE_TS. If you need a unique constraint for a table of entity that implements <<soft_deletion,SoftDeletе>> interface, then you have to do the following:

. Create a DELETE_TS_NN column in a database table. This column is not null and is initialized by a default value:
+
[source, sql]
----
...
[source, sql]
----
create trigger LIB_BOOK_DELETE_TS_NN_TRIGGER before update on LIB_BOOK
for each row
...
. Create a unique constraint for columns that are included to the unique key and for DELETE_TS_NN:
+
[source, sql]
----
create unique index IDX_LIB_BOOK_UNIQ_TITLE on LIB_BOOK (TITLE, DELETE_TS_NN)^
----

...
==== Setting Up Isolated Artifact Repository

This topic describes how to set up a personal Nexus maven repository. There is a case when the user have to develop an application in the network isolated from Internet. Therefore a personal repository is required. CUBA Studio supports isolated maven repositories.

First you need to setup the local maven repository on the machine connected to Internet.

===== Setting up proxy repository
...
[source]
----
nexus-work=${bundleBasedir}/nexus/sonatype-work/nexus
----
+
- Install the wrapper to able start and stop nexus as a service, run a command:
+
[source]
----
nexus.bat install
...
+
--
and wait for the command message:
--
+
--
[source]
----
mwrapper  | nexus installed.
----
--

===== Launch Online Repository
...
- You may see *nexus* service appears in the list
- Right click *nexus* line
+
--
a) choose *Start* from the menu, you will see a progress bar indicating the server is starting

b) wait for the step completes

c) then open the address http://localhost:8081/nexus/ in the web browser

d) browse to the right top angle of the screen and perform log in, use default login *admin* and password *admin123*
--

===== Configure the Proxy Repository

Browse the instance tree at the left side of the screen. Navigate to *Repositories* page.

Click the button *Add*, then choose *Proxy Repository*. A new repository will be added. Fill required fields:

- Repository ID: *proxy-releases*
- Repository Name: *proxy-releases*
- Remote Storage Location: https://repo.cuba-platform.com/content/groups/work
- Click *Save*

Now proxy repository ready to use.

- Find the new repository URL at the top of the screen
----
http://localhost:8081/nexus/content/repositories/proxy-releases
----
...
During the first build your Nexus server downloads necessary artifacts and keeps it in the cache for the next usage.
You may find them in `c:\nexus-2.14.3-02\sonatype-work\nexus` folder.

==== Isolated Repository

You may install the same repository in the isolated network without Internet connection using the settings above. To make isolated repo operable you need to copy following folder from the first server
----
c:\nexus-2.14.3-02\sonatype-work\nexus
----
to the same place on the second server. Then restart the service *nexus* via Windows services as described above. The repository URL will be the same as on the first server.

Now you may open CUBA Studio in isolated network:

- fill your actual repository URL in the CUBA launcher window
- start Studio
- open your project
- build the project

To add new artifacts to the isolated repository you need to repeat all steps from this topic from the beginning.

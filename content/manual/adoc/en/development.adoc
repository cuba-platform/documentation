...
[[private_repository_install_repository]]
...
[[private_repository_configure_proxy]]
...
[[private_repository_cuba_studio_for_proxy]]
===== Configuring CUBA Studio to Use Proxy Repository

Now proxy repository is ready to use. Find the group URL at the top of the screen:
----
http://localhost:8081/nexus/content/groups/cuba-group
----
- Open CUBA Studio Server window, then copy and paste `group URL` into the *Repository* field.
- Click *Auth* button next the *Repository* field and fill login and password fields; defaults are `admin/admin123`.
- Start Studio.
- Open ++http://localhost:8111/studio/++ in your browser.
- Open your project.
- Build the project.

During the first build your Nexus server downloads necessary artifacts and keeps them in the cache for the next usage. You may find them in `c:\nexus-2.14.3-02\sonatype-work` folder.

[WARNING]
====
For CUBA version 6.4 and below, before the first run of the Studio using the proxy repository, do the following steps:
...
[source]
----
gradle setupTomcat deploy createDb idea
----
--
====

[[private_repository_in_an_isolated_network]]
...
- Install another copy of the repository manager in the target network.
- Download https://services.gradle.org/distributions/[Gradle] (version 3.1 is required) and install it onto developers machines.
...
[[private_repository_studio_in_an_isolated_network]]
===== CUBA Studio in an Isolated Network

Now you may open CUBA Studio on developers machines:

- Open CUBA Studio Server window.
- Press *Advanced* button and set path to the folder where you have installed Gradle.
- Fill your actual Repository URL in the *Repository* field, for example, `++http://localhost:8081/nexus/content/groups/cuba-group++`.
- Click *Auth* button next the *Repository* field and fill login and password fields with actual values; defaults are `admin/admin123`.
- Start Studio.
- Open your project.
- Build the project.

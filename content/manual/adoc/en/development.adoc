...
==== Setting Up Own Artifact Repository

You may install your own artifact repository. Based on Sonatype Nexus product it can operate in offline mode.
...
You may configure server port, default is *8081*. This setting is located in file `c:\nexus-2.14.3-02\conf\nexus.properties`

install wrapper to able start and stop nexus as a service:

nexus.bat install
mwrapper  | nexus installed.

===== Launch
Open Windows Services: press *Win+R*, type *services.msc*, then press *Enter*.

You may see *nexus* service appears in the list.

Right click *nexus* and choose *Start* from the menu. You will see a progress bar indicating the server is starting. Wait for the step completes then open the address http://localhost:8081/nexus/ in the web browser.

Browse to the right top angle of the screen and perform log in. Use default login *admin* and password *admin123*.
...
Find the new repository URL at the top of the screen, copy and paste it to CUBA Studio starting window and then build the project.

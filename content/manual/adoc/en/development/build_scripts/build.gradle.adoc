...
You can add dependencies via the `server` configuration for *core*, *web*, and *portal* modules (modules that have a task with type `CubaDeployment`). These dependencies will be placed in the server libs by `deploy`, `buildWar`, and `buildUberJar` tasks.

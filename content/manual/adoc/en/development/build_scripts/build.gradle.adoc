...
+
--
This section contains a set of <<app_components,application components>> used by the project. There are two types of dependencies: `appComponent` - for CUBA application components dependencies and `uberJar` - for the libraries that should be loaded before the application starts.
CUBA Components are specified by their *global* module artifact. In the following example, three components are used: `com.haulmont.cuba` (*cuba* component of the platform), `com.haulmont.reports` (*reports* premium add-on) and `com.company.base` (a custom component):

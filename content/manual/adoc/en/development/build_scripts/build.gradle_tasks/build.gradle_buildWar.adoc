...
If the application project contains the *front* module, it becomes accessible at `/<appName>/front` path. In order for the frontend interface to work correctly in the single WAR, you should change the environment variable `PUBLIC_URL= /app/front/` during build (for example, in the `.env.production.local` (see https://github.com/cuba-platform/frontend#configuration[README])).
--

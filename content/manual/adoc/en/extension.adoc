...
The same logic can be used for overriding services from <<app_components_sample,application components>>: to substitute a bean implementation, you should create a class that implements the Spring bean you want to override. Below is an example based on the *workshop* sample application where the new bean overrides the https://github.com/cuba-platform/sample-workshop/blob/master/modules/core/src/com/company/workshop/service/OrderServiceBean.java[OrderService] method:
...
Then, if you register the in `spring.xml`, the new bean will be used instead of the one from `OrderService`:

...
Please note, that every entity in the response has a `_entityName` attribute with the entity name and an `_instanceName` attribute with the entity <<namePattern, instance name>>.
...
The `customer` reference doesn't have a `@Composition` annotation, that's why the REST API wil try to find a client with the given id and set it to the `customer` field. If the client is not found then an order won't be created and the method will return an error.

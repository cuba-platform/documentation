...
REST API is included as a dependency to the *web* and *portal* modules of the *cuba* application component. By default, it is turned on in the *portal* module and off in the *web* module. So if you need REST API, you have two options:
...
This chapter contains REST API usage examples.

As mentioned earlier, a detailed information about REST API methods is written according to http://swagger.io/specification[Swagger] specification and is available by address http://files.cuba-platform.com/swagger.
...
The request type must be `application/x-www-form-urlencoded`, an encoding is `UTF-8`.
...
Please note, that every entity in the response has a `_entityName` attribute with the entity name and an `_instanceName` attribute with the entity <<namePattern, short name>>.
...
The `customer` reference doesn't have a `@Composition` annotation, that's whe the REST API wil try to find a client with the given id and set it to the `customer` field. If the client is not found then an order won't be created and the method will return an error.
...
Suppose, there is an `OrderService` <<service, service>> in the system. The implementation looks as follows:
...
A service method may return a result of  simple datatype, an entity, an entities collection or a POJO. In our case a BigDecimal is returned, so the response body contains just a number:

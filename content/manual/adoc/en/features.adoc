...
When the user is working through the web interface, the file extension defines whether the file is downloaded via the browser's standard open/save dialog (`Content-Disposition = attachment`), or if the browser will attempt to show the file in the browser window (`Content-Disposition = inline`). The list of extensions for files that should be shown in the browser window is defined by the <<cuba.web.viewFileExtensions,cuba.web.viewFileExtensions>> application property.

...
Version 2 of the REST API provides the following functionality:
...
It uses the OAuth2 protocol for authentication and supports anonymous access.

The detailed documentation for the version 2 of the REST API is written according to Swagger specification (http://swagger.io/specification/). It can be found by the following URL: TODO ADD URL.

The running CUBA application also exports the swagger documentation by the URLs `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.yaml` and  `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.json`.
...
The content of the `myapp-rest-queries.xml` must be as following:
...
The list of services methods that are available with the REST API must be configured in the CUBA application in files defined by the <<cuba.rest.servicesConfig, cuba.rest.servicesConfig>> application property, e.g.
...
The content of the `myapp-rest-services.xml` must be as following:
...
By default, all CORS requests to the REST API are allowed. To restrict the origins list you can define an <<cuba.rest.allowedOrigins, cuba.rest.allowedOrigins>> application property.

[[rest_api_v2_anonymous]]
===== Anonymous access

By default, anonymous access is disabled. To enable it, use the <<cuba.rest.anonymousEnabled, cuba.rest.anonymousEnabled>> application property. A request is considered to be anonymous if it doesn't contain an `Authentication` header. In this case, an anonymous user session will be put to the <<securityContext, SecurityContext>>.
...
<<cuba.rest.client.tokenExpirationTimeoutSec, cuba.rest.client.tokenExpirationTimeoutSec>> - defines a token expiration timeout for the default client in seconds.

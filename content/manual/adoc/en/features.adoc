...
CUBA Platform provides the predefined `all` query for getting all entities with the REST API. In order to receive the number of entity instances, simply use `/all/count/` in the *GET* request URL:

  http://localhost:8080/app/rest/v2/queries/sales$Order/all/count

You can also use custom queries in the CUBA application. Predefined JPQL queries must be specified in files registered in the <<cuba.rest.queriesConfig, cuba.rest.queriesConfig>> application property of the *web* or *portal* module (e.g in the `web-app.properties file`):
...
For all value types, except `Boolean`, there is also a *Width* field available to set up the field width in `FieldGroup` in pixels or as a percentage. If the *Width* field is empty, its assumed value is 100%. For the `Enumeration` value type, the set of named values is defined in the *Enumeration* field via the list editor.

...
* <<fileLoader_recipe,Fileloader>> - the interface for files loading and storage that can be used on both middle and client tiers. Its main methods are:
+
--
** `saveStream()` – saves an `InputStream` contents into file storage on middleware.

** `openStream()` – returns an input stream to load a file contents from middleware.

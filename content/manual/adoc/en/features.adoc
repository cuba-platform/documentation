...
In the CUBA application, predefined JPQL queries must be specified in files defined by the <<cuba.rest.queriesConfig, cuba.rest.queriesConfig>> application property, e.g.
...
The content of the `myapp-rest-queries.xml` should be as follows:
...
The list of service methods that are available via the REST API must be configured in the CUBA application in files defined by the <<cuba.rest.servicesConfig, cuba.rest.servicesConfig>> application property, e.g.
...
The content of the `myapp-rest-services.xml` should be as follows:
...
Before the execution with the REST API a query must be described in the <<rest_api_v2_queries_config, configuration file>>. The `sales-rest-queries.xml` file must be created in the main package of the *web* module (e.g. `com.company.sales`). Then the file must be defined in the application properties file of the *web* module (web-app.properties).
...
Before the execution with the REST API a service method invocation must be allowed in the <<rest_api_v2_services_config, configuration file>>. The `sales-rest-services.xml` file must be created in the main package of the *web* module (e.g. `com.company.sales`). Then the file must be defined in the application properties file of the *web* module (web-app.properties).
...
Before the invocation with the REST API the method must be allowed, so we add a record to the `sales-rest-services.xml` configuration file (it was described in the <<rest_api_v2_ex_service_get>>).
...
CUBA platform provides support of HTML pages and Javascript with the help of REST API. Page files can be placed in any project folder, for simplicity, we recommend to create `VAADIN` folder in the *web* module (`modules/web/web/VAADIN`), as static resources in this folder are loaded automatically. Download and place in the same `VAADIN` project folder a https://code.jquery.com/jquery-3.1.1.min.js[jQuery library] to import it in your screen file. For better styling you may use http://getbootstrap.com/getting-started/#download[Bootstrap] framework. Put its `.css`-file (in the case below it is `bootstrap.min.css`) in the same project folder.

Below is an example of the simple screen with the login window and Customers browser displaying the list of customers.
...
This screen is created in the `VAADIN` folder of the *web* module as `customers.html` file and is available in the running application by its relative path added to URL:

image::rest_js_1.png[align="center"]

Customers window is hidden before authorization. After the user has signed in, the login window is replaced with *Logged in successfully* message, and the list of customers is shown.

In this example we use a standard <<rest_api_v2_ex_get_token,OAuth Token authentication>>, the token is stored in a simple variable `oauthToken`.

Login and password from the user input are sent to the server by the POST request. If the authentication is successful, the web page receives from server the token value, the `loginForm` `<div>` is hidden and the `loggedInStatus` `<div>` is shown.
...
To show the list of customers, the request is sent to the server <<rest_api_v2_ex_get_entities_list,to get the instances>> of `sales$Customer` entity, passing the `oauthToken` value as parameter.

In case the request is processed successfully, the `customers` `<div>` is shown, and the `customersList` `<ul>` is filled with lines containing Customer local fields values, such as names and emails.

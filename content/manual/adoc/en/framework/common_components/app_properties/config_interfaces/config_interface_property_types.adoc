...
* `UUID` – `UuidTypeFactory`, as described above. `TypeStringify` is redundant here, you can easily use `toString()` method for `UUID`.

* `java.util.Date` – `DateFactory` and `DateStringify`. Date value must be specified in `yyyy-MM-dd HH:mm:ss.SSS` format, for example:
+
--
[source, properties]
----
...
[source, java]
----
include::{sourcesdir}/common/configinterface_9.java[]
----
--
...
[source, java]
----
include::{sourcesdir}/common/configinterface_10.java[]
----
--
...
[source, java]
----
include::{sourcesdir}/common/configinterface_11.java[]
----
--

...
@PostConstruct::
This annotation can be specified for a method. Such method will be invoked right after the entity instance is created by the <<metadata,Metadata.create()>> method. This is convenient when instance initialization requires invocation of <<managed_beans,Spring beans>>. For example, see <<init_values_in_class>>.

...
* Create a <<managed_beans,Spring bean>> in the *core* module of your project and implement the `SetupAttributeAccessHandler` interface. Parameterize the interface with the type of handled entity. The bean must have the default singleton scope. You have to implement the interface methods:

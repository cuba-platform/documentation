...
+
--
When working with relational databases, use <<jpql,JPQL>> queries to load data. See the <<jpql_functions>>, <<query_case_insensitive>> and <<jpql_macro>> sections for information on how JPQL in CUBA differs from the JPA standard. Also note that `DataManager` can execute only "select" queries and use only named parameters; positional parameters are not supported.
--

...
[TIP]
====
See https://www.cuba-platform.com/guides/decouple-business-logic-with-app-events[Decouple Business Logic with Application Events] guide to learn different examples of application events.
====

++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.2/com/haulmont/cuba/core/global/Events.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
[WARNING]
====
Methods with `@EventListener` annotation do not work in <<services,services>>, <<jmx_beans,JMX beans>> and other beans with interfaces. If you define `@EventListener` on such bean you will get the following error on application start:
----
...
[[ui_events_UIRefreshEvent]]
An example of such an event is the `UIRefreshEvent`, which is fired on each web page refresh. `UIRefreshEvent` can be handled only in screen controllers of the active `AppUI`. For example:

[source, java]
----
include::{sourcesdir}/common/ui_events_UIRefreshEvent.java[]
----

...
`MessageTools` interface is a <<managed_beans,Spring bean>> containing additional methods for working with <<localization,localized messages>>. You can access `MessageTools` interface either using `Messages.getTools()` method, or as any other bean – by means of injection or through `AppBeans` class.

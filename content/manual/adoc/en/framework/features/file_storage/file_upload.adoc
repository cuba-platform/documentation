...
Temporary files are deleted automatically in the following ways:

* `FileUploadField` is used in `IMMEDIATE` <<gui_FileUploadField_fileStoragePutMode,mode>>.
* `FileUploadingAPI.putFileIntoStorage(UUID, FileDescriptor)` method is used.

In other ways, after uploading files on the server, into the file storage or `byte[]` it is recommended to delete them using `FileUploadingAPI.deleteFile(UUID)` method.

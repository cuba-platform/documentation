...
[WARNING]
====
Note that actions declared for the whole screen are not refreshed their state. It means if an action has specific `enabledRule` it will not be applied until `refreshState()` is invoked manually.
====

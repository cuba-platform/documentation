...
* List actions work with collections of entities that are displayed in tables or trees. These are <<CreateAction>>, <<EditAction>>.
+
...
* Picker field actions work with the content of the field. These are <<LookupAction>>, <<OpenAction>>.
+
...
Each standard action is implemented by a class annotated with `@ActionType("<some_type>")`. The class defines the action's default parameters and behavior.

You can specify any basic action XML attributes to override default parameters: caption, icon, shortcut, etc. For example:

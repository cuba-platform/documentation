...
The action is implemented by `com.haulmont.cuba.gui.actions.list.ClearAction` class and should be defined in XML using `type="picker_clear"` action's attribute. You can configure common action parameters using XML attributes of the `action` element, see <<declarative_actions>> for details.

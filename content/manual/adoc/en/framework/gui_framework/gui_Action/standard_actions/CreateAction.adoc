...
Create action is designed to create new entity instances by opening the entity edit screen. It is implemented by `com.haulmont.cuba.gui.actions.list.CreateAction` class and should be defined in XML using `type="create"` action's attribute.

The action has common parameters that can be configured by XML attributes of the `action` element, see <<declarative_actions>> for details. In this section, we describe parameters specific to the `CreateAction` class.

The following parameters can be set both in XML and in Java:

* `openMode` - the editor screen opening mode as a value of the `OpenMode` enum: `NEW_TAB`, `DIALOG`, etc. By default, create action opens the editor in `THIS_TAB` mode.

* `screenId` - string id of the editor screen to use. By default, create action uses either a screen, annotated with `@PrimaryEditorScreen`, or having identifier of the form `<entity_name>.edit`, e.g. `demo_Customer.edit`.

...
The action is implemented by `com.haulmont.cuba.gui.actions.list.OpenAction` class and should be defined in XML using `type="picker_open"` action's attribute. You can configure common action parameters using XML attributes of the `action` element, see <<declarative_actions>> for details. Below we describe parameters specific to the `OpenAction` class.

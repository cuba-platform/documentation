...
View action is designed to open the entity edit screen in read-only mode. It is implemented by `com.haulmont.cuba.gui.actions.list.ViewAction` class and should be defined in XML using `type="view"` action's attribute.

View action opens the edit screen in the same way as <<EditAction>>, but it makes all fields non-editable and disables actions implementing interface `Action.DisabledWhenScreenReadOnly`. If you want to allow users to switch the screen to the editable mode, add a button and link it with the predefined `enableEditing` action:
...
The view action has common parameters that can be configured by XML attributes of the `action` element, see <<declarative_actions>> for details. In this section, we describe parameters specific to the `ViewAction` class.

...
[WARNING]
====
Keep in mind that `StandardEditor`, `StandardLookup` and `MasterDetailScreen` base classes load data automatically, so you should invoke this method only in custom screens inherited directly from `Screen`.
====

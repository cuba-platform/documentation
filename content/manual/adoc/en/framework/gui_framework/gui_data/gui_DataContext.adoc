...
+
--
Use `@Inject` annotation to get `DataContext` in the screen controller:
...
You can get an instance of `DataContext` outside screen controller. Use the `getScreenData` method of the `UiControllerUtils` class: 
...
Also, you can get `DataContext` from the UI component:

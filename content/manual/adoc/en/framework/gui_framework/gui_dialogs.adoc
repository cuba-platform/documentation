...
. Input dialog with an `Optional` type result value:
+
[source, java]
----
include::{sourcesdir}/gui/dialogs/gui_input_dialog_6.java[]
----
<1> - the `getOptional()` method returns an `Optional` result value that can be obtained using a parameter identifier.
+
User can get an `Optional` type value from `InputDialog`, `InputDialogCloseEvent`, `InputDialogResult` and `ValidationContext`.
--
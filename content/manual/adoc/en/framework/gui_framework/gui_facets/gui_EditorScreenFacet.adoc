...
`EditorScreenFacet` is a non-visual component that provides an ability to pre-configure <<opening_screens,editor screen>>. Declarative definition of the editor screen replaces existing `ScreenBuilders#editor(java.lang.Class<E>, FrameOwner)`. `EditorScreenFacet` is defined in the `facets` element of the screen XML descriptor.
...
`EditorScreenFacet` can be shown explicitly via the corresponding method:

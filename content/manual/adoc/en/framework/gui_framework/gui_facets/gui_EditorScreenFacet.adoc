...
`EditorScreenFacet` is a <<gui_facets,facet>> that provides an ability to pre-configure an <<opening_screens,editor screen>>. Declarative definition of the editor screen replaces existing `ScreenBuilders.editor()` method. `EditorScreenFacet` is defined in the `facets` element of the screen XML descriptor.

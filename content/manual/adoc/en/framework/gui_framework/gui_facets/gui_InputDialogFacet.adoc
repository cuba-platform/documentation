...
`InputDialogFacet` is a <<gui_facets,facet>> that provides an ability to pre-configure an <<gui_input_dialog,input dialog>>. Declarative definition of the input dialog replaces existing `Dialogs.createInputDialog()` method. `InputDialogFacet` is defined in the `facets` element of the screen XML descriptor.

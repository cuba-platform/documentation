...
`LookupScreenFacet` is a non-visual component that provides an ability to pre-configure <<opening_screens,lookup screen>>. Declarative definition of the lookup screen replaces existing `ScreenBuilders#lookup(java.lang.Class<E>, FrameOwner)`. `LookupScreenFacet` is defined in the `facets` element of the screen XML descriptor.
...
`LookupScreenFacet` can be shown explicitly via the corresponding method:

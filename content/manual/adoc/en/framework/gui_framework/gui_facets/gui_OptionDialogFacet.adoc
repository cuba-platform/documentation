...
`OptionDialogFacet` is a <<gui_facets,facet>> that provides an ability to pre-configure an <<gui_option_dialog,option dialog>>. Declarative definition of the option dialog replaces existing `Dialogs.createOptionDialog()` method. `OptionDialogFacet` is defined in the `facets` element of the screen XML descriptor.

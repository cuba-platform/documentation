...
* `@LoadDataBeforeShow` annotation indicates that all <<gui_data_loaders,data loaders>> should be triggered automatically before showing the screen. If you need to perform some actions when loading data but before the screen is shown, remove this annotation or set its value to `false` and use `getScreenData().loadAll()` method or `load()` methods of individual loaders in a <<screen_BeforeShowEvent,BeforeShowEvent>> listener.

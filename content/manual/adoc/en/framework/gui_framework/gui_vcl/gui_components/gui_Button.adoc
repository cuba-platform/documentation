...
[[gui_Button_shortcut]]
The `shortcut` attribute is used to specify a key combination for a button from a string representation. Possible modifiers: `ALT`, `CTRL`, `SHIFT` − are separated by the "-" character. For example:

...
----
+
If the view of the entity contains a reference attribute, the related entity is loaded as property according to its view. It means that if you want to show, e.g., `address.street` you have to define it manually:
+
[source, xml]
----
...
<1> – See the `createCloseButton` method description in the https://demo.cuba-platform.com/sampler/open?screen=datagrid-detailsgenerator[DataGridDetailsGeneratorSample] class.
<2> – See the `getContent` method description in the https://demo.cuba-platform.com/sampler/open?screen=datagrid-detailsgenerator[DataGridDetailsGeneratorSample] class.

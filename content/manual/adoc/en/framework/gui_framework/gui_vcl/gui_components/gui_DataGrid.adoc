...
* `columns` - an element that defines the `DataGrid` columns set. If not specified, columns will be automatically determined as the attributes from the <<views,view>> that is defined in <<gui_data_containers,dataContainer>> or <<datasources,datasource>>.
+

...
If you don't need to populate columns with data at XML load time, use the <<gui_attr_metaClass,metaClass>> attribute instead of setting a data container.
...
Cuba Studio will generate the `ButtonRenderer` declaration code in the `init()` screen controller method:
+
[source, java]
----
...
Cuba Studio will generate the `ClickableTextRenderer` declaration code in the `init()` screen controller method:
+
[source, java]
----
...
Cuba Studio will generate the `ImageRenderer` declaration code in the `init()` screen controller method:
+
[source, java]
----
...
Уou can set predefined styles to the `DataGrig` component using the `stylename` attribute in the XML descriptor.

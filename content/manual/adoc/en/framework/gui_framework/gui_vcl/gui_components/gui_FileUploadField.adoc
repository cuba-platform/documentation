...
* In the screen controller, inject the component itself, the <<file_upload,FileUploadingAPI>> and <<dataManager,DataManager>> interfaces. Subscribe to the the `InitEvent` and add listeners which will react on successful uploads and errors:
+
[source, java]
----

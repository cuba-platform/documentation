...
* In the screen controller, inject the component itself, the <<file_upload,FileUploadingAPI>> and <<dataManager,DataManager>> interfaces. Then subscribe to the events of successful uploads or error:
+
[source, java]
----
...
See https://www.cuba-platform.com/guides/working-with-images[Working with Images] guide for more complex example of working with uploaded files.

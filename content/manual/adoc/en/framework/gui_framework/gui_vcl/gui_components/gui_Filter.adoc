...
* `applyImmediately` – allows configuring the data filtering mode. If the attribute value is `false`, the filter works in the explicit mode. In this case, the filter will be applied only after the *Search* button is clicked. If the attribute value is `true`, the immediate mode is enabled. It allows applying filter immediately after changing the filter conditions. General cases when a filter is applied immediately:
+
--
...
** After clear all values.

In the immediate mode, the **Refresh** button will be used instead of the **Search** button.

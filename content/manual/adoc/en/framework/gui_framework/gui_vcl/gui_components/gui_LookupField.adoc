...
* The `setOptionImageProvider()` method allows you to define the images for the options displayed by the `LookupField` component. This method sets a function that accepts one of the <<gui_Image_ResourceType,resource types>>.
+
[source, java]
----

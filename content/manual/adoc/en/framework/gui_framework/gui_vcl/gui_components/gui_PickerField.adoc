...
* `PickerField` can be used without binding to entities, i.e., without setting <<gui_attr_dataContainer,dataContainer>> and <<gui_attr_property,property>>. In this case, `metaClass` attribute should be used to specify an entity type for `PickerField`. For example:
+
[source, xml]
----
...
[WARNING]
====
For proper operation of the `PickerField` component you need either set a `metaClass` attribute, or simultaneously set <<gui_attr_dataContainer,dataContainer>> and <<gui_attr_property,property>> attributes.
====

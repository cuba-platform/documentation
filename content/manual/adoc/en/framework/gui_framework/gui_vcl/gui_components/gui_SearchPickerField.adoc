...
[WARNING]
====
`SearchPickerField` works only in screens based on <<gui_legacy,legacy API>>. The similar functionality for the current API is provided by the <<gui_SuggestionPickerField,SuggestionPickerField>> component.
====

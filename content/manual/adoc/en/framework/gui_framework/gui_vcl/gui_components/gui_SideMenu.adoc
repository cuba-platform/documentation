...
The appearance of the `SideMenu` component can be customized using SCSS variables with `$cuba-sidemenu-++*++` and `$cuba-responsive-sidemenu-++*++` prefixes. You can change these variables in the visual editor after creating a <<web_theme_extension,theme extension>> or a <<web_theme_creation,custom theme>>.

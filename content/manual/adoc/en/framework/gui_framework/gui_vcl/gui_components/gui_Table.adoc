...
[[gui_Table_columns]]
* `columns` – an element defining the set of columns for a table. If not specified, columns will be automatically determined as the attributes from the <<views,view>> that is defined in <<gui_data_containers,dataContainer>> or <<datasources,datasource>>.
+

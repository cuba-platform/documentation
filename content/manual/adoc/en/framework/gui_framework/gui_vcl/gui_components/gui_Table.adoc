...
** `includeAll` – load all the attributes from the <<views,view>> that is defined in <<gui_data_containers,dataContainer>> or <<datasources,datasource>>.
+
...
----
+
If the view of the entity contains a reference attribute, it will load the related entity as property according to its view. It means that if you want to show, e.g., `address.street` you have to define it manually:
+
[source, xml]
----
...
In the example above, all cells within the `color` column in the table show the <<gui_LookupPickerField,LookupPickerField>> component. The component should save its value into the `color` attribute of the entity which instance is displayed in the corresponding row.
////
For this purpose, `getItemDatasource()` method is used to get the datasource for the current entity instance from the table and pass it to the `LookupPickerField` component.
////

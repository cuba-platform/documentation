...
+
The `autoLoad` attribute of the `RowsCount` component set in `true` enables loading rows count automatically. It can be set in the XML descriptor:
+
[source, xml]
----
...
* If the `columnControlVisible` attribute is set to `false`, users cannot hide columns using the drop-down menu of the image:gui_table_columnControl.png[] button in the right part of the table header. Currently displayed columns are marked with checkmarks in the menu. It is possible to show all table columns (`Select all`) or hide all columns that are possible except the first one (`Deselect all`). The first column is not hidden to display the table correctly.

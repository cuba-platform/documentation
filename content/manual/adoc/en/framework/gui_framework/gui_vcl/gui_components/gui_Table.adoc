...
* `emptyStateMessage` attribute enables to set the message when no data is loaded, null items are set, or empty container is used. This attribute is often used together with the <<gui_Table_emptyStateLinkMessage,emptyStateLinkMessage>> attribute. The message should contain information about why the table is empty and what it is for. For example:
+
[source, xml]
----
...
----
+
`emptyStateMessage` attribute support loading message from the <<message_packs,message pack>>. If you don't want to show a message, just do not specify this attribute.
...
+
`emptyStateLinkMessage` attribute support loading message from the <<message_packs,message pack>>. If you don't want to show a message, just do not specify this attribute.
+

...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.1/com/haulmont/cuba/gui/components/TextField.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
+
Default error messages are specified in the <<main_message_pack,main localized message pack>> and have the following template: `databinding.conversion.error.<type>`, e.g.:
+
[source, properties]
----
...
----
+
The origin of the `ValueChangeEvent` can be tracked using <<gui_api_UserOriginated,isUserOriginated()>> method.

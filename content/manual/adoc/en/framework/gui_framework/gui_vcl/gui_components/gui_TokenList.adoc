...
* `setOptionsMap()` enables specifying string names and option values separately. For example, we can set the following options map for the `tokenList` component injected in the controller:
+
--
[source, java]
----
...
Here, the `getValue()` method of the component will return selected entities, and not the strings that are displayed on the screen.
--

...
The `addExpandListener()` method adds the listener implementing the `ExpandEventListener` interface to intercept the item’s expanding events.
...
The `addCollapseListener()` method adds the listener implementing the `CollapseEventListener` interface to intercept the item’s collapsing events.

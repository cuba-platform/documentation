...
In this example, the `twinColumn` component will display <<namePattern_annotation,instance names>> of `Product` entity instances located in the `allProductsDc` <<gui_data_containers,data container>>, and its `getValue()` method will return a collection of selected instances.

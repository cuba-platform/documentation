...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.1/com/haulmont/cuba/gui/components/GroupBoxLayout.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
[[gui_groupbox_light]]
There is an additional predefined style for the `groupBox` container - `light`. You can set this one using the `stylename` attribute. A `Groupbox` with the `light` style has only the top border, as shown in the example below.

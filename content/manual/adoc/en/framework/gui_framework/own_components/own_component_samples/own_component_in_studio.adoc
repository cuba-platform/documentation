...
--
{studio_man_url}/#screen_descriptor[Screen Designer] of the CUBA Studio allows developers to integrate a custom UI component (or facet) implemented in an add-on or project by adding special *UI component metadata* annotations to the component's definition.
...
<<component_metadata_property_groups,StudioProperties annotation and property groups>>
...
Prerequisites::
--
Note that the result of the Screen Designer's work is the scaffolded XML code in the screen descriptor. However, for the custom component or custom facet to be successfully loaded from the XML in the running application, the following additional code should be implemented in your project:
...
--
Studio provides for custom UI components and facets the same support for events and delegate methods as for built-in UI components. No annotations are required for declaring an event listener or delegate method in a component interface.
...
Declared event handler becomes availble in the Inspector panel:
...
Component interface for the UI component that displays collection of entities provided by the <<gui_collection_container,CollectionContainer>>:

[source, java]
----
include::{sourcesdir}/ui_component/metadata/GenericComponentInterface.java[]
----
<1> - component interface is parameterized with the `E` parameter which represents the entity class of items displayed in the tree.
<2> - by specifying the `typeParameter` annotation attribute on the attribute of the `COLLECTION_DATACONTAINER_REF` type you can direct Studio to infer actual entity type by looking at the associated collection container.

...
If you need to define a route for a <<screens,legacy screen>> add the `route` (and optionally `routeParentPrefix`) attributes to the screen's element in the <<screens.xml,screens.xml>> file, for example:

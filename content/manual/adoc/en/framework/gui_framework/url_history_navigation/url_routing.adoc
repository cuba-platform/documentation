...
* `root` is the boolean property, that enables to specify whether a route is registered for the root screen (like <<login_screen,Login screen>> or <<main_window_layout,Main screen>>). The default value is `false`.

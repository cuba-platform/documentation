...
[TIP]
====
If you create a root screen other than the default <<login_screen,Login screen>> or <<main_window_layout,Main screen>> and make it available by a link without login, you should make it available for the anonymous user:

. Add the <<cuba.web.allowAnonymousAccess,cuba.web.allowAnonymousAccess>> property to the `web-app.properties` file:
+
[source, properties]
----
...
----
+
Otherwise, the user will always be re-directed to the `/app/#login` link, and your root screen will not be opened by URL.

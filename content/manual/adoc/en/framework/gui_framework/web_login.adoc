...
[TIP]
====
Learn https://www.cuba-platform.com/guides/anonymous-access-and-social-login[Anonymous Access & Social Login] guide, which includes an example of setting up public access to the CUBA application, as well as the implementation of custom login to the application using a Google, Facebook, or GitHub account.
====
...
- `WebSessionInitializedEvent` is sent when HTTP session is initialized.
- `WebSessionDestroyedEvent` is sent when HTTP session is destroyed.

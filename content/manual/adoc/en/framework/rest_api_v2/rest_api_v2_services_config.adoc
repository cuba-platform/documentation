...
If some service method needs to be invoked without authentication even if the <<rest_api_v2_anonymous, anonymous access>> to the REST API is disabled, then this method may be marked with the `anonymousAllowed="true"` attribute in the rest services configuration file:

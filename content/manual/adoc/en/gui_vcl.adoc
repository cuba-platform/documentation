...
The `Image` component can display value of an entity attribute. In the simplest case `image` can be declaratively associated with data using the `datasource` attribute:

[source, xml]
----
<image id="image" datasource="employeeDs"/>
----

The `image` elements enable setting the content source of the four types below:
...
Methods of an `Image` interface:

[[gui_Image_setDatasource]]
* `setDatasource()` - sets the datasource and its property. Only `FileDescriptor` and `byte[]` datasource property values are supported.
+
...
* `ClasspathImageResource` - an image located in classpath with the given path.

* `FileDescriptorImageResource` - an image which can be obtained from the `FileStorage` using the given `FileDescriptor`.

* `FileImageResource` - an image stored in the file system as the given `File`.

* `StreamImageResource` - a streaming representation of an image.

* `ThemeImageResource` - a theme image, for example, `VAADIN/themes/yourtheme/some/path/image.png`.

* `UrlImageResource` - an image which can be loaded from the given URL.
--

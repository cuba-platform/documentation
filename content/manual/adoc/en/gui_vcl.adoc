...
Filter can be made _global_ (i.e., available for all users) using the *Available to all users* checkbox and _global default_ using the *Global default* checkbox. These operations require a specific <<permissions, permission>> called CUBA > Filter > Create/modify global filters. If the filter is marked as _global default_ then it will be automatically selected when users open the screen. Each user can set his own default filter using the *Default for me* checkbox. This setting overrides the _global default_ one.

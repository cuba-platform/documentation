...
You can set predefined <<gui_Button_stylenames,styles>> to the Button component either in the XML descriptor or in the screen controller:
[source, xml]
----
...
* You can set predefined `borderless` style to the DateField component to remove borders and background from the field. This can be done either in the XML descriptor or in the screen controller:
+
[source, xml]
----
...
+
To make a dropZone static and display it permanently, assign the predefined `cuba-static-drop-zone` style to its container. In this case the container should be empty with only the `label` component inside:
+
[source, xml]
----
...
You can set predefined <<gui_Label_stylenames,styles>> to the `Label` component either in the XML descriptor or in the screen controller:
[source, xml]
----
...
* You can set predefined <<gui_LookupField_styles,styles>> to the `LookupField` component either in the XML descriptor or in the screen controller:
+
--
[source, xml]
----
...
[[gui_Table_stylename]]
* `stylename` attribute enables setting predefined styles to the `Table` component either in the XML descriptor or in the screen controller:
+
--
[source, xml]
----
...
You can set predefined <<gui_TextArea_styles,styles>> to the TextArea component either in the XML descriptor or in the screen controller:
...
* You can set predefined <<gui_TextField_styles,styles>> to the TextField component either in the XML descriptor or in the screen controller:
+
--
[source, xml]
----
...
+
Please note that the components located on a lazy tab do not exist when the screen is opened. Therefore they cannot be <<screen_controller_injection,injected>> into a controller and cannot be obtained by invoking `getComponent()` in the controller's `init()` method. The lazy tab components can be accessed only after the user opens the tab. This moment may be intercepted using `Accordion.TabChangeListener`, for example:
+
[source, java]
----
...
* `stylename` attribute allows you to set the predefined `borderless` style to the `accordion` component to remove borders and background:
+
[source, java]
----
...
You can set predefined styles to the `groupBox` component either in the XML descriptor or in the screen controller.

In order to use the `well` style, add an additional `v-panel-caption` stylename to any layout inside the groupBox layout to make it look like a layout header.
...
When setting a style programmatically, select one of the HaloTheme class constants with the `LAYOUT_` or `GROUPBOX_` prefix. To remove borders and the background color of the `groupBox`, use `borderless` style combined with `showAsPanel` attribute:
...
The attributes can be set either in the XML descriptor or in the screen controller:
[source, java]
----
...
You can set predefined <<gui_tabsheet_styles,styles>> to the `TabSheet` container either in the XML descriptor or in the screen controller:
[source, xml]
----

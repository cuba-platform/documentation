...
[[gui_Image_setSource]]
* `setSource()` - sets the content source for the component. The method accepts the resource type and return the resource object that can be configured using the fluent interface. Each resource type has its own methods, for example, `setPath()` for `ThemeImageResource` type or `setStreamSupplier()` for `StreamImageResource` type:
+
--
[source,java]
----

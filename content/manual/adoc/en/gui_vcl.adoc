...
`ColorPicker` is a field component that provides a visual interface for color selection as RGB color. It is used to preview and test
colors before applying them. `ColorPicker` supports color manipulations such as RGB and HSV conversion. Component returns a HEX value as a `String`.
...
The figure below shows an example of the color picker with a closed popup.
...
You can determine visibility of popup tabs with attributes:

* `swatchesVisible` - determines the visibility of swatches tab.
* `rgbVisible` - determines the visibility of RGB tab.
* `hsvVisible` - determines the visibility of HSV tab.
...
`getValue()` method of the component will return a `java.lang.String`, containing a HEX code of color.

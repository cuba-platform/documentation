...
+
--
* `id` - mandatory attribute with the column identifier.
...
[[gui_DataGrid_responsive]]
* `responsive` - indicates that the component should react on change in the available space. Reaction can be customized with the help of <<web_theme_extension,styles>>.
...
<<gui_DataGrid_responsive,responsive>> -
<<gui_attr_rowspan,rowspan>> -
...
`SideMenu` components provides means of customizing the <<main_window_layout,main window layout>>, managing menu items, adding icons and badges and applying custom styles.
...
`SideMenu` component is used to display menu items. The `MenuItem` API enables creating men items in the screen controller. The methods below can be used for dynamic update of menu items depending on the application business logic. The example of adding a menu item programmatically:
...
+
The standard `sideMenu` template includes several predefined styles: `c-sidemenu-layout`, `c-sidemenu-panel` and `c-sidemenu-title`. The default `c-sidemenu` style is defined in `cuba/modules/web/themes/halo/components/sidemenu/sidemenu.scss` file and is supported only within the `Halo` theme or its extensions. The `Havana` theme does not support `sideMenu` styling.

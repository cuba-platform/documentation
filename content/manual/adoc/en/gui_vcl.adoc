...
`Image` component is intended for displaying graphic content from different sources. The content source can be bound to a datasource or configured manually.
...
Alternatively, the `Image` component can use different resources as the content source. You can set the resource type declaratively using the `image` elements listed below:

[[gui_Image_classpath]]
* `classpath` - a resource that represents an image located in classpath with the given path.
+
[source, xml]
----
...
[[gui_Image_file]]
* `file` - a resource that represents an image stored in the file system as the given file.
+
[source, xml]
----
...
[[gui_Image_theme]]
* `theme` - a resource that represents a theme image, e.g.,`VAADIN/themes/customTheme/some/path/image.png`.
+
[source, xml]
----
...
[[gui_Image_url]]
* `url` - a resource which represents an image which can be loaded from the given URL.
+
[source, xml]
----
...
* `setDatasource()` - sets the datasource and the entity attribute. Only `FileDescriptor` and `byte[]` property values are supported.
+
The datasource can be set programmatically, for example, to display images in the table cells:
+
[source,java]
----
...
[[gui_Image_setSource]]
* `setSource()` - sets the content source for the component.
+
Compared with declarative elements, programmatically more source types are available. Each resource type has its own methods to set and get image source, for example, `setPath()` for `ThemeImageResource` type or `setStreamSupplier()` for `StreamImageResource` type:
+
--
[source,java]
----
...
* `createResource()` - creates the image resource implementation by its type.
+
--
[source,java]
----
...
----
You can use one of the following resource types implementing the `ImageResource` interface or extend it to create a custom resource:

* `ClasspathImageResource` - an image located in classpath with the given path. This resource can be also set declaratively using the `classpath` element of the `image` component.

* `FileDescriptorImageResource` - an image which can be obtained from the `FileStorage` using the given `FileDescriptor`.

* `FileImageResource` - an image stored in the file system as the given `File`. This resource can be also set declaratively using the `file` element of the `image` component.

* `StreamImageResource` - a streaming representation of an image.
...
+
--
Defines if user settings of the component should be saved/restored.

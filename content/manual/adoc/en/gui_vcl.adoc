...
.Components Diagram
image::gui_components.png[align="center"]
...
| <<gui_Calendar,Calendar>>| image:gui_calendar.png[]
...
The `Calendar` component enables organizing and displaying calendar events.
...
* `endDate` -  the end date for the calendar's range in java.util.Date format,

[[gui_Calendar_endDateProperty]]
* `endDateProperty` - String property with the name of an entity field for mapping the end date attribute,

[[gui_Calendar_isAllDayProperty]]
* `isAllDayProperty` - String property with the name of an entity field for mapping the boolean attribute,

[[gui_Calendar_startDate]]
* `startDate` - the start date for the calendar's range in java.util.Date format,

[[gui_Calendar_startDateProperty]]
* `startDateProperty` - String property with the name of an entity field for mapping the start date attribute,

[[gui_Calendar_timeFormat]]
* `timeFormat` - enumeration of time format: 12H or 24H.

All occurrences in a calendar are represented as events. There are two ways to manage the calendar events: adding events directly to the Calendar object using the `addEvent()` method, or using the event provider mechanism.
...
To be used for the `EntityCalendarEventProvider`, an entity should have attributes: `startDate` (DateTime type), `endDate` (DateTime type) and `caption` (String type).

Entity attributes should be mapped with `CalendarEvent` attributes, for example:
[source, xml]
----
...
* addDateClickListener(CalendarDateClickListener listener); - handles date clicks:
+
[source, java]
----
...
* `addWeekClickListener(CalendarWeekClickListener listener);` - handles week number clicks;
* `addEventClickListener(CalendarEventClickListener listener);` - handles calendar event clicks;
* `addEventResizeListener(CalendarEventResizeListener listener);` - handles event size changing;
* `addEventMoveListener(CalendarEventMoveListener listener);` - handles event drag and drop;
* `addForwardClickListener(CalendarForwardClickListener listener);` - handles calendar forward scrolling;
* `addBackwardClickListener(CalendarBackwardClickListener listener);` - handles calendar backward scrolling.

Calendar events can be styled with CSS by setting them a style name suffix retrieved with `the getStyleName()` method in CalendarEvent:
[source, java]
----
include::{sourcesdir}/gui_vcl/gui_calendar_8.java[]
----
An example of the `.scss`-file:
...
<<gui_attr_description,description>> -
<<gui_attr_descriptionProperty,descriptionProperty>> -
...
<<gui_attr_stylename,stylename>> -
<<gui_attr_stylenameProperty,stylenameProperty>> -
...
You can also define a resource files directory in the <<cuba.web.resourcesRoot,cuba.web.resourcesRoot>> application property and specify the name of a file inside this directory:
...
+
The `generateField()` method of the `CustomFieldGenerator` interface is invoked by `FieldGroup`. A <<datasources,data source>> and field identifier, for which this generator is registered, are passed into the method. The method should return a visual component (or container), which will be displayed in the field.
+
Example: 
+
[source, java]
...
+
To obtain a reference to a field component in a screen controller, you can use <<screen_controller_injection,injection>> instead of the explicit invocation of `getFieldComponent()`. To do this, use the `@Named` annotation and provide an identifier of `fieldGroup` and a field identifier after a dot.
+
...
* The `dropZone` XML attribute allows you to specify a <<gui_BoxLayout,BoxLayout>> to be used as a target for drag-and-dropping files from outside of the browser. The selected container is highlighted When a user drags files over the container.
...
* The `dropZone` XML attribute allows you to specify a <<gui_BoxLayout,BoxLayout>> to be used as a target for drag-and-dropping files from outside of the browser. The selected container is highlighted When a user drags a file over the container.
...
* <<cuba.gui.genericFilterColumnsCount,cuba.gui.genericFilterColumnsCount>> - sets the default number of columns for placing conditions on the filter panel. See also <<gui_Filter_columnsQty,columnsQty>> filter attribute.
...
---- 
+
In this case, the component will display <<namePattern,instance names>> of the `Colour` entity located in the `colorsDs` data source, and the selected value will be set into the `colour` attribute of the `Car` entity, which is located in the `carDs` data source.
+
...
`SourceCodeEditor` is a component to display and enter source code. It is multi-line text area featured with code highlighting and optional print margin and gutter with line numbers.
...
+
Please note that the components located on a lazy tab do not exist when the screen is opened. Therefore they cannot be <<screen_controller_injection,injected>> into a controller and cannot be obtained by invoking `getComponent()` in the controller's `init()` method. The lazy tab components can be accessed only after the user opens the tab. This moment may be intercepted using `TabSheet.TabChangeListener`, for example:
+
[source, java]
----
...
<<gui_attr_height,height>> -
<<gui_attr_id,id>> -
<<gui_cssLayout_responsive,responsive>> -
<<gui_attr_stylename,stylename>> -
<<gui_attr_visible,visible>> -
<<gui_attr_width,width>>
...
If `captionProperty` is not defined, <<namePattern,instance name>> is shown.

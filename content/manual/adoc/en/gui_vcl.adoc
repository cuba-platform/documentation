...
Listeners of checkBox::
<<gui_checkBox_ValueChangeListener,ValueChangeListener>>
...
Changes of the color picker value, as well as of any other components implementing the `Field` interface, can be tracked using a `ValueChangeListener`.
...
Listeners of colorPicker::
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
<<gui_DataGrid_setRowStyleProvider,setCellStyleProvider>> -
...
Listeners of filter::
<<gui_Filter_ExpandedStateChangeListener,ExpandedStateChangeListener>> -
<<gui_Filter_FilterEntityChangeListener,FilterEntityChangeListener>>
...
Listeners of groupTable::
<<gui_Table_CellClickListener,CellClickListener>> -
<<gui_Table_ColumnCollapseListener,ColumnCollapseListener>>
...
Listeners of image::
<<gui_Image_SourceChangeListener,SourceChangeListener>>
...
Listeners of lookupPickerField::
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
Listeners of maskedField::
<<gui_TextField_EnterPressListener,EnterPressListener>> -
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
Listeners of popupView::
<<gui_PopupView_PopupVisibilityListener,PopupVisibilityListener>>
...
Listeners of searchPickerField::
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
Listeners of suggestionPickerField::
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
Listeners of textArea::
<<gui_TextArea_ResizeListener,ResizeListener>> -
<<gui_TextField_TextChangeListener,TextChangeListener>> -
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
* `NONE` - without conversion (the default value).
...
Listeners of tokenList::
<<gui_TokenList_ItemClickListener,ItemClickListener>> -
<<gui_TextField_ValueChangeListener,ValueChangeListener>>
...
Listeners of accordion::
<<gui_accordion_SelectedTabChangeListener,SelectedTabChangeListener>>
...
API::
<<gui_BoxLayout_addShortcutAction,addShortcutAction>>
...
API::
<<gui_ButtonsPanel_addShortcutAction,addShortcutAction>>

Listeners of buttonsPanel::
<<gui_ButtonsPanel_LayoutClickListener,LayoutClickListener>>
...
Clicks on the `CssLayout` area can be intercepted with the help of the `LayoutClickListener` interface.

[[gui_СssLayout_addShortcutAction]]
You can use keyboard shortcuts in `СssLayout`. Set the shortcut and the action to be performed using the `addShortcutAction()` method:
...
API::
<<gui_СssLayout_addShortcutAction,addShortcutAction>>

Listeners of cssLayout::
<<gui_СssLayout_LayoutClickListener,LayoutClickListener>>
...
API::
<<gui_GridLayout_addShortcutAction,addShortcutAction>>

Listeners of grid::
<<gui_GridLayout_LayoutClickListener,LayoutClickListener>>
...
API::
<<gui_GroupBox_addShortcutAction,addShortcutAction>>

Listeners of groupBox::
<<gui_GroupBox_ExpandedStateChangeListener,ExpandedStateChangeListener>>
...
API::
<<gui_ScrollBox_addShortcutAction,addShortcutAction>>
...
Listeners of split::
<<gui_SplitPanel_PositionUpdateListener,PositionUpdateListener>>
...
Listeners of tabSheet::
<<gui_TabSheet_SelectedTabChangeListener,SelectedTabChangeListener>>
...
---- 
 ...
 Common::
 Below is the list of methods available for all visual components.
 +
 --
 [[gui_api_id]]
 * `getId()` - returns the component identifier defined in <<gui_attr_id,id>> attribute.

 * `setId()` - sets the component identifier.

 [[gui_api_parent]]
 * `getParent()` - returns parent of the component.

 * `setParent()` - sets the parent for the component. This method is called implicitly when the component is added to the container using the  `add()` method.

 [[gui_api_debug]]
 * `getDebugId()` - returns the component debug identifier.

 * `setDebugId()` - sets the component debug identifier.

 [[gui_api_enable]]
 * `isEnabled()` - returns `true` if the component and its parent are <<gui_attr_enable,enabled>>.

 * `setEnabled()` - sets the component `enabled` state.

 * `isEnabledItself()` - returns `true` if the component is enabled regardless its parent.

 [[gui_api_responsive]]
 * `isResponsive()` - returns `true` if the component is <<gui_attr_responsive,responsive>>.

 * `setResponsive()` - sets the component `responsive` state.

 [[gui_api_visible]]
 * `isVisible()` - returns `true` if the component and its parent are <<gui_attr_visible,visible>>.

 * `setVisible()` - sets the component `visible` state.

 * `isVisibleItself()` returns `true` if the component is visible regardless the visibility of its parent.

 [[gui_api_focus]]
 * `requestFocus()` - set focus to this component.

 [[gui_api_height]]
 * `getHeight()` - returns the component <<gui_attr_height,height>> in pixels or in percents.

 * `getHeightUnits()` - returns the component height units: `PIXELS` or `PERCENTAGE`.

 * `setHeight()` - sets the component height in pixels or in percents of the parent container's height (if not specified, pixels are assumed by default).

 * `setHeightAuto()` - sets the component height to `AUTO_SIZE`: a default value will be used for the component height. For a container, default height is defined by the sum of all nested components' heights.

 * `setHeightFull()` - sets the component height to `100%`.

 [[gui_api_width]]
 * `getWidth()` - returns the component <<gui_attr_width,width>> in pixels or in percents.

 * `getWidthUnits()` - returns the component width units: `PIXELS` or `PERCENTAGE`.

 * `setWidth()` - sets the component width in pixels or in percents of the parent container's width (if not specified, pixels are assumed by default).

 * `setWidthAuto()` - sets the component width to `AUTO_SIZE`.

 * `setWidthFull()` - sets the component width to `100%`.

 [[gui_api_size]]
 * `setSizeFull()` - sets both the component width and height to `100%`.

 * `setSizeAuto()` - sets both the component width and height to `AUTO_SIZE`.

 [[gui_api_align]]
 * `getAlignment()` - returns the component position relative to the parent container defined in the <<gui_attr_align,align>> attribute.

 * `setAlignment()` - sets the component position relative to the parent container.

 [[gui_api_stylename]]
 * `getStyleName()` - returns current <<gui_attr_stylename,stylename>>. Styles implementation is client-specific.

 * `setStyleName()` - sets one or more style names of the component, replacing any previous styles. Multiple styles can be specified as a space-separated list of style names.

 * `addStyleName()` - adds one or more style names to the component.

 * `removeStyleName()` - removes one or more style names from component.

 [[gui_api_unwrap]]
 * `unwrap()` - returns client-specific component instance. Can be used in client module to simplify invocation of underlying API.
 ...
 * `unwrapComposition()` - returns the outmost external container of client-specific component instance. Can be used in client module to simplify invocation of underlying API.
 ...
 +
 --
 * `getAction()` - returns an <<gui_Action,action>> by its ID, or null if not found.

 * `setAction()` - sets an action to the component.

 These methods are available for the component:
 ...
 +
 --
 * `addAction()` - add an <<gui_Action,action>> to the component.

 * `removeAction()` - removes the action by its ID. If there is no action with that ID, nothing happens.

 * `removeAllActions()` - removes all actions from the component.

 * `getActions()` - returns the collection of actions added to the component.

 * `getActionNN()` - returns an action by its id or throws an exception if the id is not found.

 These methods are available for components:

 <<gui_DataGrid,DataGrid>> -
 <<gui_Frame,Frame>>
 <<gui_PickerField,PickerField>> -
 <<gui_PopupButton,PopupButton>> -
 <<gui_Table,Table>> -
 <<gui_Tree,Tree>>
 --

 Component.ActionsHolder.SecuredActionsHolder::
 +
 --
 * `getActionsPermissions()` - returns the collection of specific permissions added to the component.

 This method is available for components:
 ...
 +
 --
 * `getFrame()` - returns the frame the component belongs to.

 * `setFrame()` - sets the frame to the component.

 These methods are available for components:

 <<gui_Accordion,Accordion>> -
 <<gui_BoxLayout,BoxLayout>> -
 <<gui_BulkEditor,BulkEditor>> -
 <<gui_Button,Button>> -
 ...
 <<main_window_layout,UserIndicator>>
 --
 ...
 * `isBuffered()` - returns `true` if buffered mode is on for the component.

 * `setBuffered()` - sets the buffered mode.
 ...
 * `isModified()` - returns `true` if the object value has been modified since it was last updated from the data source.
 ...
 +
 --
 * `isExpanded()` - returns `true` if the component has been <<gui_attr_expand,expanded>>.

 * `setExpanded()` - sets the expanded state to the component.

 * `isCollapsable()` - returns `true` if the component's content can be collapsed using the image:gui_groupBox_minus.png[] or image:gui_groupBox_plus.png[] buttons.

 * `setCollapsable()` - if the value is set to `true`, the component’s content can be hidden or expanded using image:gui_groupBox_minus.png[] or image:gui_groupBox_plus.png[] buttons.
 ...
 * `removeExpandedStateChangeListener()` - removes the `ExpandedStateChangeListener` from the component.
 ...
 * `indexOf()` - returns the index of  a given component in an ordered container.
 ...
 +
 --
 * `setEditable()` - sets the component <<gui_attr_editable,editable>> state.

 * `isEditable()` - returns `true` if the component is editable and `false` if read-only.
 ...
 +
 --
 * `isFocusable()` - returns `true` if the component can take focus.

 * `setFocusable()` - sets the component focusability.

 * `getTabIndex()` - gets the tabulator index of the component, which defines the order the components take focus on a *Tab* click.

 * `setTabIndex()` - sets the tabulator index of the component.
 ...
 +
 --
 * `isBorderVisible()` - returns `true` if the border of the component is visible.

 * `setBorderVisible()` - sets the component's border visibility.
 ...
 +
 --
 * `getButtonsPanel()` - returns the component's <<gui_ButtonsPanel>>.

 * `setButtonsPanel()` - sets the `ButtonsPanel` for the table component.
 ...
 +
 --
 * `getCaption()` - returns the component's <<gui_attr_caption>>.

 * `setCaption()` - sets the caption for the component.

 * `getDescription()` - returns the component's <<gui_attr_description>>.

 * `setDescription()` - sets the description for the component.
 ...
 +
 --
 * `setFormatter()` - sets the <<gui_formatter>> for the component.

 * `getFormatter()` - returns the component's formatter.

 These methods are available for components:

 <<gui_DataGrid,DataGrid column>> -
 <<gui_FieldGroup,FieldGroup>> -
 <<gui_Label,Label>> -
 <<gui_Table,Table column>> -
 <<gui_TextField,TextField>>
 --
 ...
 +
 --
 * `getIcon()` - returns the component's <<gui_attr_icon>>.

 * `setIcon()` - sets the icon for the component.
 ...
 +
 --
 * `getInputPrompt()` - returns the component's <<gui_attr_inputPrompt>>.

 * `setInputPrompt()` - sets the textual prompt for an input field.
 ...
 +
 --
 * `getComponent()` - gets the sub-component by its name.

 This method is available for component:
 ...
 * `isSettingsEnabled()` - returns `true` if the <<gui_attr_settingsEnabled,settings>> saving is enabled.

 * `setSettingsEnabled()` - defines if the user settings for the component should be saved.
 ...
 Component.HasSettings.HasPresentations::
 +
 --
 * `usePresentations()`
 ...
 * `applyPermission()`
 ...
 This method is available only in *web* module for components:

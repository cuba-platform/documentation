...
---- 
+
The component will invoke `uploadSucceeded()` after uploading the file to a temporary storage of the client <<app_tiers,tier>>. At this point, you can get component's `FileDescriptor` object, which corresponds to the uploaded file. `com.haulmont.cuba.core.entity.FileDescriptor` (do not confuse with `java.io.FileDescriptor`) is a persistent entity, which uniquely identifies an uploaded file and then is used to download the file from the system.
+

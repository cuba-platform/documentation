...
| <<gui_ColorPicker,ColorPicker>>| image:gui_colorPicker.png[]
...
* if `handleTabKey` is `true`, the *Tab* button on the keyboard is handled to indent lines, when `false`, it is used to advance the cursor or focus to the next tab stop. This attribute should be initialized with the page loading and will not work if changed at a runtime.
...
`template` attribute defines the name of a HTML file located in the `layouts` subdirectory of your theme. You should create a <<web_theme_extension,theme extension>> or a <<web_theme_creation,custom theme>> before creating a template. For example, if your theme is Halo and the attribute contains `my_template`, the template file should be `modules/web/themes/halo/layouts/my_template.html`.

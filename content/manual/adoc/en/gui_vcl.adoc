...
The <<gui_attr_icon,icon>> attribute defines icon location. Detailed information on recommended icon placement is available in <<gui_themes>>.
...
* <<cuba.allowQueryFromSelected,cuba.allowQueryFromSelected>> enables switching off <<sequential_filter_para,sequential filters application>> mechanism.

* <<cuba.cuba.gui.genericFilterColumnsQty,cuba.gui.genericFilterColumnsQty>> - sets the default number of columns for placing conditions on the filter panel. See also <<gui_Filter_columnsQty,columnsQty>> filter attribute.
...
Ability to edit content of a component bound to data (inheritor of `DatasourceComponent` or `ListComponent`) is also influenced by the <<chapter_security,security subsystem>>. If the security subsystem information indicates that the component should not be editable, the value of its `editable` attribute is ignored.
--

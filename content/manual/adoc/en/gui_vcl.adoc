...
[[gui_colorPicker_ValueChangeListener]]
Changes of the color picker value can be tracked using a `ValueChangeListener`.
...
<<gui_colorPicker_ValueChangeListener,ValueChangeListener>>
...
<<gui_DataGrid_settingsEnabled,settingsEnabled>> -
<<gui_DataGrid_sortable,sortable>> -
<<gui_attr_stylename,stylename>> -
<<gui_attr_tabIndex,tabIndex>> -
...
[[gui_DateField_ValueChangeListener]]
...
<<gui_DateField_ValueChangeListener,ValueChangeListener>>
...
[[gui_DatePicker_ValueChangeListener]]
Changes of the `DatePicker` value, as well as of any other components implementing the `Field` interface, can be tracked using a `ValueChangeListener`.
...
<<gui_DatePicker_ValueChangeListener,ValueChangeListener>>
...
* `fieldFactoryBean`: declarative fields defined in the XML-descriptor are created with the `FieldGroupFieldFactory` interface. In order to override this factory, use this attribute with the name of your custom `FieldGroupFieldFactory` implementation. For the `FieldGroup` created programmatically, use the `setFieldFactory()` method.
...
[[gui_FileUploadField_ValueChangeListener]]
* `ValueChangeListener`.
--
...
<<gui_FileUploadField_ValueChangeListener,ValueChangeListener>>
...
[[gui_Label_ValueChangeListener]]
The dynamic changes of the label text can be tracked using the `ValueChangeListener`.
...
Listeners of label::
<<gui_Label_ValueChangeListener,ValueChangeListener>>
...
[[gui_LookupField_ValueChangeListener]]
* The changes of the value in `LookupField` can be tracked using the `ValueChangeListener`.
...
<<gui_LookupField_ValueChangeListener,ValueChangeListener>>
...
[[gui_OptionsGroup_ValueChangeListener]]
* The value changes in the `OptionsGroup` component can be tracked using the `ValueChangeListener`.
...
<<gui_OptionsGroup_ValueChangeListener,ValueChangeListener>>
...
[[gui_ProgressBar_ValueChangeListener]]
...
<<gui_ProgressBar_ValueChangeListener,ValueChangeListener>>
...
image::gui_SourceCodeEditor_dia.png[align="center"]
...
`SuggestionPickerField` is a component that provide functionality of a typical autocomplete component and the <<gui_PickerField,PickerField>> component. It suggests to user some variants in a popup depending on the logic that is defined by a programmer.

image::gui_suggestionPickerField_1.png[align="center"]

XML name of the component: `suggestionPickerField`.

image::gui_suggestionPickerField.png[align="center"]

The component is implemented for the *Web Client*.

Being a selection field, `SuggestionPickerField` is used for working with reference entity attributes, so you can set the `datasource` and the `property` attributes for the component:
...
* `asyncSearchDelayMs` - sets the delay between the last key press action and async search.

[[gui_suggestionPickerField_metaClass]]
* `metaClass` - sets the link to the component's metaClass, if the component is used without binding to entities, i.e., without setting <<gui_attr_datasource,datasource>> and <<gui_attr_property,property>>.
...
+
--
In the most common case, is sufficient to set the `SearchExecutor` to the component. `SearchExecutor` is the functional interface that contains a single method: `List<E extends Entity> search(String searchString, Map<String, Object> searchParams)`:
...
The `searchString` parameter can be used to look through the entries of the certain entity attribute:
...
+
--
Another way to use the component is to set the `EnterActionHandler` or `ArrowDownActionHandler`. These listeners are fired when a user presses *Enter* or *Arrow Down* keys and popup with suggestions is not shown. They also are functional interfaces with single method and single parameter - `currentSearchString`. You can set up your own action handlers and use `SuggestionField` `showSuggestions()` method which accepts the list with Entities to show suggestions.
...
* `ValueChangeListener` is triggered on the value changes when the user finished the text input, i.e. after the Enter press or when the component loses focus.
...
[[gui_TimeField_ValueChangeListener]]
* Changes of the `TimeField` value, as well as of any other components implementing the `Field` interface, can be tracked using a `ValueChangeListener`.
...
<<gui_TimeField_ValueChangeListener,ValueChangeListener>>
...
[[gui_TokenList_ValueChangeListener]]
...
<<gui_TokenList_ValueChangeListener,ValueChangeListener>>
...
[[gui_TwinColumn_ValueChangeListener]]
Changes of the `TwinColumn` values, as well as of any other components implementing the `Field` interface, can be tracked using a `ValueChangeListener`.
...
<<gui_TwinColumn_ValueChangeListener,ValueChangeListener>>

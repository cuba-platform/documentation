...
* The `nullOptionVisible` XML attribute sets visibility for first null element in suggestion popup. May work only if `LookupField` is not <<gui_attr_required,required>>.

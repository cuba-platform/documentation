...
The `PopupView` is a component allows to open a popup with container. Container may open by clicking on minimized value or programmatically. It can be closed by mouse out or by click on outside area.

A typical PopupView with hidden and visible popup is shown below:

image::gui_popup_view_mini.png[align="center"]
image::gui_popup_view_mini_open.png[align="center"]
...
The content of inner container are defined inside `PopupView` as any layout component like `HBoxLayout` or `VBoxLayout`.

`PopupView` methods:
--
* `setPopupVisible()` allows you to open popup window programmaticaly.
+
[source, java]
----
...
----
+
* `setMinimizedValue()` allows you to set any components as popup content.
+
[source, java]
----
...
* `minimizedValue` attribute define the text of popup button.
...
* `captionAsHtml` attribute allow to use HTML tags in caption of component.
--

...
++++
<div class="manual-live-demo-container">
    <a href="https://demo.cuba-platform.com/sampler/open?screen=datagrid-basic-settings" class="live-demo-btn" target="_blank">TODO LIVE DEMO</a>
</div>
++++
...
Basically, `CurrencyField` repeats the functionality of <<gui_TextField,TextField>>. You can manually set a <<datatype,datatype>> for the field, except that only numeric datatypes inheriting `NumericDatatype` class are supported. If the datatype is unparseable, a `RuntimeException` exception will be thrown.

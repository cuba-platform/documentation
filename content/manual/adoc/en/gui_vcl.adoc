...
This is a component that allows a user to choose from a vertical list.
If multi-select is enabled, multiple items can be selected by holding the Ctrl key pressed while clicking, or as a range by holding the Shift key.
...
* The simplest case of using `OptionsList` is to select an enumeration value for an entity attribute. For example, a `Role` entity has `type` attribute of the `RoleType` type, which is an enumeration. Then you can use `OptionsList` to edit this attribute as follows:
+
[source, xml]
----
include::{sourcesdir}/gui_vcl/optionslist_1.xml[]
----
+
In the example above `roleDs` <<datasources,data source>> is defined for the `Role` entity. In the `optionsList` component, the link to the data source is specified in the <<gui_attr_datasource,datasource>> attribute and the name of the entity attribute is set in the <<gui_attr_property,property>> attribute.
...
[source, xml]
----
include::{sourcesdir}/gui_vcl/optionslist_2.xml[]
----
+
Then inject the component into the controller and specify a list of options in the `init()` method:
+
[source, java]
----
include::{sourcesdir}/gui_vcl/optionslist_3.java[]
----
+
The component will be as follows:
+
image::gui_optionsList_integerList.png[align="center"]
+
Depending on the selected option, the `getValue()` method of the component will return `Integer` values: 2, 4, 5, 7.
...
[source, java]
----
include::{sourcesdir}/gui_vcl/optionslist_4.java[]
----
+
The component will be as follows:
+
image::gui_optionsList_integerMap.png[align="center"]
+
Depending on the selected option, the `getValue()` method of the component will return `Integer` values: 2, 4, 5, 7, and not the strings that are displayed on the screen.
...
[source, xml]
----
include::{sourcesdir}/gui_vcl/optionslist_5.xml[]
----
+
In this case, the `coloursField` component will display <<namePattern,instance names>> of the `Colour` entity, located in the `coloursDs` data source, and its `getValue()` method will return the selected entity instance.
+
...
[[gui_OptionsList_multiselect]]
* `multiselect` attribute is used to switch `OptionsList` to a multiple choice mode. If `multiselect` is turned on, you can choose more than one element by holding the Ctrl key pressed while clicking, or as a range by holding the Shift key.
+
For example, if we create the component in the XML screen descriptor:
+
[source, xml]
----
include::{sourcesdir}/gui_vcl/optionslist_6.xml[]
----
+
In this case the `getValue()` method of the component will return a `java.util.List`.
...
<<gui_attr_icon,icon>> -
<<gui_attr_id,id>> -
<<gui_OptionsList_multiselect,multiselect>> -
<<gui_attr_height,height>> -
<<gui_attr_optionsDatasource,optionsDatasource>> -
<<gui_attr_property,property>> -
<<gui_attr_required,required>> -
...
`TextArea` mostly replicates the functionality of the <<gui_TextField,TextField>> component except that a `datatype` can not be assigned to it. I.e. `TextArea` is intended to be used only for text and entity attributes of type `String`.
...
*  `simple` – when set to `true`, the items selection component will be hidden with only the *Add* button left. Clicking the *Add* button opens the screen with the list of entity instances which type is defined by the datasource. Selection screen identifier is selected according to the rules for the `PickerField.LookupAction` <<picker_actions,standard action>>.
...
** If the value of the `multiselect` attribute is set to `true`, then `true` will be passed to parameters map of the lookup screen for the `MULTI_SELECT` key. This flag can be used to set the screen into multiple selection mode. This flag is defined in the `WindowParams` enum so it is convenient to work with it in the following way:
+
[source, java]
----
...
* `button` – descriptor of the button for adding items. Can contain <<gui_attr_caption,caption>> and <<gui_attr_icon,icon>> attributes.
...
`template` attribute defines the name of a HTML file located in the `layouts` subdirectory of your theme. For example, if your theme is Halo and the attribute contains `my_template`, the template file should be `modules/web/themes/halo/layouts/my_template.html`.

Content of the HTML templated located in the `modules/web/themes/halo/layouts/sample.html` file:

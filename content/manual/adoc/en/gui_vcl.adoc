...
`Image` component is intended for displaying graphic content from different sources. It can be bound to a datasource or configured manually.

XML name of the component: `image`.

image::gui_Image.png[align="center"]
...
+
Below is an example of displaying images bound to the table datasource in the table cells:
+
[source,java]
----
...
[[gui_Image_setSource]]
* `setSource()` - sets the content source for the component using the `ImageResource` interface or its inheritors. Each resource type has his own methods to set and get image source, for example:
+
--
[source,java]
----
...
[source,java]
----
include::{sourcesdir}/gui_vcl/image_5.java[]
----
--
...
----
You can use one of the following resource type or extend it to create a custom resource:
...
[[gui_Image_getMetaPropertyPath]]
* `getMetaPropertyPath()` - returns the datasource <<MetaPropertyPath,property path>>.
...
<<gui_Image_getMetaPropertyPath,getMetaPropertyPath()>> -

...
[[gui_TextArea_datatype]]
* If the field is not connected to an entity attribute (i.e. the data source and attribute name are not set), you can set the data type using the `datatype` attribute. It is used to format field values. The attribute value accepts any data type registered in the application metadata – see <<datatype,Datatype>>. Typically, TextField uses the following data types:
+
** decimal
** double
** int
...
<<gui_attr_datasource,datasource>> -
<<gui_TextArea_datatype,datatype>> -
<<gui_attr_description,description>> -
<<gui_attr_editable,editable>> -
<<gui_attr_enable,enable>> -
<<gui_attr_height,height>> -

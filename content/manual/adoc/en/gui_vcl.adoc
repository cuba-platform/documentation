...
To display events in the calendar cells, you can add the events directly to the `Calendar` object using the `addEvent()` method or use the event provider mechanism.
...
Calendar events can be styled with CSS. To configure a style, create the style name and set the parameters in the `.scss`-file:
[source, scss]
----
include::{sourcesdir}/gui_vcl/gui_calendar_1.scss[]
----
Then use the `setStyleName` method to an event:
[source, java]
----

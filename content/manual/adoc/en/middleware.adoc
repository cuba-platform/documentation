...
Services form the layer that defines a set of Middleware operations available to the client <<app_tiers,tier>>. Services encapsulate business logic and <<transactions,transaction>> management.
...
An instance of `EntityManager` contains a _persistence context_ – a set of instances loaded from DB or newly created. Persistence context is a kind of data cache within a transaction.`EntityManager` automatically flushes to DB all changes done in its persistence context at the moment when a transaction is committed or when the `flush()` method is called explictly.
...
* Statically – the names of listener classes are listed in <<listeners_annotation,@Listeners>> annotation on the entity class.
+
[source, java]
----
@Entity(...)
...
* Dynamically – entity and listener classes are passed to `addListener()` method of the `EntityListenerManager` bean. See example in this section: <<app_start_recipe>>.
...
. Once parent classes are processed, dynamically added listeners for the given class are called first, followed by statically assigned.

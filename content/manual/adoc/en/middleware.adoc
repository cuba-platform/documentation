...
`EntityManager` interface used in CUBA applications mainly copies the standard link:http://docs.oracle.com/javaee/5/api/javax/persistence/EntityManager.html[javax.persistence.EntityManager] interface. Let us have a look at its main methods:
...
The methods of `Query` mainly correspond to the methods of a standard link:http://docs.oracle.com/javaee/5/api/javax/persistence/Query.html[javax.persistence.Query] interface. Let us have a look at the differences.

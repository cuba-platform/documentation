...
.2+|Aggregate functions with scalar expression
...
.2+|CASE Expression in UPDATE query
...
.3+|String Functions (CONCAT, SUBSTRING, TRIM, LOWER, UPPER, LENGTH, LOCATE)
...
|TRIM is not supported with trim char
...
|Not supported: subquery with path variable

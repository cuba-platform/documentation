...
In any CUBA application, there is always the main data store which contains system and security entities and where the users log in. When we mention a database in this manual, we always mean the main data store if other is not explicitly stated. The main data store must be a relational database connected through a JDBC data source. The main data source is located in JNDI and should have a name specified in the <<cuba.dataSourceJndiName,cuba.dataSourceJndiName>> application property, which is `jdbc/CubaDS` by default.
...
For example, if you need to work with two additional data stores: `db1` (a PostgreSQL database) and `mem1` (an in-memory storage implemented by some project bean), specify the following application properties in the `app.properties` file of your *core*:
...
[TIP]
====
CUBA Studio allows you to set up additional data stores on the *Project properties > Advanced* tab. It automatically creates all required application properties and JDBC data sources, as well as maintains additional `persistence.xml` files.
====
...
* `getDataSource()` – returns the `javax.sql.DataSource` instance for the currently used database. The method can accept a <<data_store,data store>> name. If it is not provided, the main data store is assumed.
+
[WARNING]
====
...
[TIP]
====
Unlike for <<dataManager,DataManager>>, all local attributes are loaded regardless of what is specified in the view. In `EntityManager`, the view affects only reference attributes.
====
--
...
[TIP]
====
See <<dm_vs_em>> for information on differences between <<dataManager,DataManager>> and EntityManager.
====
...
* `eclipselink.cache.shared.default = true` - turns on caching of all entities.

* `eclipselink.cache.shared.sales$Customer = true` - turns on caching of `sales$Customer` entity only.

...
`@Transactional` annotation supports a number of parameters. The main parameter is the transaction creation mode – `Propagation`. The `REQUIRED` value corresponds to `getTransaction()`, the `REQUIRES_NEW` value – to `createTransaction()`. The default value is `REQUIRED`.

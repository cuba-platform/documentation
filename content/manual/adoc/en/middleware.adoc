...
* Uses a concise object query language – <<jpql, JPQL>>.
...
* `setView()` – sets the default <<views,view>>, which will be used for further loading of instance via `find()` or JPQL requests. As a result, all non-lazy attributes of the view will be <<eager_fetching, eagerly fetched>>.
+
...
Lazy loading generates more DB queries than <<eager_fetching, eager fetching>>, but it is stretched in time.

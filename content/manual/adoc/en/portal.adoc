...
The `/deployViews?s=<sessionId>` POST request allows to load to middleware descriptions of <<views,view>> objects, required for the client. The view objects are sent as standard XML description of a view used in the platform. XML is placed in the request body. For more information about the format, see .

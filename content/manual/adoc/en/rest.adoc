...
Any running CUBA application also exports the swagger documentation, see <<rest_swagger,>>.
====
...
The `<query>` element has a `cacheable` attribute that enables <<entity_cache,caching>> for the query.
...
Any running CUBA application also exports at runtime the project-specific Swagger documentation generated by the `SwaggerGenerator` bean according to the version 2.0 of https://swagger.io/docs/specification/2-0/basic-structure/[Swagger] https://github.com/OAI/OpenAPI-Specification/blob/master/versions/2.0.md[specification].

The documentation is available by the following addresses:

* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.yaml` - YAML version of generic documentation,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.json` - JSON version of generic documentation,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swaggerDetailed.yaml` - YAML version of project-specific Swagger documentation,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swaggerDetailed.json` - JSON version of project-specific Swagger documentation.
...
. Predefined REST queries, such as <<rest_api_v2_queries_config,JPQL queries>> defined in `rest-queries.xml`:
+
image::swagger_query.png[align="center"]

. Exposed services, such as <<rest_api_v2_services_config,services>> defined in `rest-services.xml`:
+
image::swagger_service.png[align="center"]

This documentation can be used to test the project's REST API, for example, by https://swagger.io/swagger-inspector/[Swagger Inspector] or via importing to https://www.getpostman.com/[Postman], to https://swagger.io/swagger-codegen/[generate] a code that will interact with the app via REST API, or simply to https://swagger.io/swagger-ui/[visualize] the existing REST API.
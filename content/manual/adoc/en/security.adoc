...
. For Windows domain users, enable logging in through Single Sign-On without having to specify the username and password.
...
* `cuba.web.ldap.user` – the distinguished name of a user which has the right to read the information from the directory.

* `cuba.web.ldap.password` – the password for the user defined in the `cuba.web.ldap.user` property.
...
When creating users in the application, use their `sAMAccountName` without domain if you work with Active Directory.

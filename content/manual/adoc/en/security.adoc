...
In Web Client, the standard login window can be customized or completely replaced in the project using the *Screens > Create login window* link in Studio. See also <<gui_web,>>.
...
Also, it is possible to link any <<list_actions,item tracking action>> with a certain constraint. The `constraintOperationType` attribute should be set for the `action` XML element.
...
If the LDAP integration is enabled, a user still needs an account in the application. All the user permissions and properties (except password) are stored in the application database, LDAP is used only for authentication. It is recommended to leave the application password empty for most users except the ones that require also the standard authentication (see below). The password field in the user editor screen is not required if the <<cuba.web.externalAuthentication,cuba.web.externalAuthentication>> property is set to `true`.

When the user logs in to the application, the first authentication attempt is made via LDAP. If it fails, and if the user login is listed in the <<cuba.web.standardAuthenticationUsersWhiteList,cuba.web.standardAuthenticationUsersWhiteList>> application property, the application tries to authenticate the user by the password hash stored in the database. As a result, a user from the white list can log in to the system with this password even if he is not registered in LDAP or has a different LDAP password.

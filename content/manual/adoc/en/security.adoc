...
You can implement your own login mechanism using custom `LoginProvider`, `HttpRequestFilter` or events described in <<web-login, Web Login Specifics>>.
...
If the `cuba.web.ldap.enabled` property is set to `true`, the `LdapLoginProvider` is used by default. In this case, the Spring LDAP library is used for user authentication.

The following Web Client application properties are used to set up LDAP integration:

* `cuba.web.ldap.enabled` - enable/disable LDAP integration.

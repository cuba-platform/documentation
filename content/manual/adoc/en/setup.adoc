...
+
--
TIP: CUBA Studio supports auto-update. The update files are saved in the `~/.haulmont/studio/update` folder in versioned subfolders, and the newest update is launched when you start Studio. On Linux, in case of any problems or if you need to start another downloaded version, you can manually remove the updates and launch Studio as usual.
--

...
+
--
Задает подверсию REST API. Значение `1` включает REST API, использовавшийся в версиях платформы до 5.4. Значение `2` включает более новую версию REST API с поддержкой вызова сервисов. Данное свойство применимо только к <<rest_api,REST API v1>>.

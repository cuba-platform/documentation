...
+
--
Включает отображение пиктограмм действий в пунктах контекстного меню <<gui_Table,Table>> и <<gui_PopupButton,PopupButton>>.
...
+
--
Задает время жизни токена <<rest_api_v2,>> в секундах для клиента по умолчанию.

Значение по умолчанию: `43200` (12 hours)
...
+
--
Разделенный запятыми список логинов пользователей, которые могут входить в систему используя стандартную аутентификацию, если <<cuba.web.externalAuthentication,внешняя аутентификация>> не удалась. Так, если пароль (на самом деле хэш) пользователя из данного списка не пустой в основной БД, и пользователь ввел верный пароль, он сможет войти в систему даже если внешняя аутентификация ему отказала.

Пустой список означает, что никто не может обойти внешнюю аутентификацию.

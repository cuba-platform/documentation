...
+
--
Задает версию <<rest_api,REST API v1>>. Значение `1` включает REST API, использовавшийся в версиях платформы до 5.4. Значение `2` включает новую версию REST API с поддержкой вызова сервисов. Данное свойство *не применимо* к <<rest_api_v2,REST API v2>>.
...
+
--
<<additive_app_properties,Аддитивное>> свойство, задающее файл, в котором перечисляются <<services,сервисы>>, доступные для вызова через <<rest_api,REST API>> приложения.

...
+
--
Определяет, как компонент главного окна переключает вкладки главного TabSheet: только скрывает их или выгружает и загружает вкладку заново при переключении.
...
Пустой список означает, что все должны использовать внешнюю аутентификацию, если она включена.

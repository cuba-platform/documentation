...
+
--
При установке в `true` включает проверку прав на атрибуты сущностей на уровне Middleware. Если значением является `false`, права на атрибуты проверяются только на клиентском уровне, т.е. в <<gui_framework,Generic UI>> и {rest_man_url}[REST API].

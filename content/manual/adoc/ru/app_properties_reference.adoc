...
Компоненты приложения могут использовать это <<additive_app_properties,аддитивное>> свойство для того, чтобы задавать шаблоны URL-адресов, которые не должны обрабатываться в <<rest_api_v2,REST API>>.

Значение свойства представляет собой разделенный запятыми список шаблонов. Список обязательно должен завершаться запятой.

Шаблон начинается с символа `"/"` и представляет собой завершающую часть пути базового отображения REST API после `/rest/v2`. Пример для обхода всех запросов `/rest/v2/idp/**` при аутентификации IDP REST:

...
+
--
Задает использование бина `HtmlSanitizer` компонентами, реализующими интерфейс `HasHtmlSanitizer`, для предотвращения cross-site scripting (XSS) в HTML-содержимом. Санитизация может быть также включена или выключена в каждом компоненте по отдельности.

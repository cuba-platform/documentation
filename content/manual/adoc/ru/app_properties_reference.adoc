...
+
--
Позволяет настроить режим фильтрации для компонента <<gui_Filter,Filter>>. Если установлено значение `true`, фильтр работает в режиме немедленного исполнения, когда каждое изменение параметров фильтра автоматически перезагружает данные. Если установлено значение `false`, фильтр работает в явном режиме. В этом случае фильтр будет применен только после нажатия кнопки **Search**. См. также атрибут фильтра <<gui_filter_immediately,applyImmediately>>.

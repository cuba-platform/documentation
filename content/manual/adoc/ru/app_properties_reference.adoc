...
+
--
Включает режим <<db_update_server,выполнения скриптов обновления БД сервером>> на старте приложения.

Начиная с CUBA 7.2 выполнением скриптов можно управлять по отдельности для главного и дополнительных хранилищ. Для главного хранилища используется свойство `cuba.automaticDatabaseUpdate_MAIN`, для дополнительных - свойства в формате `cuba.automaticDatabaseUpdate_<store_name>`. Конкретные свойства имеют приоритет над общим.
...
Значение по умолчанию: подкаталог `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/conf`, что означает расположение в подкаталоге <<app_home,домашнего каталога>> приложения.
...
Значение по умолчанию: `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/work`, что означает расположение в подкаталоге <<app_home,домашнего каталога>> приложения.
...
+
--
Определят глубину вложенности атрибутов сущности в диалоговом окне "Добавить условие". Например, если значение глубины равно 2, то для условия фильтра вы можете выбрать атрибут `contractor.city.country`, если значение 3, то `contractor.city.country.name` и т.д.
...
+
--
Определяет форму диалога, возникающего при попытке закрытия <<gui_screens,экрана>>, имеющего несохраненные изменения в <<gui_DataContext,DataContext>>.
...
+
--
Конфигурационный параметр, указывающий расположение <<log_dir,каталога журналов>> данного <<app_tiers,блока приложения>>.

Значение по умолчанию: `${app.home}/logs`, что означает расположение в подкаталоге `logs` <<app_home,домашнего каталога>> приложения.
...
+
--
Задает время истекания "remember me" cookies и экземпляров сущности `RememberMeToken`.

Значение по умолчанию: `30 * 24 * 60 * 60` (30 дней)
...
+
--
Для обеспечения обратной совместимости, включает использование конфигурации <<default-permission-values.xml>>. Подробнее см. <<legacy_roles>>.
...
+
--
Для обеспечения обратной совместимости, определяет режим работы ролей подсистемы безопасности. Подробнее см. <<legacy_roles>>.
...
+
--
Включает синхронную отправку в кластер новых пользовательских сессий после логина в систему. Детальная информация здесь: <<cluster_mw_server_sync_session_repl,Синхронная репликация пользовательских сессий>>.
...
Значение по умолчанию: `${app.home}/${<<cuba.webContextName,cuba.webContextName>>}/temp`, что означает расположение в подкаталоге <<app_home,домашнего каталога>> приложения.
...
+
--
Регулярное выражение, используемое обработчиком <<unique_constraint_violation_exceptions, UniqueConstraintViolationHandler>>, для определения того, что данное исключение произошло по причине нарушения ограничения уникальности в базе данных. Имя индекса, поддерживающего ограничение, будет взято из первой непустой группы выражения. Например:
...
+
--
При установке данного свойства в `true` блоки Web Client и Web Portal вызывают сервисы Middleware в обход сетевого стека, что положительно сказывается на производительности системы. Это возможно в случае <<fast_deployment,быстрого развертывания>>, единого <<build.gradle_buildWar,WAR>> и единого <<build.gradle_buildUberJar,Uber JAR>>. В других вариантах развертывания данное свойство необходимо установить в `false`.
...
+
--
Включает отображение <<folders_pane,панели папок>> для экрана и использование <<shortcuts,горячих клавиш>> в папках.
...
+
--
Задает использование бина `HtmlSanitizer` компонентами, реализующими интерфейс `HasHtmlSanitizer`, для предотвращения cross-site scripting (XSS) в HTML-содержимом. Санитизация может быть также включена или выключена в каждом компоненте по отдельности с помощью <<gui_attr_htmlSanitizerEnabled,htmlSanitizerEnabled>>.

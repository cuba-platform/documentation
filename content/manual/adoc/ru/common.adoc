...
`CachingFacadeMBean` предоставляет методы очистки различных кэшей в блоках http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/CachingFacadeMBean.html[Middleware] и http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/web/jmx/CachingFacadeMBean.html[Web Client].
...
`ConfigStorageMBean` позволяет просматривать и задавать значения <<app_properties,свойствам приложения>> в блоках http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/ConfigStorageMBean.html[Middleware], http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/web/jmx/ConfigStorageMBean.html[Web Client] и http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/portal/jmx/ConfigStorageMBean.html[Web Portal].
...
http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/EmailerMBean.html[EmailerMBean] позволяет просмотреть текущие значения параметров <<email_sending,отсылки email>>, а также отправить тестовое сообщение.
...
http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/PersistenceManagerMBean.html[PersistenceManagerMBean] предоставляет следующие возможности:
...
http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/ScriptingManagerMBean.html[ScriptingManagerMBean] является JMX-фасадом для интерфейса инфраструктуры <<scripting,Scripting>>.
...
http://files.cuba-platform.com/javadoc/cuba/6.8/com/haulmont/cuba/core/jmx/ServerInfoMBean.html[ServerInfoMBean] предоставляет общую информацию о данном блоке Middleware: номер и дату сборки, <<serverId,идентификатор сервера>>.
...
Данный механизм работает следующим образом (см. также <<entity_attribute_access_changes,изменения в версии 6.8.1>> ниже):
...
+
--
В связи с проблемами, найденными в случае имплементации обработчиков в виде Spring application event listeners, данный механизм был изменен в версии 6.8.1 платформы. Правила, описанные выше, по прежнему действуют по умолчанию, но в дополнение был введен альтернативный способ объявления обработчиков. Данный способ включается с помощью свойства приложения <<cuba.useSpringApplicationEventsToSetupAttributeAccess,cuba.useSpringApplicationEventsToSetupAttributeAccess>>, и будет основным и единственным способом начиная с версии 6.9. Поэтому мы рекомендуем изменить ваши обработчики в соответствии с правилами, описанными далее.

Обработчик должен быть <<managed_beans,управляемый бином>> модуля *core* проекта и реализовывать интерфейс `SetupAttributeAccessHandler`. Бин должен иметь дефолтный singleton scope. Вам необходимо реализовать методы интерфейса:

...
Файл по умолчанию расположен в пакете `com.haulmont.chile.core.datatypes` <<app_components,базового проекта>> *cuba*. Если в прикладном проекте в <<app_modules,модуле>> *global* создать аналогичный файл в корне CLASSPATH, то типы данных будут загружены из него.
...
Расположение файла `menu.xml` задается в свойстве приложения `<<cuba.menuConfig,cuba.menuConfig>>`.
...
** `description` - текст, появляющийся во всплывающей подсказке при наведении курсора мыши.

** `icon` - пиктограмма для элемента меню. См. <<gui_attr_icon,icon>>.
...
Расположение файла `permissions.xml` задается в свойстве приложения <<cuba.permissionConfig,cuba.permissionConfig>>.
...
Расположение файла `screens.xml` задается в свойстве приложения `<<cuba.windowConfig,cuba.windowConfig>>`.

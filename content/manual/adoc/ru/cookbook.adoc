...
----
+
Сущность `Airline` также по умолчанию будет отмечена как ведущая сторона отношений, так как отношения "master-detail" для аннотации <<manyToMany_annotation,@ManyToMany>> не являются обязательными.
...
===== Ассоциация Many-to-Many через дополнительную сущность

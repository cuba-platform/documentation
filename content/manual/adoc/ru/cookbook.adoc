 ...
 * {proj_business_logic}/blob/master/modules/core/com/company/sample/service/DiscountService.java[DiscountService.java] - интерфейс сервиса.
...
* Измените имя класс на `OrderEntityListener` и включите флажки для интерфейсов `BeforeInsertEntityListener`, `BeforeUpdateEntityListener` и `BeforeDeleteEntityListener`.
...
Детальная информация о методах REST API описана согласно спецификации http://swagger.io/specification[Swagger] и доступна по адресу http://files.cuba-platform.com/swagger.

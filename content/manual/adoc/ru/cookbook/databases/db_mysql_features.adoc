...
* Положите JAR-файл в каталог `Program Files/Haulmont/CUBA Studio 2018.3/plugins/cuba-studio/lib/` и в подкаталог `lib` установленного сервера Tomcat

* После этого остановите демона Gradle, выполнив в командной строке `gradle --stop`, а затем перезапустите Studio.

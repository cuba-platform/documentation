...
Здесь мы задаём системное свойство `app.home`, содержащее рабочий каталог приложения, и указываем, где находится конфигурационный файл `logback.xml`. Вместо переменной `%USERPROFILE%` можно использовать абсолютный путь.
...
. Приложение будет доступно по адресу http://localhost:8080/app. Логи записываются в рабочий каталог приложения: `C:\Users\UserName\app_home\logs`.

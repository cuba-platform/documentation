...
. Создаем файл конфигурации <<logging, Logback>>. Вы можете взять файл `build\tomcat\conf\logback.xml` после стандартного <<fast_deployment,быстрого развертывания>> в Tomcat. Замените переменную `${catalina.home}` на `${app.home}` в содержимом файла и скопируйте файл на сервер в домашний каталог приложения.
...
. Лог-файлы создаются в каталоге `c:\work\app_home\logs`.

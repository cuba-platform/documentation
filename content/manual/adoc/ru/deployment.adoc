...
Стандартное для JavaEE развертывание приложений в WAR-файлы осуществляется с помощью задачи сборки <<build.gradle_buildWar,buildWar>>. Рассмотрим пример сборки WAR-файлов и их развертывания на сервере *Glassfish 4*.

. Добавляем в конец <<build.gradle,build.gradle>> задачу сборки <<build.gradle_buildWar,buildWar>>:
+
[source, java]
----
...
+
В результате в подкаталоге `build/distributions/war` проекта создаются файлы `app-core.war` и `app.war`.

. Создаем домашний каталог приложения на сервере, например, `/home/user/app_home`.
...
. Создаем файл конфигурации <<logging, Logback>> на основе файла `build/tomcat/conf/logback.xml`, заменяя переменную ${catalina.home} на ${app.home}. Копируем файл на сервер в домашний каталог приложения.
...
* `++logback.configurationFile = file:///home/user/app_home/logback.xml++` - файл конфигурации <<logging,логгирования>>, созданнный на шаге выше.

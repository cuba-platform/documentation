...
TIP: Если сервер Tomcat ранее уже был установлен, и некоторые используемые им переменные были установлены как переменные среды, это может вызвать проблемы с запуском сервера, которые не будут отражаться в логах. В этом случае необходимо удалить все переменные среды `CATALINA_` и `CLASSPATH` и перезагрузить компьютер.
...
+
WARNING: Следует иметь в виду, что текущая реализация Polymer UI требует ручного указания тега `<base>` для текущего веб-контекста, доступного во время работы приложения. Таким образом, после изменения контекста с помощью аргумента командной строки `-frontContextName` Polymer UI может стать недоступным.
...
TIP: В примерах ниже пути к конфигурационным файлам приводятся для варианта <<fast_deployment>>.

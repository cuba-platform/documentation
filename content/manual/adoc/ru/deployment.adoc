...
. Нажмите на ссылку *Cloud deployment settings* на вкладке *Project properties*.
...
[TIP]
====
Развёртывание в облаке IBM® Bluemix® в настоящее время рекомендуется только для проектов, использующих базу данных PostgreSQL. HSQLDB доступна только с опцией _in-process_, таким образом, база данных будет пересоздаваться каждый раз при перезапуске облачного приложения, соответственно, пользовательские данные будут потеряны.
====

. Создайте учётную запись в сервисе IBM® Bluemix®. Также скачайте и установите следующее программное обеспечение:
...
. Создайте новое пространство (Space) в облаке IBM® Bluemix®, задайте ему любое имя. В дальнейшем вы можете поместить несколько приложений в одно пространство.
...
. Настройте CUBA-проект на базу данных PostgreSQL (на СУБД, аналогичную той которую Вы используете в облаке IBM® Bluemix®).
...
.. Нажмите кнопку *Generate* рядом с полем *Custom context.XML*. В появившемся диалоге укажите параметры подключения к базе данных - сервису в облаке IBM® Bluemix®.
+
...
* `memory`: CUBA -приложение потребует больше памяти, чем выделяется серверу Tomcat по умолчанию (256M). Тажке количество выделенной памяти может быть установленно через WEB-интерфейс  IBM® Bluemix®. Учтите, что количество памяти, выделенной приложению, влияет на стоимость облачного размещения.
...
. Создайте подключение к IBM® Bluemix®.
+
[source, yml]
----
...
. Зайдите в Вашу учетную запись IBM® Bluemix®.
+
[source, yml]
----
...
. По завершению процесса развёртывания CUBA-приложение будет доступно в облаке IBM® Bluemix®. Чтобы его открыть, воспользуйтесь URL `host.domain` в браузере. Этот URL будет отображаться в поле *ROUTE* таблицы ваших приложений *Cloud Foundry Apps*.

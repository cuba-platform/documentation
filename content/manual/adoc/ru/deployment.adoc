...
[WARNING]
====
Пожалуйста убедитесь, что ваше окружение на содержит переменных `CATALINA_HOME`, `CATALINA_BASE` и `CLASSPATH`. Эти переменные могут вызвать проблемы с запуском сервера, которые не будут отражаться в логах. Перезагрузите компьютер после удаления переменных.
====
...
WAR-файлы с приложением CUBA можно разворачивать на сервере *WildFly*. Рассмотрим пример сборки WAR-файлов для приложения, использующего PostgreSQL 9.6, и их развертывания на сервере WildFly версии 8.2.
...
. Подготовьте рабочий каталог приложения:
+
--
...
Здесь мы задаём системное свойство `app.home`, содержащее рабочий каталог приложения, и указываем, где находится конфигурационный файл *logback*. Вместо переменной `%USERPROFILE%` можно использовать абсолютный путь.

* Сравните версии Hibernate Validator в WildFly и приложении CUBA (платформа обычно использует наиболее свежую версию). Замените файл `C:/wildfly/modules/system/layers/base/org/hibernate/validator/main/hibernate-validator-x.y.z-sometext.jar` более новым файлом из каталога `tomcat/shared/lib`, например, *hibernate-validator-5.4.1.Final.jar*.
...
. Приложение будет доступно по адресу http://localhost:8080/app. Логи записываются в рабочий каталог приложения: `C:\Users\me\app_home\logs`.
...
UberJAR - это один из наиболее простых способов запустить приложение CUBA в режиме эксплуатации. Вы собираете единые JAR-файлы для каждого блока приложения с помощью задачи Gradle <<build.gradle_buildUberJar>> (см. также вкладку *Deployment settings > Uber JAR* в Studio) и запускаете приложение из командной строки, используя команду `java`:
...
----
+
Следует учесть, что при указании порта для блока *core* необходимо также задать свойство приложения <<cuba.connectionUrlList,cuba.connectionUrlList>>, указав соответствующие адреса и для клиентских блоков, например:
+
----

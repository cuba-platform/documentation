...
* `portalContextPath` - имя веб-контекста для модуля *portal*, запущенного из единого Uber JAR.

* `jettyConfPath` - путь к файлу конфигурации сервера Jetty, см. https://www.eclipse.org/jetty/documentation/9.4.x/jetty-xml-config.html.

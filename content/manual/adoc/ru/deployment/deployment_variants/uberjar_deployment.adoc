...
----
+
Следует учесть, что при указании порта для блока core необходимо также задать свойство приложения <<cuba.connectionUrlList,cuba.connectionUrlList>> для клиентских блоков, указав соответствующий адрес Middleware:
+
[source,plain]
----

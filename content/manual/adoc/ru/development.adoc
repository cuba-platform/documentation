...
В <<vaadin_addon_sample,предыдущем разделе>> мы подключили в проект сторонний компонент *stepper*. Рассмотрим процесс подключения данного компонента, но уже с интеграцией в универсальный UI.

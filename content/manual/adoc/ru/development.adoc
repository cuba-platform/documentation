...
TIP: Эту задачу можно настроить на вкладке *Deployment settings > Uber JAR* в Studio.
...
+
WARNING: Следует иметь в виду, что текущая реализация Polymer UI требует ручного указания тега `<base>` для текущего веб-контекста, доступного во время работы приложения. Таким образом, после изменения контекста с помощью *Module prefix*, параметра задачи `appName` или аргумента командной строки `-frontContextName` Polymer UI может стать недоступным.
...
* `appName` - имя приложения, для которого будет выполнен поиск WAR-файла. По умолчанию имя совпадает с именем модуля *web* без `-web`, например, `app`.

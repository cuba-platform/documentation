...
Платформа CUBA поддерживает технологию Hot Deploy, которая позволяет мгновенно отображать сделанные в проекте изменения в работающем приложении без необходимости перезапускать сервер. Принцип работы Hot deploy заключается во временном копировании изменённых ресурсов и классов проекта в конфигурационный каталог приложения, откуда они затем будут использоваться работающим приложением.

Как это работает::
+
--
При запуске сервера приложения Studio осуществляет сборку веб-клиента в соответствующие JAR-файлы в папке `build/tomcat/webapps/app`. Если во время работы сервера, к примеру, некий экран будет изменен в Studio или IDE, Studio создаст копию этого экрана в конфигурационном каталоге веб-клиента (по умолчанию `tomcat/conf/app`).

Ресурсы в `conf/app` dимеют приоритет над JAR-файлами приложения, поэтому работающее приложение подхватит именно эти ресурсы. Если ресурс не найден в конфигурационном каталоге, загрузчик экрана вызовет этот ресурс из JAR-файлов.

При перезагрузке сервера приложения все файлы в папке `tomcat/conf/app` удаляются, и изменения, сделанные в них, сохраняются в JAR-файлах.
--

Что можно изменять на лету::
+
--
...
Какие изменения не будут подхвачены::
+
--
...
[TIP]
====
Изменения в прочих классах и бинах UI и среднего слоя, включая их статические методы, могут применяться на лету только тогда, когда изменяется ещё хотя бы один файл экрана или реализации сервиса.

Причина такого поведения в том, что перезагрузка классов вызывается только по сигналу: для контроллера экрана это переоткрытие этого экрана, а для сервисов Studio генерирует особый файл-триггер, который будет распознан сервером и использован для перезагрузки конкретного класса сервиса и всех его зависимостей.
====
--

Настройки Hot deploy можно изменить в Studio или с помощью свойства приложения <<cuba.confDir,cuba.confDir>>:

image::hot_deploy.png[align="center"]

Мгновенное применение изменений по умолчанию доступно для всех проектов в Studio. Эту опцию можно запретить с помощью чекбокса *Instant hot deploy* в настройках Studio.

Если мгновенный hot deploy отключен, применение изменений можно вызвать вручную командой *Run > Hot deploy conf* в Studio или *CUBA > Deploy file to Tomcat* в контекстном меню <<ide_integration,плагина CUBA для IDE>>.

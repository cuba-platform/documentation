...
* `coreProject` - проект Gradle, представляющий модуль *core* (Middleware).

* `webProject` - проект Gradle, представляющий модуль *web* (Web Client).

* `appName` - имя результирующего WAR-файла. В случае раздельных WAR-файлов, для Middleware WAR добавляется суффикс `-core`.
...
* `webXml` - путь к специальному файлу `web.xml` для единого WAR, так как он отличается от файлов `web.xml`, создаваемых для <<fast_deployment, быстрого развертывания>> по умолчанию.

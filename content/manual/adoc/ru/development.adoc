...
----
+
В контроллер экрана редактирования `CustomerEdit.java` добавляем следующий код:
+
[source, java]
----
...
Последовательно рассмотрим сгенерированные студией заготовки файлов и внесем в них необходимые изменения. Откройте проект в  IDE.
...
* Класс компонента Vaadin `RatingFieldServerComponent`. Он определяет API для серверного кода, различные get/set методы для работы, слушатели событий и подключение источников данных. Прикладные разработчики всегда используют в своём коде методы этого класса.
+
[source, java]
----

...
Платформа CUBA не ограничена существующим набором визуальных компонентов. Если какого-либо компонента нет в поставке платформы, то у разработчика есть 3 пути:

. *Подключение аддона Vaadin*. Много сторонних компонентов Vaadin распространяются в виде дополнений (add-on). Библиотека аддонов находится по адресу https://vaadin.com/directory. Разработчик может подключить любой из них к проекту.

. *Подключение компонента, написанного на JavaScript*. Vaadin дает возможность создавать серверные компоненты, интегрированные с JavaScript-библиотекой. Далее разработчик на платформе просто использует серверный компонент Vaadin в коде контроллера экрана.

. *Создание собственного компонента Vaadin* с клиентской частью, написанной на GWT.

Каждый из этих способов на выходе дает нам компонент Vaadin, который мы можем инициализировать и добавить на экран модуля web (либо сделать это в соответствующем <<gui.adoc#companions,компаньоне>>). Если новый компонент используется на одном-двух экранах, то такой вариант вполне может нас устроить. Однако, если компонент используется многократно, и мы хотим иметь возможность объявлять его в XML-дескрипторе экрана, то необходимо интегрировать компонент Vaadin в универсальный пользовательский интерфейс (Generic UI) платформы.

Далее в этом разделе приводятся примеры создания новых визуальных компонентов каждым из описанных выше способов. Интеграция в Generic UI одинакова для всех трех способов, поэтому она описана только для примера с <<vaadin_addon_sample_gui,подключением аддона Vaadin>>.

[[vaadin_addon_sample_no_gui]]
===== Подключение аддона Vaadin без интеграции в Generic UI

Способ подключения сторонних компонентов Vaadin описан в секции <<gui.adoc#vaadin_addon>> руководства.
...
Для демонстрации подключения компонента создадим новый проект в CUBA Studio. Назовем его `addon-demo`.

Для подключения аддона Vaadin проект должен иметь модуль *web-toolkit*. Создадим его, нажав на кнопку *Create web-toolkit module* секции *Project properties* навигатора Studio.

Далее нажмите на кнопку *Create new UI component* - откроется окно создания визуального компонента. В секции *Component type* выберите значение `Vaadin add-on`.

image::ui_component/studio_vaadin_addon_wizard_no_gui.png[align="center"]

Рассмотрим поля, которые нам необходимо заполнить.
...
. Одной строкой в том виде, как вы добавляете зависимости в <<development.adoc#build.gradle,build.gradle>>:
+
[source, plain]
----
...
* *Inherited widgetset* - в этом поле необходимо указать имя набора виджетов подключаемого аддона. В нашем случае это `org.vaadin.risto.stepper.widgetset.StepperWidgetset`.
...
Нажимаем кнопку *OK* - аддон подключен к проекту. Давайте посмотрим, что студия сделала для нас. Откройте проект в IDE. Можете сделать это, нажав на кнопку *IDE* секции *Project properties* навигатора.

Студия изменила 2 файла:

. <<development.adoc#build.gradle, build.gradle>>. В модуле *web* появилась новая зависимость от аддона, содержащего компонент:
+
[source, java]
----
configure(webModule) {
...
. В файл `AppWidgetSet.gwt.xml` модуля *web-toolkit* проекта подключен набор виджетов add-on после набора виджетов платформы:
+
[source, xml]
----
<module>
...
* Сгенерируем для новой сущности стандартные экраны. В диалоге генерации стандартных экранов изменим значение поля *In module* на `Web Module`. Экраны, использующие компоненты Vaadin напрямую, должны располагаться в модуле *web*.
+
TIP: На самом деле экран может располагаться и в модуле *gui*, но тогда код, работающий с Vaadin компонентом, должен быть вынесен в отдельный <<gui.adoc#companions,компаньон>>.

* Далее добавим компонент `stepper` на экран. Вы можете поместить его как в `FieldGroup`, так и вне ее. Рассмотрим оба способа.
+
--
. В XML-дескрипторе экрана редактирования `customer-edit.xml` поле `score` компонента <<gui.adoc#gui_FieldGroup, fieldGroup>> делаем кастомным, добавив атрибут `custom = "true"`:
+
[source, xml]
----
...
----
+
В контроллер экрана редактирования `CustomerEdit` добавляем следующий код:
+
[source, java]
----
...
. Чтобы использовать новый компонент вне `FieldGroup` в произвольном месте экрана в XML-дескрипторе объявим контейнер и удалим поле `score` из `FieldGroup`:
+
[source, xml]
----
...
* Создаем базу данных, запускаем проект и наслаждаемся результатом.
...
В секции <<gui.adoc#own_gui_component>> руководства рассмотрены принципы интеграции "нативных" компонентов в универсальный UI для того, чтобы их можно было объявлять в XML-дескрипторах экранов и связывать с данными.

В <<vaadin_addon_sample_no_gui,предыдущем разделе>> мы подключили в проект сторонний компонент *stepper*. Рассмотрим процесс подключения данного компонента, но уже с интеграцией в универсальный UI.

Для демонстрации подключения компонента создадим новый проект в CUBA Studio. Назовем его `addon-demo-gui`. В поле *Project namespace* вводим значение `adg`.

Для подключения аддона Vaadin проект должен иметь модуль *web-toolkit*. Создадим его, нажав на кнопку *Create web-toolkit module* секции *Project properties* навигатора Studio.

Далее нажимаем на кнопку *Create new UI component* - откроется окно создания визуального компонента. В секции *Component type* выбираем значение `Vaadin add-on`.

Рассмотрим поля, которые нам необходимо заполнить.

image::ui_component/studio_vaadin_addon_wizard_gui.png[align="center"]

* *Add-on maven dependency* - в этом поле необходимо указать maven-координаты аддона Vaadin для подключения его как зависимость к текущему проекту. Указание координат возможно в двух форматах:
...
* *Integrate into generic UI* указывает на необходимости интеграции компонента в универсальный интерфейс платформы.

* *Component name in screen XML* -  имя компонента в XML-дескрипторе экрана. Введите значение `stepper`.
...
После нажатия кнопки *OK* студия сделает следующее:

* Добавит аддон Vaadin в зависимости модуля *web* в файле *build.gradle*

* Подключит набор виджетов аддона в файле `AppWidgetSet.gwt.xml` модуля *web-toolkit*

* Если выбран чек-бокс *Integrate into generic UI*, то сгенерирует заготовки для следующих файлов:

** `Stepper` - интерфейс универсального компонента в модуле *gui*
...
Откройте проект в IDE. Можете сделать это, нажав на кнопку *IDE* секции *Project properties* навигатора.

Последовательно пройдемся по сгенерированным студией заготовкам файлов и внесем в них необходимые изменения.
...
----
+
В качестве базового класса выбран `WebAbstractField`, который реализует логику интерфейса `Field` по связыванию с <<gui.adoc#datasources,источником данных>> и другие его методы.

* `StepperLoader` в модуле *gui* - это XML-загрузчик компонента.
+
[source, java]
----
...
* Удалите поле `stepper` из `FieldGroup`.
...
. Использование компонента в кастомном поле <<gui.adoc#gui_FieldGroup>>:
+
[source, xml]
----
...
В данном примере мы подключим компонент https://jqueryui.com/slider/#range[slider] из библиотеки *jquery-ui*. Слайдер будет иметь 2 ползунка, определяющих диапазон значений.

Для того, чтобы использовать JavaScript компонент в приложении мы должны выполнить следующее:

. В модуле *web* создать серверный компонент Vaadin. Серверный компонент определяет API для серверного кода, различные get/set методы, слушатели событий и т.п. Компонент должен быть унаследован от `AbstractJavaScriptComponent`. Обратите внимание, что модуль *web-toolkit* и компиляция виджетсета при интеграции в проект JavaScript компонентов не требуются.

. Создать JavaScript коннектор. Коннектор будет содержать функцию, инициализирующую JavaScript компонент и отвечающую за взаимодействие между кодом JavaScript и сервером.

. Создать класс состояния. Он определяет, какие данные будут пересылаться между сервером и клиентом. В нём описываются публичные поля, которые будут автоматически сериализованы на сервере и десериализованы на клиенте. Класс состояния должен быть унаследован от `JavaScriptComponentState`.

Для демонстрации подключения компонента JavaScript создадим новый проект в CUBA Studio. Назовите его `jscomponent`.

Далее нажмите на кнопку *Create new UI component* - откроется окно создания визуального компонента. В секции *Component type* выберите значение `JavaScript component`.

TIP: Обратите внимание, что для подключения JavaScript компонента наличие модуля *web-toolkit* необязательно.

image::ui_component/studio_js_component_wizard.png[align="center"]

В поле *Vaadin component class name* диалога генерации компонента введите значение `JsSlider`.
...
После нажатия кнопки *OK* студия сгенерирует файлы:
...
Последовательно пройдемся по сгенерированным студией заготовкам файлов и внесем в них необходимые изменения. Откройте проект в IDE.
...
----
+
Серверный компонент Vaadin определяет набор геттеров и сеттеров для работы с состоянием слайдера, а также интерфейс слушателя изменения значений. Класс компонента должен наследовать `AbstractJavaScriptComponent`.
+
Метод `addFunction` в конструкторе класса объявляет обработчик RPC-вызова функции `valueChanged` с клиента.
+
Аннотации @JavaScript и @StyleSheet указывают на файлы, которые должны быть загружены на веб-страницу. В нашем примере это JS файлы библиотеки jquery-ui и коннектора, а также файл со стилями для jquery-ui. Расположим их в одном пакете с серверным компонентом.
...
NOTE: На самом деле экран может располагаться и в модуле *gui*, но тогда код, работающий с Vaadin компонентом, должен быть вынесен в отдельный <<gui.adoc#companions,компаньон>>.
...
Перейдите к файлу `product-edit.xml`. Поля `minDiscount` и `maxDiscount` сделайте нередактируемыми, добавив к соответствующим тегам атрибут `editable="false"`. Затем добавьте в `fieldGroup` новое кастомное поле `slider`.
...
Запустите проект и откройте экран редактирования продукта. Изменение положения ползунка на слайдере должно изменять значение в соответствующем поле fieldGroup.
...
Для возможности создания собственного GWT-компонента проект должен иметь модуль *web-toolkit*. Создадим его, нажав на кнопку *Create web-toolkit module* секции *Project properties* навигатора Studio.

Далее нажмите на кнопку *Create new UI component* - откроется окно создания визуального компонента. В секции *Component type* выберите значение `New GWT component`.
...
* Класс компонента Vaadin `RatingField`. Он определяет API для серверного кода, различные get/set методы для работы, слушатели событий и подключение источников данных. Прикладные разработчики всегда используют в своём коде методы этого класса.
+
[source, java]
----
...
В коде виджета `RatingFieldWidget` нет упоминаний внешнего вида, кроме назначения имён стилей ключевым элементам. Для того, чтобы определить внешний вид нашего компонента, сперва, создадим файлы стилей. Для этого можем воспользоваться действием *Create theme extension* секции *Project properties* в навигаторе студии. В появившемся диалоге выбираем тему `halo`. Эта тема использует вместо иконок глифы шрифта FontAwesome, чем мы и воспользуемся. Studio создаст пустые файлы SCSS для расширения темы в каталоге `themes` модуля *web*.

Стили каждого компонента принято выделять в отдельный файл `componentname.scss` в каталоге `components/componentname` в формате примеси SCSS. В каталоге `themes/halo` создадим пару вложенных каталогов: `components/ratingfield`. Затем внутри `ratingfield` создадим файл `ratingfield.scss`:
...
Добавим экран в меню приложения. Перейдите в секцию *Main menu* навигатора студии и нажмите кнопку *Edit*. Откроется редактор меню. Добавьте созданный экран в меню `application`.
...
[[ui_component_studio_support]]
===== Поддержка новых компонентов в CUBA Studio

В данном разделе описывается, как добавить поддержку новых визуальных компонентов в дизайнер экранов CUBA Studio. Новый компонент появится в палитре компонентов, вы сможете перетаскивать его на экран и редактировать его свойства в панели свойств компонента.

Информацию о новых визуальных компонентов CUBA Studio получает из плагина, который седержит реализацию расширения `CustomComponent`.

Рассмотрим процесс создания плагина, добавляющего в студию поддержку компонента `stepper`, создание которого было описано в <<vaadin_addon_sample_gui>>.

Откроем проект, содержащий компонент `stepper`.

TIP: Если указанный проект вы не создавали, ничего страшного. Можете воспроизвести шаги, описанные ниже, и на новом проекте. Поддержку компонента студией вы увидите, не сможете только запустить приложение.

На панели *Project properties* нажмем кнопку *Generate Studio plugin project*.

image::ui_component/component_plugin_generation_window.png[align="center"]

Рассмотрим поля, которые необходимо заполнить.

* *Plugin id* - идентификатор плагина. Введем значение `stepper`

* *Plugin path* - каталог, в котором будет создан проект плагина

* *Deploy plugin* - если чек-бокс выбран, то плагин будет автоматически скопирован в папку `plugins` студии после создания. Оставим ее выделенной.

* *Generate base project extension* - снимите флаг, т.к. нам не нужна поддержка нового базового проекта.

* *Generate UI component extension* - этот чек-бокс должен быть выбран, т.к. мы хотим, чтобы плагин добавлял поддержку нового визуального компонента. После выделения этого чек-бокса, на экране появится область для заполнения свойств нового визуального компонента.

* *Component name in screen XML* - имя компонента в том виде, как он должен быть добавлен в XML-дескриптор экрана. В нашем случае это `stepper`.
+
...
* *Component namespace URI* - пространство имен из XSD, описывающего компонент. Если вы генерировали новый компонент с помощью CUBA Studio, то посмотреть значение этого поля вы можете в файле `ui-component.xsd`.

* *Component namespace prefix* - префикс, с которым тег компонента должен быть добавлен в XML-дескриптор экрана.

* *Standard properties* - в секции выбираются стандартные свойства компонента, которые должны быть доступны для редактирования в панели свойств компонента в дизайнере экранов студии.
+
Выделите `caption`, `datasource` и `property`.
+
TIP: Свойства `id`, `align`, `height`, `width`, `enable`, `stylename`, `visible` по умолчанию подключены ко всем компонентам.

* *Custom properties* - в таблице добавляются специфичные для компонента свойства, которые должны редактироваться в панели свойств дизайнера экранов студии.
+
--
...
Новые визуальные компоненты инициализируются при старте сервера студии. Откройте окно сервера студии, остановите сервер и запустите его снова.
...
Для начала удалим поле `stepper` из `fieldGroup`, т.к. мы хотим для его редактирования использовать специальный компонент. Перейдите на закладку *XML* и удалите строку `<field id="score">`.

Вернитесь обратно на закладку дизайнера *Layout*.
...
Выделяем компонент `stepper` и переходим на закладку свойств компонента *Properties*.
...
После этого переходим на закладку *XML*, чтобы увидеть результат.
...
Видим, что в XML экрана объявлено пространство имен компонента с префиксом `app`, компонент `stepper` добавлен на экран, и у него установлены необходимые свойства.

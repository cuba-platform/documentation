...
* `singleJar` - параметр, позволяющий создать единый Uber JAR из всех модулей проекта (core, web, portal, polymer). По умолчанию: `false`.

* `coreJettyConfPath` - относительный путь к файлу, который будет использован в качестве файла конфигурации сервера Jetty для Core Uber JAR (если `singleJar` = `false`), см. https://www.eclipse.org/jetty/documentation/9.4.x/jetty-xml-config.html.

* `webJettyConfPath` - относительный путь к файлу, который будет использован в качестве файла конфигурации сервера Jetty для Web Uber JAR (если `singleJar` = `false`). Если `singleJar` = `true`, параметр `webJettyConfPath` будет использован для конфигурации Jetty Server для единого Uber JAR.

* `portalJettyConfPath` - относительный путь к файлу, который будет использован в качестве файла конфигурации сервера Jetty для Portal Uber JAR (если `singleJar` = `false`).
...
* `logbackConfigurationFile` - параметр, содержащий относительный путь к файлу, в котором содержится конфигурация логирования.

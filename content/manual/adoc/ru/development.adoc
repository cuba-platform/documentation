...
Далее нажмите на ссылку *Create new UI component*. Откроется окно создания визуального компонента *New UI component*. В секции *Component type* выберите значение `Vaadin add-on`.
...
. Закройте ппроект `customers` и откройте `sales`.
...
. Откройте `build.gradle` проекта `customers` в текстовом редакторе. Замените репозиторий и им/пароль пользователя в секции `buildscript/repositories` и добавьте `uploadRepository` в секции `cuba`:
+
[source,groovy]
----
...
. В окне сервера Studio укажите ваш репозиторий и имя/пароль для доступа к нему вместо стандартного репозитория CUBA. Запустите сервре Studio.

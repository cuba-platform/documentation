...
Рассмотрим процесс создания расширения студии, добавляющего в студию поддержку компонента `stepper`, создание которого было описано в <<vaadin_addon_sample_gui>>.
...
На панели *Project properties* нажмем кнопку *Extend Studio*.
...
Для начала удалим поле `score` из `fieldGroup`, т.к. мы хотим для его редактирования использовать специальный компонент. Перейдите на закладку *XML* и удалите строку `<field id="score">`.

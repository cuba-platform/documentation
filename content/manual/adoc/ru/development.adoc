...
* Для Java и Groovy кода рекомендуется придерживаться стандартного стиля, описанного в документе link:$$http://www.oracle.com/technetwork/java/codeconventions-150003.pdf$$[Java Code Conventions]. При программировании в *IntelliJ IDEA* для этого достаточно использовать стиль по умолчанию, а для переформатирования применять сочетание клавиш *Ctrl-Alt-L*.
+
...
[WARNING]
====
Если ваш проект использует премиум-дополнения, и вы запускаете сборку вне Studio, вы можете передать имя и пароль к репозиторию дополнений в аргументах `-P` командной строки, например:
...
Другой вариант: чтобы использовать задачи Gradle из коммандной строки, не передавая дополнительных параметров, необходимо создать файл `~/.gradle/gradle.properties` со следующим содержимым:

[source]
----
premiumRepoUser=<license-user>
premiumRepoPass=<license-pwd>
----

, где `premiumRepoUser` - первая часть лицензионного ключа до тире, а `premiumRepoPass` - вторая часть после тире, см. раздел <<access_to_repo,Доступ к премиум-дополнениям CUBA>>.

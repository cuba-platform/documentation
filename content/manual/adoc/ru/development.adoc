...
* Затем получим экземпляр `FileDescriptor` с изображением, которое хранится в <<file_storage,File Storage>>. Ссылка на это изображение хранится в атрибуте `imageFile` сущности `Employee`. Используем тип ресурса `FileDescriptorResource`, чтобы задать источник данных для компонента `Image`.

...
[TIP]
====
Начиная с версии 7.0, дополнения BPM, Charts, Full-Text Search и Reposrts распространяются свободно. Они располагаются в основном репозитории, описанном выше, поэтому конфигурировать доступ к премиум-репозиторию нужно только для использования других премиум-дополнений, например WebDAV.
====
...
* Рекомендуемый способ - создать файл `~/.gradle/gradle.properties` и указать параметры в нем:

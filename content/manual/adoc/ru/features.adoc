...
* <<fileLoader_recipe,Fileloader>> - интерфейс для загрузки и хранения файлов, который можно испольщовать как на уровне Middleware, так и на клиентском уровне. Основные методы:
+
--
** `saveStream()` – сохранить содержимое файла, переданное в `InputStream`, в промежуточное хранилище на *Middleware*.

** `openStream()` – вернуть `InputStream` с содержимым файла, сохранённого на *Middleware*.

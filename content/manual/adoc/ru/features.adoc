...
[source,properties]
----
cuba.rest.queriesConfig = +com/company/myapp/myapp-rest-queries.xml
----

Файл `myapp-rest-queries.xml` должен находиться в главном пакете модуля *web* или *portal* (например, `com.company.myapp`) и его содержимое должно иметь вид:
...
[source,properties]
----
cuba.rest.servicesConfig = +com/company/myapp/myapp-rest-services.xml
----

Файл `myapp-rest-services.xml` должен находиться в главном пакете модуля *web* или *portal* (например, `com.company.myapp`) и его содержимое должно иметь вид:

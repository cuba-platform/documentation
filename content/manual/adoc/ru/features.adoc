...
Контроллеры REST API регистрируются в контексте Spring, определяемом файлом `<<dispatcher-spring.xml,portal-dispather-spring.xml>>` соответствующего модуля (`portal-dispatcher-spring.xml` or `web-dispatcher-spring.xml`):
...
Здесь мы предполагаем, что вы используете REST API, включенный в модуль *portal*, поэтому все запросы имеют URL, начинающийся с `{host:port}/app-portal/api`. Если вы используете REST API в модуле *web*, URL должен начинаться с `{host:port}/app/dispatch/api`.
...
Для получения объекта необходимо выполнить GET запрос `{host:port}/app-portal/api/find.<format>` с параметрами:
...
Семантика полей XML-документа определяется в схеме {xsd_url}/restapi-commit-v2.xsd.
...
Схема, содержащая описание результата вызова функции, находится по адресу {xsd_url}/restapi-instance-v2.xsd.
--

[[rest_upload_file]]
====== Загрузка файла в хранилища

Для загрузки файла в <<file_storage,FileStorage>> необходимо выполнить POST запрос `{host:port}/app-portal/api/upload` с параметрами:
...
Файл отсылается в теле запроса.

[[rest_download_file]]
====== Выгрузка файла из хранилища

Для выгрузки файла из <<file_storage,FileStorage>> необходимо выполнить GET запрос `{host:port}/app-portal/api/download` с параметрами:
...
XSD запроса доступна по адресу {xsd_url}/restapi-service-v2.xsd.
--
...
XSD результата доступна по адресу {xsd_url}/restapi-service-v2.xsd.
--
...
Версия 2 универсального REST API предоставляет следующую функциональность:
...
REST API Version 2 использует протокол OAuth2 для аутентификации и поддерживает анонимный доступ.
...
В приложении на CUBA предопределенные JPQL запросы должны быть объявлены в файлах, определенных свойством приложения <<cuba.rest.queriesConfig, cuba.rest.queriesConfig>>, например:

[source,properties]
----
cuba.rest.queriesConfig = +myapp-rest-queries.xml
----

Содержимое файла `myapp-rest-queries.xml` должно иметь вид:
...
Список методов сервисов, доступных для вызова через REST API, должен быть объявлен в приложении в конфигурационных файлах, заданных свойством приложения <<cuba.rest.servicesConfig, cuba.rest.servicesConfig>>, например:

[source,properties]
----
cuba.rest.servicesConfig = +myapp-rest-services.xml
----

Содержимое файла `myapp-rest-services.xml` должно иметь вид:
...
Перед выполнением запроса с помощью REST API необходимо описать его в <<rest_api_v2_queries_config, конфигурационном файле>>. В главном пакете модуля *web* (например, `com.company.sales`) необходимо создать файл `sales-rest-queries.xml`. Затем этот файл объявляется в файле свойств приложения модуля *web* (web-app.properties):

[source, properties]
----
cuba.rest.queriesConfig = +com/company/sales/sales-rest-queries.xml
----

Содержимое файла `sales-rest-queries.xml`:
...
Перед выполнением метода сервиса с помощью REST API необходимо разрешить его вызов в <<rest_api_v2_services_config, конфигурационном файле>>. В главном пакете модуля *web* (например, `com.company.sales`) необходимо создать файл `sales-rest-services.xml`. Затем этот файл объявляется в файле свойств приложения модуля *web* (web-app.properties):
...
Содержимое файла `sales-rest-services.xml`:
...
Перед вызовом данного метода с помощью REST API необходимо разрешить его, добавив запись в конфигурационный файл `sales-rest-services.xml` (его создание было рассмотрено в <<rest_api_v2_ex_service_get>>):
...
Платформа CUBA позволяет добавлять в приложение HTML-страницы и Javascript с использованием REST API. Такие страницы могут быть помещены и в другую папку проекта, но для простоты мы рекомендуем создать в модуле *web* папку `VAADIN` (`modules/web/web/VAADIN`), так как из этой папки статические ресурсы загружаются автоматически. Скачайте и поместите в папку `VAADIN` https://code.jquery.com/jquery-3.1.1.min.js[библиотеку jQuery], чтобы указать ссылку на неё в дескрипторе экрана. Для использования стилей можно использовать http://getbootstrap.com/getting-started/#download[Bootstrap], поместив его `.css`-файлы (в примере ниже это `bootstrap.min.css`) в ту же папку проекта.

Ниже представлен пример простого экрана, содержащего окно аутентификации и экран просмотра для сущности Customer, отображающий список покупателей.
...
Экран создан в модуле *web* в файле `customers.html` и доступен в приложении по относительному пути, добавленному к URL приложения:

image::rest_js_1.png[align="center"]

Блок просмотра покупателей скрыт до авторизации. Когда вход в приложение выполнен, форму входа заменяет сообщение *Logged in successfully*, и отображается список покупателей.

В этом примере используется стандартная <<rest_api_v2_ex_get_token,аутентификация через OAuth токен>>, сам токен хранится в простой переменной `oauthToken`.

Имя пользователя и пароль из полей ввода передаётся на сервер POST-запросом. В случае успешной аутентификации, страница получает с сервера значение токена, скрывается блок `loginForm` `<div>` и отображается блок `loggedInStatus`.
...
Чтобы отобразить список покупателей, на сервер отправляется <<rest_api_v2_ex_get_entities_list,запрос на получение списка экземпляров сущности>> `sales$Customer`, передавая значение `oauthToken` в качестве параметра.

Если запрос выполнен, блок `customers` отображается на экране, и маркированный список `customersList` заполняется строками, содержащими значения локальных полей сущности Customer: name и email.
...
[WARNING]
====
Бин отображается в списке и доступен для выбора, только если он объявлен в модуле *core* и у него есть интерфейс, содержащий подходящие для вызова из задания методы. <<services,Сервисы>> и бины без интерфейса не поддерживаются.
====
...
Для всех типов значения, кроме `Boolean`, также доступно поле *Width*, позволяющее задать ширину поля в `FieldGroup` в пикселах или процентах. Если поле *Width* не заполнено, его значение по умолчанию равно 100%. Если выбран тип значения `Enumeration`, множество значений перечисления задаётся в поле *Enumeration* через запятую.
...
В таблице сравнения отображаются имена атрибутов и их новые значения, при выборе строки показывается детальная информация по изменениям атрибута в двух снимках. Ссылочные поля выводятся в соответствии с их шаблоном <<namePattern_annotation,
          @NamePattern
...
* Для Microsoft SQL Server версии ниже 2012 каждой последовательности соответствует таблица `++SEC_UN_{domain}++` с полем IDENTITY.

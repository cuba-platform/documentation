...
Атрибут сущнщости переименован::
+
--
...
Видим, что ответ содержит атрибуты `oldNumber` и `date`, хотя сущность в последней версии приложения уже имеет переименованне атрибуты `newNumber` и `deliveryDate`.
--
...
+
--
Вы также можете создать и зарегистрировать свой собственный трасформер JSON. В качестве примера рассмотрим следующую ситуацию.
...
Так будет выглядеть конфиг трансформации для данного случая:
...
Конфиг содержит элемент `custom` с вложенными элементами `toVersion` и `fromVersion`. Эти элементы содержат ссылки на бины, т.е. кастомный трансформер должен быть зарегистрирован как Spring bean.
...
Еще пара слов о кастомных трансформерах. Они должны реализовывать интерфейс `EntityJsonTransformer`. Вы также можете отнаследоваться от класса `AbstractEntityJsonTransformer` и переопрделить его метод `doCustomTransformations`. Класс `AbstractEntityJsonTransformer` содержит функциональность стандартных трансформеров.
--
...
Для того чтобы предоставить <<securityContext,SecurityContext>> коду, выполняемому задачами Spring на среднем слое, используйте <<system_authentication, системную аутентификация>>.
...
image::EmailScheduler.png[align="center"]

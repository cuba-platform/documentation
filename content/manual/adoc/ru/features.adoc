...
В платформе CUBA имеется готовый запрос `all` для получения списка сущностей через REST API. Чтобы получить количество экземпляров сущности, используйте `/all/count/` в *GET*-запросе:

  http://localhost:8080/app/rest/v2/queries/sales$Order/all/count

Вы также можете сохранять свои запросы в приложении на CUBA. Предопределённые JPQL запросы должны быть объявлены в файлах, определённых свойством приложения <<cuba.rest.queriesConfig, cuba.rest.queriesConfig>>. Свойство должно быть определено в модуле *web* или *portal* (например, в файле `web-app.properties`):
...
<bean id="sqlSessionFactory" class="org.mybatis.spring.SqlSessionFactoryBean">
  <property name="cubaDataSource" ref="cubaDataSource"/>

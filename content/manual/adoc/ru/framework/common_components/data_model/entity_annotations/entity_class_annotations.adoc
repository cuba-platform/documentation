...
[TIP]
====
В руководстве https://www.cuba-platform.com/guides/initial-entity-values[Initial Entity Values] приводится пример определения начального значения непосредственно в классе сущностей с помощью аннотации `@PostConstruct`.
====

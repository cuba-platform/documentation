...
Интерфейсы инфраструктуры реализуются бинами Spring Framework, поэтому они могут быть инжектированы в любые другие управляемые компоненты (<<managed_beans,бины>>, <<services,сервисы среднего слоя>>, <<screen_controller,контроллеры>> экранов универсального пользовательского интерфейса).

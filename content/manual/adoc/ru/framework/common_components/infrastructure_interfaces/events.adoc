...
[TIP]
====
Руководство https://www.cuba-platform.com/guides/decouple-business-logic-with-app-events[Decouple Business Logic with Application Events] содержит примеры использования событий в приложениях.
====
...
Примером такого события является `UIRefreshEvent` - событие, которое запускается при каждом обновлении веб-страницы. `UIRefreshEvent` может быть обработано только в контроллерах экрана активного `AppUI`. Например:

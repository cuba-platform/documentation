...
Действие реализовано классом `com.haulmont.cuba.gui.actions.list.BulkEditAction` и объявляется в XML с помощью атрибута `type="bulkEdit"`. Общие свойства действий можно конфигурировать с помощью атрибутов элемента `action` element, подробнее см. раздел <<declarative_actions>>. Ниже рассматриваются параметры, специфичные для класса `BulkEditAction`.

...
`EditorScreenFacet` – это невизуальный компонент, который обеспечивает возможность предварительной настройки <<opening_screens,экрана редактирования>>. Декларативное описание экрана редактирования заменяет существующий `ScreenBuilders#editor(java.lang.Class<E>, FrameOwner)`. `EditorScreenFacet` определяется в элементе `facets` XML-дескриптора экрана.
...
Экран редактирования может быть показан явно с помощью соответствующего метода:

...
`InputDialogFacet` – это <<gui_facets,фасет>>, который обеспечивает возможность предварительной настройки <<gui_input_dialog,Input Dialog>>. Декларативное описание диалога ввода заменяет существующий метод `Dialogs.createInputDialog()`. `InputDialogFacet` определяется в элементе `facets` XML-дескриптора экрана.

...
Диалоговое окно может быть показано явно с помощью соответствующего метода:
...
или может быть подписано на любое действие (см. атрибут <<gui_NotificationFacet_onAction,onAction>>) или кнопку (см. атрибут <<gui_NotificationFacet_onButton,onButton>>) по идентификатору:

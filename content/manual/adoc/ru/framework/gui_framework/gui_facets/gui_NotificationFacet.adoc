...
++++
<div class="manual-live-demo-container">
    <a href="https://demo.cuba-platform.com/sampler/open?screen=notification-facet" class="live-demo-btn" target="_blank">ДЕМОНСТРАЦИЯ</a>
</div>
++++

`NotificationFacet` – это <<gui_facets,фасет>>, который обеспечивает возможность предварительной настройки <<gui_notifications,уведомлений>>. Декларативное описание уведомления заменяет существующий метод `Notifications.create()`. `NotificationFacet` определяется в элементе `facets` XML-дескриптора экрана.

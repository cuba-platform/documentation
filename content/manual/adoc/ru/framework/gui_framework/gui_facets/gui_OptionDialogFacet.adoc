...
`OptionDialogFacet` – это <<gui_facets,фасет>>, который обеспечивает возможность предварительной настройки <<gui_option_dialog,Option Dialog>>. Декларативное описание диалога выбора заменяет существующий метод `Dialogs.createOptionDialog()`. `OptionDialogFacet` определяется в элементе `facets` XML-дескриптора экрана.

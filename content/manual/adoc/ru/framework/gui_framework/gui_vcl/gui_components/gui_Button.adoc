...
[TIP]
====
Если для `Button` установлен экземпляр `Action`, то кнопка возьмет из него следующие свои свойства: <<gui_attr_caption,caption>>, <<gui_attr_description,description>>, <<gui_attr_icon,icon>>, <<gui_attr_enable,enable>>, <<gui_Button_shortcut,shortcut>>, <<gui_attr_visible,visible>>. Свойства `caption`, `description` и `icon` будут проставлены из действия только в том случае, если они не установлены в самом `Button`. Остальные перечисленные свойства действия имеют безусловный приоритет над свойствами кнопки.
...
[[gui_Button_shortcut]]
С помощью атрибута `shortcut` можно задать комбинацию клавиш для кнопки в виде строкового представления. Возможные модификаторы: `ALT`, `CTRL`, `SHIFT` − отделяются символом "-". Например:

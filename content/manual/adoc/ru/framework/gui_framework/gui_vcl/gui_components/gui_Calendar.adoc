...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.1/com/haulmont/cuba/gui/components/Calendar.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
`ContainerCalendarEventProvider` получает данные напрямую из атрибутов сущности. Чтобы `ContainerCalendarEventProvider` мог использовать сущность, она должна иметь *как минимум* следующие атрибуты: дата начала события,  дата окончания события (один из типов <<gui_Calendar_datatype,datatype>>) и заголовок события (тип String).

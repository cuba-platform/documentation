...
+
--
<<gui_DataGrid_elements,Элементы dataGrid>>

<<gui_DataGrid_attributes,Атрибуты dataGrid>>

<<gui_DataGrid_methods,Методы интерфейса DataGrid>>

<<gui_DataGrid_DescriptionProviders,Использование всплывающих подсказок>>

<<gui_DataGrid_setDetailsGenerator,Использование интерфейса DetailsGenerator>>

<<gui_DataGrid_inline_editor,Использование режима внутристрочного редактирования>>

<<gui_DataGrid_addGeneratedColumn,Использование интерфейса ColumnGenerator>>

<<gui_DataGrid_setRenderer,Использование рендереров>>

<<gui_DataGrid_header,Header и Footer>>

<<gui_DataGrid_styles,Стили DataGrid>>
--
...
* `columns` - элемент, определяющий набор колонок `DataGrid`. Если не задан, то столбцы таблицы будут автоматически определены из атрибутов <<views,представления>>, используемого при загрузке данных из <<gui_data_containers,контейнера данных>>. Элемент `columns` имеет следующие атрибуты:
+
--
...
Для агрегируемых колонок необходимо указать элемент `aggregation` с атрибутом `type`, задающим функцию агрегации.
По умолчанию в агрегируемых колонках поддерживаются только числовые типы данных, такие как `Integer`, `Double`, `Long` и `BigDecimal`. Агрегированные значения столбцов выводятся в дополнительной строке вверху `DataGrid`. Агрегирование в компоненте `DataGrid` функционирует также, как и в компоненте <<gui_Table, Table>>. Это означает, что вы можете использовать <<gui_Table_column_aggregation_strategyClass,strategyClass>>, <<gui_Table_column_aggregation_valueDescription,valueDescription>> и <<gui_Table_column_aggregation_formatter,formatter>> для агрегации.
...
* `editorCrossFieldValidate` - включает перекрестную проверку полей во <<gui_DataGrid_inline_editor,встроенном редакторе>>. По умолчанию имеет значение `true`.

[[gui_DataGrid_editorEnabled]]
* `editorEnabled` - включает отображение UI для внутристрочного редактирования ячеек. Если `dataGrid` привязан к источнику данных с типом <<gui_keyvalue_containers,KeyValueCollectionContainer>>, предполагается, что он используется только для чтения, и использование атрибута `editorEnabled` в этом случае бессмысленно.
...
+
** `HtmlRenderer`
** <<gui_DataGrid_setRowDescriptionProvider,RowDescriptionProvider>> с режимом `ContentMode.HTML`
** <<gui_DataGrid_setDescriptionProvider,DescriptionProvider>> с режимом `ContentMode.HTML`
+
...
* Метод `getAggregationResults()` возвращает мэп с результатами <<gui_DataGrid_aggregatable,агрегации>>, где ключи в мэп − идентификаторы столбцов `DataGrid`, а значения − значения агрегации.
...
<1> – Смотрите код метода `createCloseButton` целиком в классе https://demo.cuba-platform.com/sampler/open?screen=datagrid-detailsgenerator[DataGridDetailsGeneratorSample].
<2> – Смотрите код метода `getContent` целиком в классе https://demo.cuba-platform.com/sampler/open?screen=datagrid-detailsgenerator[DataGridDetailsGeneratorSample].
...
Встроенный редактор `DataGrid` может учитывать <<bean_validation_custom_constraints,ограничения сущностей>> (перекрестная проверка полей). Если есть ошибки валидации, `DataGrid` покажет сообщение об ошибке. Чтобы включить/отключить проверку или получить текущее состояние, используйте следующие методы:
...
----
+
`ColumnGeneratorEvent` хранит информацио о сущности, которая отображается в текущей строке `DataGrid`, и идентификатор колонки.
...
+
`ButtonRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания кликов по кнопке.
Studio сгенерирует код объявления `ButtonRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
+
`ClickableTextRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания нажатий на ссылку.
Studio сгенерирует код объявления `ClickableTextRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
+
`ImageRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания нажатий на изображение.
Studio сгенерирует код объявления `ImageRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
Внешний вид компонента `DataGrid` можно настроить с помощью переменных SCSS с префиксом `$cuba-datagrid-*`. Эти переменные можно изменить в визуальном редакторе после <<web_theme_extension,расширения темы>> или создания <<web_theme_creation,новой темы>>.

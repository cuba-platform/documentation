...
Если вам не требуется заполнить колонки данными на момент загрузки XML, используйте атрибут <<gui_attr_metaClass,metaClass>> вместо указания источника данных.
...
+
`ButtonRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания кликов по кнопке. Cuba Studio сгенерирует код объявления `ButtonRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
+
`ClickableTextRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания нажатий на ссылку. Cuba Studio сгенерирует код объявления `ClickableTextRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
+
`ImageRenderer` не может быть объявлен в XML-дескрипторе, так как в нем невозможно определить слушатель отслеживания нажатий на изображение. Cuba Studio сгенерирует код объявления `ImageRenderer` в методе `init()` контроллера экрана:
+
[source, java]
----
...
К компоненту `DataGrig` можно применить заданные стили. Стиль задается в XML-дескрипторе с помощью атрибута `stylename`.

...
** `includeAll` – загружает все атрибуты <<views,представления>>, используемого при загрузке данных из <<gui_data_containers,контейнера данных>> или <<datasources,источника данных>>.
+
...
В веб-клиенте к компоненту `DataGrig` можно применить заданные стили. Стиль задается в XML-дескрипторе с помощью атрибута `stylename`.

...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.1/com/haulmont/cuba/gui/components/DataGrid.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
----
+
Если представление сущности содержит ссылочный атрибут, связанная сущность загружается как свойство в соответствии с ее представлением. Это означает, что если вы хотите показать поле `address.street`, вы должны определить его вручную:
+
[source, xml]
----

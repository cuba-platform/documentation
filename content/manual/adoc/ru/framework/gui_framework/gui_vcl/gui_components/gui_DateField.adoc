...
* Для создания поля даты, связанного с данными, необходимо использовать атрибуты <<gui_attr_dataContainer,dataContainer>>/<<gui_attr_datasource,datasource>> и <<gui_attr_property,property>>:
+
[source, xml]
----

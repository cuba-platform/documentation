...
* В контроллере экрана инжектируем сам компонент, а также интерфейсы <<file_upload,FileUploadingAPI>> и <<dataManager,DataManager>>. Затем в событии `InitEvent` добавляем слушатели, которые будут реагировать на события успешной загрузки или ошибки:
+
[source, java]
----

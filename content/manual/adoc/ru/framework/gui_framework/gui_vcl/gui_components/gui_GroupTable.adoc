...
Для `GroupTable` должен быть указан <<gui_collection_container,data container>> с типом `CollectionContainer`. В противном случае группировка работать не будет.
...
+
`GroupStyleProvider` имеет специальный метод для стилизации сгрупированных строк с <<gui_GroupTable_groupInfo,GroupInfo>> в качестве принимаего параметра. Он будет вызываться для каждой сгруппированной строки в `GroupTable`.
+
...
----
+
Далее нужно определить стили в теме приложения. Подробная информация о создании темы находится в <<gui_themes,Создание темы приложения>>. Для веб-клиента новые стили определяются в файле `styles.scss`. Имена стилей, заданные в контроллере, образуют CSS-селекторы. Например:
+
[source, css]
----

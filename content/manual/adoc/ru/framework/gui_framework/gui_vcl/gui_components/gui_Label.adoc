...
Атрибут `htmlSanitizerEnabled` разрешает или запрещает санитизацию HTML. Если атрибуты <<gui_Label_htmlEnabled,htmlEnabled>> и `htmlSanitizerEnabled` установлены в `true`, то значение атрибута <<gui_Label_value,value>> будет санитизировано.

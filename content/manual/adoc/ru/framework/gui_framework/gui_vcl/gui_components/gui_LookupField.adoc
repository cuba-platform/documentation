...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.0/com/haulmont/cuba/gui/components/LookupField.html" class="api-docs-btn" target="_blank">API DOCS</a>
</div>
++++
...
----
+
В данном случае компонент отобразит отобразит <<namePattern_annotation,имена экземпляров>> сущности `Color`, находящихся в коллекции данных `colorsDs`, а выбранное значение подставится в атрибут `color` сущности `Car`, находящейся в контейнере `carDs`.
+

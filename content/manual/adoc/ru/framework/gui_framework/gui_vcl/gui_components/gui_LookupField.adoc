...
* Список опций компонента может быть задан произвольно с помощью методов `setOptionsList()`, `setOptionsMap()` и `setOptionsEnum()`, либо с помощью XML-атрибута `optionsContainer`.
+
--

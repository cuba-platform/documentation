...
`addDoubleClickListener()` добавляет слушатель, реализующий интерфейс `DoubleClickEvent`, для отслеживания двойных кликов по названиям опций компонента.
...
С той же целью вы можете подписаться на событие двойного клика, к примеру:
...
Разница в API между `OptionsList` и <<gui_OptionsGroup,OptionsGroup>> заключается в том, что `OptionsList` не имеет атрибута `orientation`.

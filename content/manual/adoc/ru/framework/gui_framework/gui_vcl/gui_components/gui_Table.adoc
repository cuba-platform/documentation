...
[[gui_Table_columns]]
* `columns` - элемент, определяющий набор колонок таблицы. Если не задан, то столбцы таблицы будут автоматически определены из атрибутов <<views,представления>>, используемого при загрузке данных из <<gui_data_containers,контейнера данных>> или <<datasources,источника данных>>. Элемент `columns` имеет следующие атрибуты:
+
--

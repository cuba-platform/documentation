...
Компонент может принимать список опций из data container: для этого используется атрибут <<gui_attr_optionsContainer,optionsContainer>>. Кроме этого, список опций для компонента можно указать с помощью методов `setOptions()`, `setOptionsList()` и `setOptionsMap()`. В этом случае атрибут `<lookup>` в дескрипторе экрана можно оставить пустым.
...
[[gui_TokenList_setOptionsMap]]
* `setOptionsMap()` позволяет задать строковые названия и значения опций по отдельности. Например, для инжектированного в контролллер экрана компонента `tokenList` задаём мэп опций:
+
--
[source, java]
----
include::{sourcesdir}/gui_vcl/gui_tokenList_6.java[]
----

При этом метод `getValue()` компонента будет возвращать выбранные сущности, а не строки, отображаемые на экране.
--

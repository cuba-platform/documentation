...
[[gui_Tree_ExpandListener]]
Метод `addExpandListener()` добавляет слушатель, реализующий интерфейс `ExpandEventListener`, для отслеживания событий разворачивания элементов дерева.

[source, java]
----
include::{sourcesdir}/gui_vcl/treeExpandListener.java[]
----

[[gui_Tree_CollapseListener]]
Метод `addCollapseListener()` добавляет слушатель, реализующий интерфейс `CollapseEventListener`, для отслеживания событий сворачивания элементов дерева.

[source, java]
----
include::{sourcesdir}/gui_vcl/treeCollapseListener.java[]
----
...
API::
<<gui_Tree_CollapseListener,addCollapseListener>> -
<<gui_Tree_ExpandListener,addExpandListener>> -

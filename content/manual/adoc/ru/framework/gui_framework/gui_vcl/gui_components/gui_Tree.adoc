...
Для компонента `Tree` можно установить <<gui_DataGrid_setDescriptionProvider,DescriptionProvider>>, с помощью которого отображается HTML в том случае, если задан режим `ContentMode.HTML`. Результат выполнения провайдера будет санитизирован, если для компонента `Tree` атрибут `htmlSanitizerEnabled` установлен в значение `true`.

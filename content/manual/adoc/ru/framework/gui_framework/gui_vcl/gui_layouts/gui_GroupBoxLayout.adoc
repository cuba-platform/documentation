...
[[gui_groupbox_light]]
Существует дополнительный предопределенный стиль контейнера `groupBox` - `light`. Этот стиль можно задать с помощью атрибута `stylename`. `Groupbox` со стилем `light` имеет только верхнюю границу, как показано в примере ниже.

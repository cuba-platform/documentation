...
* С помощью методов `getMinSplitPosition()` и `getMaxSplitPosition()` можно получить текущую <<gui_SplitPanel_minmax,минимальную>> или <<gui_SplitPanel_minmax,максимальную>> позицию разделителя соответственно.

[[gui_SplitPanel_getMinMaxSplitPositionSizeUnit]]
* С помощью методов `getMinSplitPositionSizeUnit()` и `getMaxSplitPositionSizeUnit()` можно получить единицу измерения <<gui_SplitPanel_minmax,минимальной>> или <<gui_SplitPanel_minmax,максимальной>> позиции разделителя соответственно. Возможные единицы измерения: `Component.UNITS_PIXELS` и `Component.UNITS_PERCENTAGE`.

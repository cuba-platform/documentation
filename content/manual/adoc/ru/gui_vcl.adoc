...
* `setIconByName()` - устанавливает значок элемента меню.
...
Значением атрибута должен быть путь к файлу значка относительно каталога <<gui_themes,темы>> либо его имя в используемом <<icon_set,наборе значков>>. Например:

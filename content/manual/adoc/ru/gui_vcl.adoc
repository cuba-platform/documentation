...
* Метод `scrollTo()` позволяет программно прокрутить таблицу до нужной записи. Метод должен получать экземпляр сущности, определяющий нужную строку в таблице.
+
Образец использования метода:
+
[source, java]
----

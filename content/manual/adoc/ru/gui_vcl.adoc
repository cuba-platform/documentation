...
Компонент `Image` предназначен для отображения графического содержимого из различных источников. Его можно привязать к источнику данных или настроить источник вручную.
...
+
Ниже приведён пример отображения изображений в ячейках таблицы с привязкой к источнику данных:
+
[source,java]
----
...
[[gui_Image_setSource]]
* `setSource()` - устанавливает источник изображения с помощью интерфейса `ImageResource` или его наследников. Для каждого типа источников есть свои методы для получения и назначения источников изображения, например:
+
--
[source,java]
----
...
----
Вы можете использовать следующие типы ресурсов или расширить их:
...
* `getMetaPropertyPath()` - возвращает <<MetaPropertyPath,property path>> источника.

...
* Атрибут `loadDynamicAttributes` управляет отображением <<dynamic_attributes,динамических утрибутов>> редактируемой сущности в окне редактора `bulkEditor`. Значение по умолчанию `true`.
--

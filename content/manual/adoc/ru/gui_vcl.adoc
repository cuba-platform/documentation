...
Атрибут `template` задает имя HTML-файла, находящегося в подкаталоге `layouts` темы. Например, если вы используете тему Halo и хотите назвать шаблон `my_template`, укажите `my_template` в атрибуте и разместите шаблон в файле `modules/web/themes/halo/layouts/my_template.html`.

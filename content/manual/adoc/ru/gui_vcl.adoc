...
** необязательный атрибут `generator` содержит ссылку на метод в контроллере экрана для отображения содержимого ячейки с помощью визуального компонента, отличного от стандартного:
+
[source, xml]
----

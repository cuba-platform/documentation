...
[[gui_Image_setSource]]
* `setSource()` - устанавливает источник изображения. Метод принимает тип ресурса и возвращает объект ресурса, который может быть сконфигурирован далее. Для каждого типа ресурсов есть свои методы, например `setPath()` для `ThemeImageResource` или `setStreamSupplier()` для `StreamImageResource`:
+
--
[source,java]
----
...
Можно использовать следующие типы ресурсов, реализующие интерфейс `ImageResource`, или расширить их:

* `ClasspathImageResource` - для изображений, хранимых в classpath. Этот ресурс также можно использовать декларативно с помощью элемента `classpath` компонента `image`.

* `FileDescriptorImageResource` - для изображений, получаемых из `FileStorage`.

* `FileImageResource` - для изображений, хранимых в файловой системе. Этот ресурс также можно использовать декларативно с помощью элемента `file` компонента `image`.

* `StreamImageResource` - для изображений, получаемых из потока.

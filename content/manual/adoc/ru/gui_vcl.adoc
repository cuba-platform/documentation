...
Компонент `Embedded` предназначен для вывода изображений и встраивания в экран произвольных веб-страниц. Начиная с версии 6.6 платформы для вывода изхображений доступен также компонент <<gui_Image,Image>>.
...
Компонент `Image` предназначен для отображения графического содержимого из различных источников. Источник содержимого можно привязать к источнику данных или настроить вручную.
...
[[gui_Image_classpath]]
* `classpath` - ссылка на ресурс с изображением, расположенным в classpath.
+
[source, xml]
----
...
[[gui_Image_file]]
* `file` - ссылка на файл с изображением, хранящимся в файловой системе.
+
[source, xml]
----
...
[[gui_Image_theme]]
* `theme` - ресурс, представленный изображением из темы приложения, например: `VAADIN/themes/customTheme/some/path/image.png`.
+
[source, xml]
----
...
[[gui_Image_url]]
* `url` - URL изображения, которое будет использовано как ресурс.
+
[source, xml]
----
...
[[gui_Image_setSource]]
* `setSource()` - устанавливает источник изображения. Для каждого типа источников есть свои методы для получения и назначения источников изображения, например:
+
По сравнению с декларативными элементами, при программном создании доступно больше типов источников. Для каждого типа есть свои методы для получения и назначения источников изображения, например, `setPath()` для `ThemeImageResource` или `setStreamSupplier()` для `StreamImageResource`:
+
--
[source,java]
----
...
* `createResource()` - создаёт ресурс изображения указанного типа.
+
--
[source,java]
----
...
----
Вы можете использовать следующие типы ресурсов, реализующие интерфейс `ImageResource`, или расширить их:

* `ClasspathImageResource` - для изображений, хранимых в classpath по указанному пути. Этот ресурс также можно использовать декларативно с помощью элемента `classpath` компонента `image`.

* `FileDescriptorImageResource` - для изображений, получаемых из `FileStorage` по указанному `FileDescriptor`.

* `FileImageResource` - для изображений, хранимых в файловой системе как указанных объект `File`. Этот ресурс также можно использовать декларативно с помощью элемента `file` компонента `image`.

* `StreamImageResource` - для изображений, представленных в виде потока.

* `ThemeImageResource` - для изображений темы, например, `VAADIN/themes/yourtheme/some/path/image.png`. Этот ресурс также можно использовать декларативно с помощью элемента `theme` компонента `image`.

* `UrlImageResource` - для изображений, загружаемых по указанному URL. Этот ресурс также можно использовать декларативно с помощью элемента `url` компонента `image`.
--

[[gui_Image_SourceChangeListener]]
* `addSourceChangeListener()` - добавляет листнер для отслеживания изменений источника изображения.
...
* `NONE` - конвертация отключена (значение по умолчанию).
--
...
+
--
Определяет, нужно ли сохранять пользовательские настройки отображения компонента.

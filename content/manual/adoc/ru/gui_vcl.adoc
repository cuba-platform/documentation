...
+
--
* `id` - обязательный атрибут, содержит строковый идентификатор колонки..
...
* `addChildItem/removeChildItem` - добалвяет/удаляет элементы меню в подгруппу корневого элемента.
...
+
Стандартный шаблон главного экрана с `sideMenu` стилизован несколькими предопределёнными стилями: `c-sidemenu-layout`, `c-sidemenu-panel` и `c-sidemenu-title`. Стиль бокового меню по умолчанию `c-sidemenu` описан в файле `cuba/modules/web/themes/halo/components/sidemenu/sidemenu.scss` и поддерживается только в рамках темы `Halo` и темах, её расширяющих. В теме `Havana` стили `sideMenu` не поддерживаются.

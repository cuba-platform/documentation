...
Фильтр можно сделать _global_ (то есть доступным для всех пользователей) с помощью установки флажка *Available for all users (Общий)* для всех пользователей, или установить текущий фильтр в качестве фильтра по умолчанию с помощью установки флажка *Default (По умолчанию)*. Для этой операции пользователю требуется <<permissions, разрешение>>.

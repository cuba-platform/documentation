...
<<gui_stylename_borderless,borderless>> -
<<gui_stylename_borderless-colored,borderless-colored>> -
<<gui_stylename_danger,danger>> -
<<gui_stylename_friendly,friendly>> -
<<gui_stylename_icon-align-right,icon-align-right>> -
<<gui_stylename_icon-align-top,icon-align-top>> -
<<gui_stylename_icon-only,icon-only>> -
<<gui_stylename_primary,primary>> -
<<gui_stylename_quiet,quiet>>
...
Изменение значения компонента `ColorPicker` можно отслеживать с помощью слушателя `ValueChangeListener`.
...
Изменение значения поля `DatePicker`, так же, как и любого другого компонента, реализующего интерфейс `Field`, можно отслеживать с помощью слушателя `ValueChangeListener`.
...
* `fieldFactoryBean`: декларативные поля, объявленные в XML-дескрипторе, создаются с помощью интерфейса `FieldGroupFieldFactory`. Чтобы переопределить эту фабрику, используйте атрибут `fieldFactoryBean` с именем вашей реализации `FieldGroupFieldFactory`. Для элемента `FieldGroup`, полностью созданного программно, для этой цели используется метод `setFieldFactory()`.
...
* Метод `setComponent()` задаёт собственное представление поля. Он может быть использован вместе с атрибутом `custom="true"` элемента `field` или с полем, созданным программно методом `createField()` (см. выше).  Экземпляр FieldConfig можно получить с помощью методов `getField()` или `getFieldNN()`, и затем вызвать метод `setComponent()`, как показано в следующем примере:
+
[source, java]
----
...
Динамические изменения текста надписи можно отслеживать с помощью слушателя `ValueChangeListener`.
...
[[gui_stylename_colored]]
* `colored` - цветной текст.

[[gui_stylename_failure]]
...
<<gui_stylename_bold,bold>> -
<<gui_stylename_colored,colored>> -
<<gui_stylename_failure,failure>> -
<<gui_stylename_h1,h1>> -
<<gui_stylename_h2,h2>> -
<<gui_stylename_h3,h3>> -
<<gui_stylename_h4,h4>> -
<<gui_stylename_light,light>> -
<<gui_stylename_no-margin,no-margin>> -
<<gui_stylename_spinner,spinner>> -
<<gui_stylename_success,success>>
...
* Изменения значения в поле `LookupField` можно отслеживать с помощью слушателя `ValueChangeListener`.
...
* Изменения значения компонента `OptionsGroup` можно отслеживать с помощью слушателя `ValueChangeListener`.
...
Кнопка имеет надпись, заданную с помощью атрибута <<gui_attr_caption,caption>>, и всплывающую подсказку, определенную в атрибуте <<gui_attr_description,description>>. Выпадающий список <<gui_Action,действий>> задан в элементе `actions`. `PopupButton` отображает только следующие свойства действий: `caption`, `enable`, `visible`. Свойства `description`, `shortcut` игнорируются. Обработка свойства `icon` зависит от свойства приложения <<cuba.gui.showIconsForPopupMenuActions,cuba.gui.showIconsForPopupMenuActions>> и от атрибута `showActionIcons` компонента. Последний имеет приоритет.
...
Компонент `SuggestionPickerField` объединяет в себе функциональность классического поля с автодополнением и компонента <<gui_PickerField,PickerField>>. Он отображает выпадающий список с подсказками возможных вариантов заполнения, сформированный согласно заданной логике.
...
`SuggestionPickerField` является полем выбора ссылочного атрибута, поэтому для компонента могут быть указаны атрибуты `datasource` и `property`:
...
+
--
Во многих случаях для компонента достаточно просто установить `SearchExecutor`. `SearchExecutor` - это функциональный интерфейс, содержащий всего один метод: `List<E extends Entity> search(String searchString, Map<String, Object> searchParams)`:
...
Параметр `searchString` может быть использован для поиска по определённому атрибуту сущности:
...
+
--
Компонент также может быть использовать обработчики событий `EnterActionHandler` и `ArrowDownActionHandler`. Эти листнеры срабатывают, когда пользователь нажимает клавиши *Enter* или *Arrow Down*, а подсказки при этом не появляются. Они также представляют собой функциональные интерфейсы с единственным методом с одним параметром - `currentSearchString`. Вы можете настроить и свои обработчики событий и использовать метод `showSuggestions()` интерфейса `SuggestionField`, который принимает список сущностей, для отображения подсказок:
...
Слушатели suggestionPickerField::
<<gui_LookupField_ValueChangeListener,ValueChangeListener>>
...
События изменения размеров области можно отслеживать с помощью слушателя `ResizeListener`.
--
...
* `ValueChangeListener` позволяет обрабатывать изменения значения в текстовом поле, когда пользователь уже закончил ввод, т.е. после нажатия клавиши Enter или при потере компонентом фокуса.
...
* Изменение значения поля `TimeField`, так же, как и любого другого компонента, реализующего интерфейс `Field`, можно отслеживать с помощью слушателя `ValueChangeListener`.
...
----
Атрибут `multiselect` работает только в том случае, если для атрибута `simple` задано значение `true`.
--
...
Изменение значений поля `TwinColumn`, так же, как и любого другого компонента, реализующего интерфейс `Field`, можно отслеживать с помощью слушателя `ValueChangeListener`.

...
`CurrencyField` в основном повторяет функциональность <<gui_TextField,TextField>>, за исключением того, что он поддерживает только числовые <<datatype,типы данных>>.

Его можно привязать к <<datasources,источнику данных>> с помощью атрибутов `datasource` и `property`:

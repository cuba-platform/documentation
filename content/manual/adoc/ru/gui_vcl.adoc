...
Если атрибут `disableOnClick` имеет значение `true` кнопка будет автоматически отулючена после клика по ней. Обычно делается для того, чтобы предотвратить случайные повторные клики по ней. В последствии, вы можете снова включить кнопку с помощью вызова `setEnabled(true)`.

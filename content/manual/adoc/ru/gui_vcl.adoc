...
`ColorPicker` представляет собой компонент, являющийся полем и позволяющим визуально выбирать RGB цвета. Он используется для предпросмотра цветов до их применения.
`ColorPicker` поддерживает такие манимуляции с цветами как RGB и HSV. Компонент возвращает HEX-код в виде `String`.
...
Вы можете определить видимость вкладок окна палитры с помощью атрибутов:
...
Вы можете переопределить надписи окна палитры:
...
Метод компонента `getValue()` возвращает `java.lang.String`, содержащую HEX-код цвета.

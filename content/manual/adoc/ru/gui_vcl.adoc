...
`PopupView` представляет собой компонент, позволяющий открывать popup с контейнером. Контейнер может открываться по клику на минимизированное значение или программно. Он может быть закрыт отведением курсора или по клику вне него.
...
Содержимое popup-а определено внутри компонента `PopupView` как любой контейнер вроде `HBoxLayout` или `VBoxLayout`.
...
* `captionAsHtml` позволяет использовать HTML тэги в подписи компонента.
--

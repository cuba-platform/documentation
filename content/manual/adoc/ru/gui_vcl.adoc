...
* Вы можете получить позицию разделителя с помощью метода `getSplitPosition()`.
* Если вам нужно получить единицу измерения позиции разделителя, используйте метод `getSplitPositionUnit()`. Он возвращает `Component.UNITS_PIXELS` или `Component.UNITS_PERCENTAGE`.

...
`DatePicker` это компонент для выбора даты. Он выглядит точно так же как и выпадающий календарь в <<gui_DateField,DateField>>.
...
* Вы можете указать доступные для выбора даты с помощью атрибутов `rangeStart` и `rangeEnd`. Если вы их установите, даты, выходящие за пределы указанного промежутка, будут отключены.
+
[source, xml]
----

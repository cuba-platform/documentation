...
* `addEditorPreCommitListener()`, `removeEditorPreCommitListener()` - слушатель редактора `DataGrid`, срабатывающий в процессе коммита изменений, после того как валидация завершена успешно и перед отправкой данных на *Middleware*.

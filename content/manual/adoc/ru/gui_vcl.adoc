...
* Метод `requestFocus()` позволяет установить фокус на определенное поле конкретной записи. Требуется два параметра: экземпляр сущности, определяющй строку и идентификатор колонки. Образец программной установки фокуса:
+
[source, java]
----

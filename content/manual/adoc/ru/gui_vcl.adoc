...
[[gui_DateField_range]]
* Вы можете указать диапазон доступных дат с помощью атрибутов `rangeStart` и `rangeEnd`. Если данные значения установлены, все даты, выходящие за пределы диапазона, будут отключены. Вы можете установить значения в XML с помощью маски "yyyy-MM-dd" или установить программно с помощью соответствующих сеттеров.
+
[source, xml]
----

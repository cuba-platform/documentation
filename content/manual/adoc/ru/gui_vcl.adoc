...
Компонент `Image` может отображать значение атрибута сущности. Для этого используется атрибут `datasource`, например:
...
Элементы `image` позволяют задать источник содержимого следующих четырёх типов:
...
* `ClasspathImageResource` - для изображений, хранимых в classpath по указанному пути.

* `FileDescriptorImageResource` - для изображений, получаемых из `FileStorage` по указанному `FileDescriptor`.

* `FileImageResource` - для изображений, хранимых в файловой системе как указанных объект `File`.

* `StreamImageResource` - для изображений, представленных в виде потока.

* `ThemeImageResource` - для изображений темы, например, `VAADIN/themes/yourtheme/some/path/image.png`.

* `UrlImageResource` - для изображений, загружаемых по указанному URL.
--

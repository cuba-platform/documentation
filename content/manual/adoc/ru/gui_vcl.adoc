...
| <<gui_BulkEditor,BulkEditor>>| image:gui_bulkEdit.png[]

| <<gui_RelatedEntities,RelatedEntities>>| image:gui_relatedEntities.png[]
...
| <<gui_FileMultiUploadField,FileMultiUploadField>>| image:MultiUpload.png[]
...
| <<gui_ProgressBar,ProgressBar>>| image:gui_progressBar.png[]

| <<gui_Filter,Filter>>| image:gui_filter_mini.png[]

| <<gui_Embedded,Embedded>>| image:cuba-64-01.png[]
...
Название кнопки задается с помощью атрибута <<gui_attr_basic_caption,caption>>, всплывающая подсказка − с помощью атрибута <<gui_attr_basic_description,description>>.

Атрибут <<gui_attr_basic_icon,icon>> указывает на местоположение пиктограммы. Подробную информацию о том, где следует располагать файлы пиктограмм, можно прочитать в <<gui_themes,>>
...
Действие для кнопки можно также создавать программно, в контроллере экрана, используя наследование от класса <<baseAction,AbstractAction>>.

Если для `Button` установлен экземпляр `Action`, то кнопка возьмет из него следующие свои свойства: <<gui_attr_basic_caption,caption>>, <<gui_attr_basic_description,description>>, <<gui_attr_basic_icon,icon>>, <<gui_attr_basic_enable,enable>>, <<gui_attr_basic_visible,visible>>. Свойства `caption` и `description` будут проставлены из действия только в том случае, если они не установлены в самом `Button`. Остальные перечисленные свойства действия имеют безусловный приоритет над свойствами кнопки. Если свойства действия меняются уже после установки этого `Action` для `Button`, то соответственно меняться будут и свойства `Button`, то есть кнопка слушает изменение свойств действия. В этом случае меняется и свойства `caption` и `description`, причем даже если они изначально были назначены на саму кнопку.
...
| action| <<gui_attr_basic_enable,enable>>| <<gui_attr_basic_stylename,stylename>>
...
| <<gui_attr_basic_description,description>>| invoke
...
`Bulk Editor` - компонент, позволяющий менять значения атрибутов сразу нескольких выбранных экземпляров сущностей. Компонент представляет собой кнопку, добавляющуюся к <<gui_Table,таблице>> или <<gui_Tree,дереву>> и при нажатии открывающую редактор сущностей. 
...
Для создания флажка, связанного с данными, необходимо использовать атрибуты <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>.
...
* Для создания поля даты, связанного с данными, необходимо использовать атрибуты <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>: 
+
[source, xml]
----
...
---- 
+
Как видно из примера, в экране описывается источник данных `orderDs` для некоторой сущности Заказ (`Order`), имеющей атрибут `date`. В компоненте ввода даты в атрибуте <<gui_attr_basic_datasource,datasource>> указывается ссылка на источник данных, а в атрибуте <<gui_attr_basic_property,property>> − название атрибута сущности, значение которого должно быть отображено в поле. 
...
+
Формат задается по правилам класса `SimpleDateFormat` (link:$$http://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html$$[http://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html]). Если в формате отсутствуют символы `H` или `h`, то поле времени не выводится.
...
| Связанная сущность| Задан атрибут ` optionsDatasource`| <<gui_LookupPickerField,LookupPickerField>>

| | <<gui_PickerField,PickerField>>

| Перечисление (`enum`)| | <<gui_LookupField,LookupField>>

| `string`| Задан атрибут `mask`| <<gui_MaskedField,MaskedField>>

| Задан атрибут `rows`| <<gui_TextArea,TextArea>>

| | <<gui_TextField,TextField>>

| `boolean`| | <<gui_CheckBox,CheckBox>>

| `date`, `dateTime`| | <<gui_DateField,DateField>>

| `time`| | <<gui_TimeField,TimeField>>

| `int`, `long`, `double`, `decimal`| | <<gui_TextField,TextField>>
...
* В контроллере экрана инжектируем сам компонент, а также интерфейсы <<file_upload,FileUploadingAPI>> и <<dataSupplier,DataSupplier>>. Затем в методе `init()` добавляем компоненту слушатель, который будет реагировать на события успешной загрузки или ошибки:
+
[source, java]
----
@Inject
...
* Максимальный размер загружаемого файла определяется свойством приложения <<cuba.client.maxUploadSizeMb,cuba.client.maxUploadSizeMb>> и по умолчанию равен 20Мб. При выборе пользователем файла большего размера выдается соответствующее сообщение и загрузка прерывается.
...
----
+
Метод `uploadSucceeded()` будет вызван компонентом после успешной загрузки файла во временное хранилище клиентского <<app_tiers,уровня>>. В этот момент у компонента можно получить объект `FileDescriptor`, соответствующий загруженному файлу. Объект `com.haulmont.cuba.core.entity.FileDescriptor` (не путать с `java.io.FileDescriptor`) является персистентной сущностью, которая однозначно идентифицирует загруженный файл и впоследствии используется для выгрузки файла из системы.
+
...
* *Pin applied (Закрепить)* – использовать результаты последнего поиска для последовательной фильтрации данных (см. <<sequential_filter,>>).
...
** `caption` - локализованное название атрибута сущности для отображения условия фильтра. Как правило, представляет из себя строку с префиксом `msg://` по правилам `MessageTools.<<messageTools.loadString,loadString>>()`.
+
...
** `caption` − обязательный атрибут - локализованное название условия. Как правило, представляет из себя строку с префиксом `msg://` по правилам `MessageTools.<<messageTools.loadString,loadString>>()`.
...
+
Если значение этого атрибута равно `false`, то фильтр не будет отображать поле *Показывать строк*. Количество записей в источнике данных (и соответственно, показываемых таблицей) будет ограничено только параметром `MaxFetchUI` механизма <<entity_statistics,статистики сущностей>>, по умолчанию - 10000.
+
...
| <<gui_attr_filter_useMaxResults,useMaxResults>>| <<gui_attr_basic_visible,visible>>
...
* Чтобы иметь возможность изменять максимальное количество строк на странице таблицы с помощью флажка и поля *Show first N rows* пользователь должен иметь разрешение `cuba.gui.filter.maxResults`. См. также атрибут фильтра <<gui_attr_filter_useMaxResults,useMaxResults>>.
...
* <<cuba.gui.genericFilterManualApplyRequired,cuba.gui.genericFilterManualApplyRequired>> − позволяет отключить автоматическое применение фильтра (то есть загрузку данных) сразу при открытии экрана.
...
* <<cuba.cuba.gui.genericFilterColumnsQty,cuba.gui.genericFilterColumnsQty>> - задает количество колонок по умолчанию для размещения условий фильтра.
...
`group` − необязательный элемент, может в единственном экземпляре находиться внутри <<gui_element_table_columns,columns>>. Содержит набор элементов `column`, по которым будет выполняться первоначальная группировка при открытии экрана.
...
| <<gui_attr_table_allowPopupMenu,allowPopupMenu>>| <<gui_attr_basic_height,height>>| <<gui_attr_table_reorderingAllowed,reorderingAllowed>>| <<gui_attr_basic_width,width>>

| <<gui_attr_table_columnControlVisible,columnControlVisible>>| <<gui_attr_basic_id,id>>| <<gui_attr_table_sortable,sortable>>

| <<gui_attr_basic_editable,editable>>| <<gui_attr_table_multiselect,multiselect>>| <<gui_attr_basic_stylename,stylename>>

| <<gui_attr_basic_enable,enable>>| <<gui_attr_table_presentations,presentations>>| <<gui_attr_basic_visible,visible>>
...
Атрибуты <<gui_element_table_columns,column>>:

[cols="4", frame="none"]
|===

| <<gui_attr_table_column_caption,caption>>| <<gui_attr_dateField_dateFormat,dateFormat>>| <<gui_attr_dateField_resolution,resolution>>

| <<gui_attr_basic_captionProperty,captionProperty>>| <<gui_attr_table_column_editable,editable>>| <<gui_attr_basic_visible,visible>>

| <<gui_attr_basic_clickAction,clickAction>>| <<gui_attr_table_column_id,id>>| <<gui_attr_table_column_width,width>>

| <<gui_attr_table_column_collapsed,collapsed>>| <<gui_attr_basic_optionsDatasource,optionsDatasource>>

|===

Элементы <<gui_element_table_columns,column>>:
...
Атрибуты <<gui_element_table_rows,rows>>:
...
В веб клиенте текст, содержащийся в атрибуте `value`, будет разбит на несколько строк, если по длине он превысит значение атрибута <<gui_attr_basic_width,width>>. Поэтому для отображения многострочной надписи, достаточно указать абсолютное значение атрибута <<gui_attr_basic_width,width>>. Если текст надписи слишком длинный, а значение атрибута <<gui_attr_basic_width,width>> не определено, то текст будет урезан.
...
Компонент `Label` может отображать значение атрибута сущности. Для этого используются атрибуты <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>. Например:
...
Атрибут `htmlEnabled` указывает, каким образом будет рассматриваться значение атрибута <<gui_attr_label_value,value>>: как `html`-код, при `htmlEnabled="true"`, или как строка. Обратите внимание, что не все `html`-теги поддерживаются в десктоп-реализации экрана. 
...
Другие атрибуты `Link`: <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_icon,icon>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 
...
Кнопка-ссылка отличается от обычной кнопки `Button` только своим внешним видом. Все свойства и поведение идентичны <<gui_Button,описанным для Button>>.

Пример XML-описания кнопки-ссылки, вызывающей метод `someMethod()` контроллера, с надписью (атрибут <<gui_attr_basic_caption,caption>>), всплывающей подсказкой (атрибут <<gui_attr_basic_description,description>>) и пиктограммой (атрибут <<gui_attr_basic_icon,icon>>): 
...
----
+
Как видно из примера, в экране описывается <<datasources,источник данных>> `roleDs` для сущности `Role`. В компоненте `lookupField` в атрибуте <<gui_attr_basic_datasource,datasource>> указывается ссылка на источник данных, а в атрибуте <<gui_attr_basic_property,property>> − название атрибута сущности, значение которого должно быть отображено. В данном случае атрибут является перечислением, и в выпадающем списке будут отображены <<enum_localization,локализованные названия>> всех значений этого перечисления.

* Аналогично можно использовать `LookupField` для выбора экземпляра связанной сущности. Для формирования списка опций используется атрибут <<gui_attr_basic_optionsDatasource,optionsDatasource>>:
+
[source, xml]
----
<dsContext>
...
+
С помощью атрибута <<gui_attr_basic_captionProperty,captionProperty>> можно указать, какой атрибут сущности использовать вместо имени экземпляра для строковых названий опций. 
...
Атрибуты `lookupField`: 
 ...
 <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_captionProperty,captionProperty>> | <<gui_attr_basic_datasource,datasource>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_editable,editable>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_lookupField_filterMode,filterMode>> | <<gui_attr_basic_height,height>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_inputPrompt,inputPrompt>> | <<gui_attr_newOptionHandler,newOptionHandler>> | <<gui_attr_nullName,nullName>> | <<gui_attr_basic_optionsDatasource,optionsDatasource>> | <<gui_attr_basic_property,property>> | <<gui_attr_basic_required,required>> | <<gui_attr_basic_requiredMessage,requiredMessage>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 
...
Атрибуты `lookupPickerField`: 
 ...
 <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_captionProperty,captionProperty>> | <<gui_attr_basic_datasource,datasource>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_editable,editable>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_lookupField_filterMode,filterMode>> | <<gui_attr_basic_height,height>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_inputPrompt,inputPrompt>> | <<gui_attr_pickerField_metaclass,metaClass>> | <<gui_attr_nullName,nullName>> | <<gui_attr_basic_optionsDatasource,optionsDatasource>> | <<gui_attr_basic_property,property>> | <<gui_attr_basic_required,required>> | <<gui_attr_basic_requiredMessage,requiredMessage>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 
...
----
+
Как видно из примера, в экране описывается <<datasources,источник данных>> `roleDs` для сущности `Role`. В компоненте `optionsGroup` в атрибуте <<gui_attr_basic_datasource,datasource>> указывается ссылка на источник данных, а в атрибуте <<gui_attr_basic_property,property>> − название атрибута сущности, значение которого должно быть отображено.
+
...
** Компонент может брать список опций из источника данных. Для этого используется атрибут <<gui_attr_basic_optionsDatasource,optionsDatasource>>. Например:
+
[source, xml]
----
<dsContext>
...
image::gui_textField_secret.png[align="center"]
...
* Как правило, `PickerField` используется для работы со ссылочными атрибутами сущностей. При этом компоненту достаточно указать атрибуты <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>:
+
[source, xml]
----
<dsContext>
...
----
+
Декларативное и программное создание действий подробно описано в <<gui_Action,>>.



* Компонент `PickerField` можно использовать без непосредственной привязки к данным, то есть без указания <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>. В этом случае для указания типа сущности, с которой должен работать `PickerField`, используется атрибут `metaClass`. В нем необходимо указать <<entity_class_annotations,имя сущности в метаданных>>, например:
+
[source, xml]
----
...
[WARNING]
====
Для правильной работы компонента `PickerField` необходима либо установка атрибута `metaClass`, либо одновременная установка атрибутов <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>.
====
...
Кнопка имеет надпись, заданную с помощью атрибута <<gui_attr_basic_caption,caption>>, и всплывающую подсказку, определенную в атрибуте <<gui_attr_basic_description,description>>. Выпадающий список <<gui_Action,действий>> задан в элемене `actions`. `PopupButton` отображает только следующие свойства действий: `caption`, `enable`, `visible`. Свойства `description`, `icon`, `shortcut` игнорируются.
...
Атрибут `++openType=&rdquo;NEW_TAB&rdquo;++` устанавливает режим открытия браузера (новая вкладка). По умолчанию браузер открывается в текущей вкладке. 
...
| <<gui_attr_basic_align,align>>| <<gui_attr_related_exclude_1,exclude>>| <<gui_attr_tokenList_lookup_openType,openType>>

| <<gui_attr_basic_caption,caption>>| <<gui_attr_related_for_1,for>>| <<gui_attr_basic_stylename,stylename>>
...
| <<gui_attr_related_property_filterCaption_1,filterCaption>>

| <<gui_attr_related_property_name_1,name>>

| <<gui_attr_related_property_screen_1,screen>>
...
`RichTextArea` в основном повторяет функциональность `
          <<gui_TextField,TextField>>
        `, за исключением того, что ему нельзя установить `datatype`. То есть `RichTextArea` предназначен для работы только с текстом и строковыми атрибутами сущностей.
...
----
+
В данном случае компонент будет искать экземпляры сущности `Colour` по вхождению подстроки в ее атрибут `name`. Префикс `(?i)` служит для регистро-независимого поиска (см. <<datasource_query_case_insensitive,>>). Выбранное значение подставится в атрибут `colour` сущности `Car`, находящейся в источнике данных `carDs`.
...
Атрибуты `searchPickerField`: 
 ...
 <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_captionProperty,captionProperty>> | <<gui_attr_basic_datasource,datasource>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_editable,editable>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_lookupField_filterMode,filterMode>> | <<gui_attr_basic_height,height>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_inputPrompt,inputPrompt>> | <<gui_attr_pickerField_metaclass,metaClass>> | <<gui_attr_searchPickerField_minSearchStringLength,minSearchStringLength>> | <<gui_attr_nullName,nullName>> | <<gui_attr_basic_optionsDatasource,optionsDatasource>> | <<gui_attr_basic_property,property>> | <<gui_attr_basic_required,required>> | <<gui_attr_basic_requiredMessage,requiredMessage>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 
...
----
 ...
 Здесь в элементе `dsContext` определен <<datasources,источник данных>> `collectionDatasource`, который выбирает сущности `Order` с помощью <<jpql,JPQL>> запроса 
...
* Атрибут `sortable` разрешает или запрещает сортировку в таблице. По умолчанию имеет значение `true`. Если сортировка разрешена, то при нажатии на название колонки справа от названия появляется значок image:gui_sortable_down.png[]/image:gui_sortable_up.png[].
+
...
* Атрибут `presentations` управляет механизмом <<gui_Table_presentations,представлений>>. Значение по умолчанию равно `false`. Когда значение атрибута равно `true`, то в верхнем правом углу таблицы появляется значок image:gui_presentation.png[]. Механизм представлений реализован только для блока *Web Client*.
...
+
Для редактируемой колонки, отображающей связанную сущность, можно дополнительно указать атрибуты <<gui_attr_basic_optionsDatasource,optionsDatasource>> и <<gui_attr_basic_captionProperty,captionProperty>>. При указании `optionsDatasource` вместо <<gui_PickerField,PickerField>> используется компонент <<gui_LookupField,LookupField>>.
+
...
----
+
Далее нужно определить заданные для строк и ячеек стили в теме приложения. Подробная информация о создании темы находится в<<gui_themes,>>. Для веб-клиента новые стили определяются в файле `styles.scss`. Имена стилей, заданные в контроллере, совместно с префиксами, обозначающими строку или колонку таблицы, образуют CSS-селекторы. Например: 
+
[source, css]
----
...
| <<gui_attr_table_allowPopupMenu,allowPopupMenu>>| <<gui_attr_basic_enable,enable>>| <<gui_attr_table_multiselect,multiselect>>| <<gui_attr_basic_stylename,stylename>>

| <<gui_attr_table_columnControlVisible,columnControlVisible>>| <<gui_attr_basic_height,height>>| <<gui_attr_table_presentations,presentations>>| <<gui_attr_basic_visible,visible>>

| <<gui_attr_basic_description,description>>| <<gui_attr_basic_id,id>>| <<gui_attr_table_reorderingAllowed,reorderingAllowed>>| <<gui_attr_basic_width,width>>

| <<gui_attr_basic_editable,editable>>| <<gui_attr_basic_margin,margin>>| <<gui_attr_table_sortable,sortable>>
...
| <<gui_attr_table_column_caption,caption>>| <<gui_attr_dateField_dateFormat,dateFormat>>| <<gui_attr_table_column_linkInvoke,linkInvoke>>| <<gui_attr_basic_optionsDatasource,optionsDatasource>>

| <<gui_attr_basic_captionProperty,captionProperty>>| <<gui_attr_table_column_editable,editable>>| <<gui_attr_table_column_linkScreen,linkScreen>>| <<gui_attr_dateField_resolution,resolution>>

| <<gui_attr_basic_clickAction,clickAction>>| <<gui_attr_table_column_id,id>>| <<gui_attr_table_column_linkScreenOpenType,linkScreenOpenType>>| <<gui_attr_basic_visible,visible>>

| <<gui_attr_table_column_collapsed,collapsed>>| <<gui_attr_table_column_link,link>>| <<gui_attr_table_column_maxTextLength,maxTextLength>>| <<gui_attr_table_column_width,width>>
...
`TextArea` в основном повторяет функциональность `
          TextField`, за исключением того, что ему нельзя установить `datatype`. То есть `TextArea` предназначен для работы только с текстом и строковыми атрибутами сущностей.
...
* Для создания текстового поля, связанного с данными, необходимо использовать атрибуты <<gui_attr_basic_datasource,datasource>> и <<gui_attr_basic_property,property>>.
+
[source, xml]
----
<dsContext>
...
----
+
Как видно из примера, в экране описывается <<datasources,источник данных>> `customerDs` для некоторой сущности `Покупатель (Customer)`, имеющей атрибут `name`. В компоненте текстового поля в атрибуте <<gui_attr_basic_datasource,datasource>> указывается ссылка на источник данных, а в атрибуте <<gui_attr_basic_property,property>> − название атрибута сущности, значение которого должно быть отображено в текстовом поле.
...
Атрибуты `textField`:
 ...
 <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_datasource,datasource>> | <<gui_attr_textField_datatype,datatype>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_editable,editable>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_basic_height,height>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_inputPrompt,inputPrompt>> | <<gui_attr_textField_maxLength,maxLength>> | <<gui_attr_basic_property,property>> | <<gui_attr_basic_required,required>> | <<gui_attr_basic_requiredMessage,requiredMessage>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_textField_trim,trim>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 
...
Атрибуты `timeField`: <<gui_attr_basic_align,align>> | <<gui_attr_basic_caption,caption>> | <<gui_attr_basic_editable,editable>> | <<gui_attr_basic_enable,enable>> | <<gui_attr_basic_datasource,datasource>> | <<gui_attr_basic_description,description>> | <<gui_attr_basic_height,height>> | <<gui_attr_basic_id,id>> | <<gui_attr_basic_property,property>> | <<gui_attr_basic_required,required>> | <<gui_attr_basic_requiredMessage,requiredMessage>> | <<gui_attr_timeField_showSeconds,showSeconds>> | <<gui_attr_basic_stylename,stylename>> | <<gui_attr_timeField_timeFormat,timeFormat>> | <<gui_attr_basic_visible,visible>> | <<gui_attr_basic_width,width>> 

Элементы `timeField`: <<gui_validator,validator>> 
...
*  `simple` - значение `true` позволяет убрать компонент выбора, оставляя только кнопку добавления. При нажатии на кнопку добавления сразу показывается экран списка экземпляров сущности, тип которой задан источником данных `datasource`. Идентификатор экрана выбора определяется по правилам, описанным для <<standard_actions,стандартного действия>> `PickerField.LookupAction`. 
...
** Атрибут `lookupScreen` задает идентификатор экрана для выбора значений в режиме `lookup="true"`. Если данный атрибут не задан, то идентификатор экрана выбора определяется по правилам, описанным для <<standard_actions,стандартного действия>> `PickerField.LookupAction`. 

** Атрибут `openType` можно задать способ открытия экрана выбора, аналогично описанному для <<standard_actions,стандартного действия>> `PickerField.LookupAction`. По умолчанию - `++THIS_TAB++`.
...
* `button` − описатель кнопки добавления значений. Может содержать атрибуты <<gui_attr_basic_caption,caption>> и <<gui_attr_basic_icon,icon>>.
...
Все атрибуты <<gui_element_tokenList_lookup,lookup>>:
...
Атрибуты <<gui_element_tokenList_button,button>>:
...
Если `buttonsPanel` находится внутри `table`, то она комбинируется с компонентом <<gui_attr_table_rowsCount,rowsCount>> таблицы, тем самым оптимально расходуя место по вертикали. Кроме того, в этом случае при открытии экрана выбора методом `IFrame.openLookup()` (например, из компонента <<gui_PickerField,PickerField>>) панель кнопок скрывается.
...
Элемент `iframe` предназначен для включения в экран <<frame,фрейма>>.
...
* <<gui_attr_basic_caption,caption>> - заголовок вкладки. 
...
[WARNING]
====
Formatter предназначен для использования с read-only компонентами, такими как <<gui_Label,Label>>, колонка <<gui_Table,Table>> и тому подобными. Для форматирования значения в редактируемых компонентах, например <<gui_TextField,TextField>>, используйте механизм `
            <<datatype,Datatype>>
          `.
====
...
Кроме того, класс `DateFormatter` распознает также атрибут `type`, который может принимать значения `DATE` или `DATETIME`. В этом случае форматирование производится с помощью механизма `
          <<datatype,Datatype>>
        ` по строке формата `dateFormat` или `dateTimeFormat` соответственно. Например:
...
+
--
Задает имя атрибута сущности, отображаемого компонентом. Используется только для сущностей, находящихся в источнике данных (например заданном для <<gui_LookupField,LookupField>> свойством <<gui_attr_basic_optionsDatasource,optionsDatasource>>).

Если `captionProperty` не задан, будет отображаться <<namePattern,имя экземпляров>>, содержащихся в источнике данных.
--

[[gui_attr_basic_clickAction]]
clickAction:: 
+
--
Атрибут содержит описание действия, которое будет выполнено при клике в ячейке или в поле (для компонента <<gui_FieldGroup,FieldGroup>>). Возможны два типа действий:
...
+
image::gui_clickAction_open.png[align="center"]
...
+
--
Указывает, сколько <<gui_GridLayout_span,колонок сетки>> должен занять компонент (по умолчанию 1).
...
При указании атрибута `datasource` для компонента, реализующего интерфейс `DatasourceComponent`, необходимо также задать атрибут <<gui_attr_basic_property,property>>. 
--
...
+
--
Атрибут, указывающий на возможность редактирования содержимого компонента (не путать с <<gui_attr_basic_enable,enable>>).
...
Атрибут используется для компонентов <<gui_TextField,TextField>>, <<gui_LookupField,LookupField>>, <<gui_LookupPickerField,LookupPickerField>>, <<gui_SearchPickerField,SearchPickerField>> только в *web* клиенте.
--
...
Совместно с `optionsDatasource` может использоваться атрибут <<gui_attr_basic_captionProperty,captionProperty>>.
--
...
Используется всегда совместно с атрибутом <<gui_attr_basic_datasource,datasource>>.
--
...
+
--
XML-атрибут, используемый совместно с атрибутом <<gui_attr_basic_required,required>>. Позволяет установить сообщение, выводимое пользователю в случае нарушения требования <<gui_attr_basic_required,required>>.
...
+
--
Указывает, сколько <<gui_GridLayout_span,строк сетки>> должен занять компонент (по умолчанию 1).

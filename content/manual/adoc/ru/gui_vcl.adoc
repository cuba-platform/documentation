...
При использовании <<application_folder,Папок приложения>> и <<search_folder,Папок поиска>> можно настроить постоянные условия фильтрации и скрыть их от пользователя, оставив видимым только FTS-условие:

image::fts_search_folder.png[align="center"]

В результате, папка поиска будет выглядеть следующим образом:

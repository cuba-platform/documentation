...
m|SELECT AVG(o.quantity)/2.0 FROM app$Order o
...
.2+|Оператор CASE Expression в UPDATE-запросе
...
m|UPDATE app$Employee e SET e.salary = CASE e.rating WHEN 1 THEN e.salary * 1.1 WHEN 2 THEN e.salary * 1.05 ELSE e.salary * 1.01 END
...
m|SELECT e FROM app$Employee e WHERE TYPE(e) IN (app$Exempt, app$Contractor)
...
m|SELECT u FROM sec$User u WHERE function('hasRoles', u.createdBy, u.login)
...
m|SELECT v.location.street, KEY(i).title, VALUE(i) FROM app$VideoStore v JOIN v.videoInventory i WHERE v.location.zipcode = '94301' AND VALUE(i) > 0
...
m|SELECT e FROM app$Employee e WHERE e.startDate = {d'2012-01-03'}
...
m|SELECT e FROM app$Employee e WHERE 'write code' MEMBER OF e.codes
...
m|SELECT NEW com.acme.example.CustomerDetails(c.id, c.status, o.count) FROM app$Customer c JOIN c.orders o WHERE o.count > 100
...
.3+|Строковые функции (CONCAT, SUBSTRING, TRIM, LOWER, UPPER, LENGTH, LOCATE)
...
m|SELECT x FROM app$Magazine x WHERE TRIM(TRAILING 'J' FROM x.title) = 'D'

|ДА: TRIM

m|SELECT x FROM app$Magazine x WHERE TRIM(TRAILING FROM x.title) = 'D'
...
|НЕТ: вложенный запрос с переменной path

m|SELECT c FROM app$Customer c WHERE (SELECT AVG(o.price) FROM c.orders o) > 100
...
m|SELECT e FROM Employee e JOIN e.projects p WHERE TREAT(p as LargeProject).budget > 1000000

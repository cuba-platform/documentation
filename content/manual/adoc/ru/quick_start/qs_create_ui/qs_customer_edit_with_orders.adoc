...
<<screen_controller_injection,Инжектируйте>> loader для заказов в класс контроллера. Для этого нажмите Alt+Insert и выберите *Inject*, в появившемся списке выберите `ordersDl`. Либо вы можете ввести код вручную:
...
Затем подпишитесь на событие `BeforeShowEvent`, чтобы указать значение параметра `customer` в запросе `ordersDl`. Для этого нажмите Alt+Insert и выберите *Subscribe to Event*, в появившемся списке выберите *BeforeShowEvent*. Либо можно добавить код вручную: 

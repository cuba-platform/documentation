...
Любое запущенное приложение на CUBA также экспортирует документацию Swagger, см. <<rest_swagger,>>.
====
...
Атрибут `cacheable` элемента `<query>` включает <<entity_cache,кэширование>> для данного запроса.
...
Если сущность, возвращенная REST API, имеет атрибут-коллекцию, и для сущностей в этой коллекции имеется <<constraints,ограничение>>, то возвращаемый JSON может содержать системный атрибут `+__securityToken+`. Если данная сущность будет в дальнейшем передана в методы REST API для обновления, то в объекте JSON необходимо определить атрибут `+__securityToken+` со значением, полученным при чтении сущности. Если этого не сделать, коллекция может быть сохранена некорректно.
...
Любое запущенное приложение на CUBA также экспортирует документацию Swagger конкретно для этого приложения. Её на лету генерирует бин `SwaggerGenerator` в соответствии с версией 2.0 https://github.com/OAI/OpenAPI-Specification/blob/master/versions/2.0.md[спецификации] технологии https://swagger.io/docs/specification/2-0/basic-structure/[Swagger].

Генерируемая документация доступна по следующим адресам:

* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.yaml` - YAML-версия общей документации,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swagger.json` - JSON-версия общей документации,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swaggerDetailed.yaml` - YAML-версия проектной документации,
* `\http://HOST:PORT/APP_NAME/rest/v2/docs/swaggerDetailed.json` - JSON-версия проектной документации.
...
image::swagger_crud_model_2.png[align="center"]
--

. Предопределённые REST-запросы, например, <<rest_api_v2_queries_config,запросы JPQL>>, зарегистрированные в `rest-queries.xml`:
+
image::swagger_query.png[align="center"]

. <<rest_api_v2_services_config,Сервисы>>, доступные через REST и зарегистрированные в `rest-services.xml`:
+
image::swagger_service.png[align="center"]

Такая документация может использоваться для тестирования REST API проекта, например, с помощью https://swagger.io/swagger-inspector/[Swagger Inspector] или через импорт в https://www.getpostman.com/[Postman], для https://swagger.io/swagger-codegen/[генерации] кода, который будет взаимодействовать с REST API вашего проекта, или просто для https://swagger.io/swagger-ui/[визуализации] существующего REST API.

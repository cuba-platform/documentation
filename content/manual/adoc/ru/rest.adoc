...
===== Создание своего механизма аутентификации для REST API
...
. Ссылку на этот файл укажите в свойстве приложения `cuba.restSpringContextConfig` в файде `modules/web/src/web-app.properties`:
+
[source, properties]
----

...
By default, a token expires after 12 hours of inactivity. After this timeout requests stop working and the user has to re-login.
...
+
--
We currently don't provide a specific web component that assists in entity updates, but CUBA REST API contains a method for this. See the http://files.cuba-platform.com/swagger/{revnumber}[REST API specification] for details.
--

...
Despite the name of this section, techniques listed here are required to build almost any real web application
that consists of more than a dozen components.
...
As you can see, `this.$` can be used to find elements that contain id. In found elements, you can change properties and call methods.
...
In such cases you can use `this.$$("selector")` to find a required element. This method will search for the element dynamically.
...
* Prefix element attributes with "$" if you want them to be changed when element properties change. Mostly, it's required

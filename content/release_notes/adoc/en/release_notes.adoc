...
You can implement additional authentication mechanisms for REST-API, e.g. login-by-link, social login (Facebook, Twitter, etc) or LDAP.

It can be done using a new REST-API component: OAuthTokenIssuer.

The process is as follows:

- You implement custom Spring MVC controller that must perform custom authentication.
- Register this controller in REST API Spring context using custom Spring XML file and cuba.restSpringContextConfig application property.
- If a user is authenticated the controller creates OAuth2 Access token usin OAuthTokenIssuer bean.
- Created OAuth2 token should be returned to the user.

After that, a client of REST API can use it as usual passing OAuth2 access token to REST API.

YouTrack: https://youtrack.cuba-platform.com/issue/PL-9365[PL-9365].

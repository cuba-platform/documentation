...
User tasks may have an assignee specified in the *assignee* property of the User Task node. The property value may be a CUBA user identifier, a process variable holding a user identifier or a service invocation that returns a user identifier. ProcActor object for such cases may be not created beforehand, it will be ctreated automaticallly. See the {bpm}/user_task.html[BPM manual] for details.

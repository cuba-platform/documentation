...
Login form generated by Studio has more sophisticated design now:

image::polymer_ui_login.png[align="center"]

[[polymer_serve]]
==== Running "polymer serve" from Studio
Now in order to run native Polymer development server - https://www.polymer-project.org/2.0/docs/tools/polymer-cli-commands#serve[polymer serve], you do not need to have Node.js installed locally. Studio allows to do it from UI using project's `node` and `polymer-cli`.
Use 'Run' -> 'Start "polymer serve"' item in main menu:

...
. Types of the following parameters of charts have been changed:
+
  * Parameter `plotAreaFillColors` of `SerialChart`, `GanttChart`, `XYChart` and `StockPanel` has `List<Color>` type.
  * Parameters `fillColors` and `negativeFillColors` of `SerialChart`, `GanttChart`, `XYChart`, `RadarChart` and StockGraph have `List<Color>` type.
  * Parameter `compareGraphFillColors` of StockGraph has `List<Color>` type.
  * Parameter `startDuration` of `HasStartEffect` has `Double` type.
  * Parameter `minorTickInterval` of `GaugeAxis` has `Double` type.
+
https://youtrack.cuba-platform.com/issues/PL?q=%23Charts%20%23breaking-changes%20%23%7BRelease%206.7%7D%20[Related issues]

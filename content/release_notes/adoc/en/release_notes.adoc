...
. The {manual}/build.gradle_buildWar.html[buildWar] and {manual}/build.gradle_buildUberJar.html[buildUberJar] tasks get Polymer UI from the directory corresponding to the latest version, now it is `es6-unbundled`. If you have Polymer UI in your project, and you build WAR or UberJAR, you should add the following parameter to the build task until you <<polymer_client,migrate to Polymer 2>>:
+
[source,groovy]
----

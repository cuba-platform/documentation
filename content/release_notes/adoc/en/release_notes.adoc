...
. Internet Explorer 8-10 is not supported any more. Supported web browsers are: Chrome, Firefox 45+, Safari 4.1+, Internet Explorer 11, Microsoft Edge.
...
. Removed multiSelect attribute of TwinColumn UI component.

. TextArea and ResizableTextArea are different UI components. If you inject `<textArea>` element in a screen controller it will lead to ClassCastException.

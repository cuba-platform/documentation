...
More info about new features on https://www.polymer-project.org/2.0/docs/about_20[Polymer website].

==== Changes in build process
Dependency on Gulp and `gulpfile.js` were removed in favour of direct `polymer-cli` usage. By default Polymer client is targeting browsers with es6 support however it's possible to specify different build target. More info in {manual}/polymer_build_and_structure.html[documentation].
...
You can migrate your project manually by performing the following steps:

. Bump components versions in `bower.json`. Here is the example of components which are used in Polymer 2.0 compatible client
+
[source, json]
----
   "dependencies": {

...
. Login process and mechanisms of the Web Client have changed dramatically.
+
--
** Removed `AbstractConnection`, `DefaultConnection` classes.
** Removed `cuba-web-auth` artifact.
** `App` / `Connection` beans now must have `vaadin` scope in Spring context.
** App property `cuba.web.externalAuthenticationProviderClass` is set `to com.haulmont.cuba.web.auth.NoOpAuthProvider` by default. If you use LDAP authentication you have to enable it explicitly using `cuba.web.ldap.enabled` property: {manual}/ldap_basic.html[documentation].
** Deprecated `CubaAuthProvider` interface - use `LoginProvider` and `HttpRequestFilter` beans instead.
** Deprecated app properties: `cuba.web.externalAuthentication`, `cuba.web.externalAuthenticationProviderClass`.

Please note, that old `LdapAuthProvider` and `IdpAuthProvider` settings should work as before.

See {manual}/web-login.html[documentation] for details.

--

. `AuthenticationManager` uses separate session with user "server" for `authenticate` and `login` processing. This server session instantiated directly without login procedure. See details in the {youtrack}/PL-10140[issue].

...
+
For JPQL queries the https://doc.cuba-platform.com/manual-6.3/data_store.html[datastore] should be set, if not, the default `Main` data store is used. To use an entity from an additional datastore, put its name in the *Data store* field:

image::report_structure_jpql.png[align="center"]

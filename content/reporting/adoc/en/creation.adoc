...
[[sql_preprocessing]]
SQL preprocessing in bands::
+
--
If you need to modify the SQL/JPQL query dynamically depending on the report input parameters, or parameter values from the parent band, you can use the SQL preprocessing. In order to activate it, check the *Preprocess SQL as Groovy template* checkbox below the band editor. The resulting query will be processed by `GStringTemplateEngine`. The template engine will have access to:

* the report parameters: `${<parameter_name>}`,

* the values from the parent band: `{<band_name>.<parameter_name>}`.

For example, depending on the `createTs2` report parameter you need to select one of the two the query conditions: `e.create_ts < ${createTs2}` or `e.create_ts < current_timestamp`.
...
Then the template engine will transform it into
...
, thus enabling you write queries in SQL/JPQL instead of Groovy directly.
--

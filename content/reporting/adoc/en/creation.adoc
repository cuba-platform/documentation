...
Pivot Table output type is available if the application project includes the *charts* component. For more information in Pivot Table see the https://doc.cuba-platform.com/charts-latest/pivotTable.html[corresponding chapter] in the *Charts* manual.
...
The pivot table displays all properties for <<structure_groovy,Groovy>>, <<structure_sql,SQL>>, <<structure_jpql,JPQL>> and <<structure_json,JSON>> datasets. These properties should be set as the pivot table's properties on the *Properties options* tab of the template editor:
...
* *Filter function* - JavaScript function that will be used for filtration.

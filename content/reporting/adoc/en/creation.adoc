...
Jasper formatter allows you to use Jasper-kind templates without Jasper Server. The template will be processed by the report engine, providing an output of defined type, see <<template_to_output>>.

You can create JRXML templates using Jasper tools (for example, Jaspersoft Studio) or, simply, a text editor. Each report band must have a corresponding `band` element in the template which is, in turn, placed within one of the standard Jasper report sections (which are also named as the bands in Jasper terms):

* `title` — at the beginning of the report;
* `pageHeader` — on the top of each page;
* `columnHeader` — on the top of report column;
* `detail` — one for each band dataset iteration;
* `columnFooter` — at the bottom of report column;
* `pageFooter` — at the bottom of each page;
* `lastPageFooter` — at the bottom of the last page (in place of `pageFooter`);
* `summary` — at the end of the report;
* `noData` — in place of all other sections, if a dataset contains no records;
* `background` — in the background of each page.

The report engine puts all report band data in one datasource: `JRBandDataDataSource`, which contains data as the tree with the `Root` band as the root, and passes the `CubaJRFunction` instance to the template as the main datasource that can be addressed using the `REPORTING` parameter. Declaring this parameter in the report template is not mandatory, it will be added automatically if omitted, but if you want to compile the template in Jasper IDE, you need to declare this parameter explicitly.

The `REPORTING` parameter supports two functions:

* `dataset` -  gets the sub-datasource from the main datasource that can be used, for example, in tables or in subreports as subDataset. This method searches among the children of the root band for the band with specified name and creates a new datasource with this band data as a new root. For example:
+
[source, xml]
----
...
The table displays columns for all properties for SQL, JPQL datasets. Iа the Entity/List of Entities dataset is used, the table will display only the columns for selected attributes.

...
Let's consider the following example: depending on the `createTs2` report parameter you need to select one of the query conditions: `e.create_ts < ${createTs2}` or `e.create_ts < current_timestamp`.
...
Thus, the template engine enables you to modify SQL/JPQL queries using Groovy.
--

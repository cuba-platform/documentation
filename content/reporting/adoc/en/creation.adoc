...
====
+
By default, for SQL queries the `Main` data store (see Developer's Manual) is used. If there is any additional data store included in the project, the *Data store* field will become visible, and the required data store name can be put in.
...
+
For JPQL queries the data store should be set, if not, the default `Main` data store is used. To use an entity from an additional datastore, put its name in the *Data store* field.
...
----
+
In case the additional data store is used, its name should be used as a parameter for transaction and `EntityManager`. By default the `Main` data store is used.
+
[source, groovy]
----
def tx = persistence.createTransaction('additionalStore')
try {
    def em = persistence.getEntityManager('additionalStore')
...
    tx.commit()
} finally {
    tx.end()

...
. `<band_name>`*_dynamic_header* - this band is copied to the right like a vertical band with table columns header.

. `<band_name>`*_master_data* - this band is copied downwards like a horizontal band with table rows header.

. an empty matrix content band that will fulfil the matrix cells.

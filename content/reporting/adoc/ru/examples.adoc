...
Для создания перекрестного, или матричного, отчета необходмимо выбрать ориентацию полосы *Crosstab* на вкладке <<structure,Report structure>> редактора отчетов. При выборе этой ориентации к полосе автоматически добавляются три набора данных:
...
Для примера создадим матричный отчет для сущности `Order` из демо-приложения https://github.com/cuba-platform/sample-sales[Sales] со следующей структурой:
...
* набор данных `orders_master_data` возвращает имена и идентификаторы покупателей, выбранных пользователем в качестве <<parameters,внешнего параметра отчета>>:
+
[source, sql]
...
В шаблоне нужно создать <<template_xls_regions,именованные регионы>> для всех трех наборов данных полосы `orders`, а также, дополнительно, регион для заголовка столбца: `<band_name>_header`. В нашем случае это `orders_header`.

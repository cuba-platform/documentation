...
Таблица в этом шаблоне привязана к дочернему источнику данных `subDataset`. Элемент `title` обращается к данным полосы *Header* напрямую. Вы можете заранее посмотреть, как будет выглядеть отчёт, открыв шаблон в визуальном редакторе Jasper.

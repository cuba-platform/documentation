...
+
--
Dependency injection is the main way of getting references to visual components and API endpoints in the screen controller code. Of course, you can write a field of the required type and annotate it manually, but Studio has a special window that allows you to select an object from the list and create the approriate field automatically. This window is opened if you press Alt+Insert (Cmd+N) and select *Inject...* in the *Generate* popup menu:
